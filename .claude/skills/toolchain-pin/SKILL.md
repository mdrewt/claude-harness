...
| **spacetime** | global `spacetime version use <X>` — **no per-project file exists** | Can't be file-pinned. Document the version in the project `AGENTS.md` and verify with `spacetime --version`. Optionally a `just check-toolchain` guard. **Pin the `spacetimedb` Rust crate to the SAME version** — the crate version *is* the product version (since crate 2.0.0), so a drifting crate pin means you write against a different API than you deploy on. A CI workflow that pins the CLI is a third copy: keep all three in lockstep, or add a tripwire that fails when they diverge (monster-realm's `observability-metrics-contract` B4 is a working example). |
