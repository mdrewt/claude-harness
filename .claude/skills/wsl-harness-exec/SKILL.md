...
- **`codebase-memory-mcp cli <tool>` returns `{"error":"project not found or not indexed"}` even with a correct-looking name** → the `project` arg was omitted; the CLI query tools require it explicitly. **Avoid:** run `codebase-memory-mcp cli list_projects` first, pass `project` = that **exact** name. All other cbm/CodeGraph usage, routing, and gotchas: **`code-intel` skill (SSOT)** — note cbm CLI queries serve a **snapshot** (0.9.0's auto_watch only helps a resident MCP server; re-index explicitly — there is no "catch up" for CLI use), `http://localhost:9749` is the graph UI only (MCP is stdio), and per-project SQLite lives in `~/.cache/codebase-memory-mcp/` — never write it directly while the server is live.
