...
## 2026-07-15 — m16.5d TERMINAL STATE — PR #187 OPEN, local `just ci` EXIT=0

**Branch:** `feat/m16.5d-trade-runtime-coverage`, tip `a412b27`, **PR:** https://github.com/mdrewt/monster-realm/pull/187
**ADR:** 0115 (accepted); **ADR next-free = 0116**
**Worktree:** `.claude/worktrees/m16.5d`

**What shipped:** Three deliverables: (1) `window.__mrTrade` test hook in `client/src/main.ts` — four trade reducer wrappers + `allTradeOffers()/allPlayers()` queries, all BigInt→string for Playwright boundary, mirrors `window.__game` pattern; (2) `client/e2e/trade-full.spec.ts` — two-context Playwright e2e (two `chromium.launch()` for distinct SpacetimeDB identities), hook-existence test + full propose→respond→confirm flow with monster conservation assertion; (3) `evals/trade-escrow-guards.eval.mjs` — TR-13 added (`attempt_recruit/escrowed_item_qty`, resolves TR-12 tag collision with `use_battle_item`), `bodyHasGuard` hardened with RT-SEC-02b string-literal stripping. 1193 Rust tests, 943 vitest, 61 evals (12 guard sites TR-2..TR-13). EXIT=0.

**Key trap (RT-SEC-02b):** `bodyHasGuard` used `countOccurrences(body, guard + '(')`. A string literal like `log::info!("reject_if_monster_in_trade(x)")` contains the needle and would be counted as a real guard call site (false positive). Fix: strip Rust string literal contents with `body.replace(/"(?:[^"\\]|\\.)*"/g, '""')` before counting. The red-team found this and added a RED tooth (RT-SEC-02b); implementation fixed it GREEN.

**Key trap (TR-13 collision):** `attempt_recruit` had `escrowed_item_qty` guard at taming.rs:99 since M16.5a, but it was tagged TR-12 in the eval GUARD_SITES (same as `use_battle_item`'s TR-12). Only the eval was fixed — taming.rs was NOT changed.

**Supervisor owns squash-merge.** After merge, M16.5 Ninth-review residuals is fully CLOSED. ADR next-free = 0116.

---

## 2026-07-15 — m16.5c TERMINAL STATE — PR #185 OPEN, local `just ci` EXIT=0

**Branch:** `feat/m16.5c-trade-client-completion`, tip `94e8912`, **PR:** https://github.com/mdrewt/monster-realm/pull/185
**ADR:** 0114 (accepted); **ADR next-free = 0115**
**Worktree:** `.claude/worktrees/m16.5c`

**What shipped:** Three M16.5 ninth-review residuals closed: (1) Overlay symmetry e2e test (KeyQ/KeyH/KeyG guards already correct; verified in place); (2) Typed TradeStatus `'Pending' | 'ConfirmedByCounterparty'` with exhaustive switch + TS2366 compile-error safety; (3) Render hygiene — feedback clears on status change, buttons disabled during pending, **critical red-team fix (TV-4)**: `finally()` uses `querySelectorAll` loop instead of captured btn closure to prevent deadlock when `render()` fires mid-flight. 943 tests, 61 evals, EXIT=0.

**Key trap (render hygiene):** SpacetimeDB batch listener can fire `render()` while a reducer Promise is mid-flight. Old code: `finally()` captured btn by closure, then `render()` orphaned it via `innerHTML=''`. New code: `querySelectorAll<HTMLButtonElement>('button')` loop scans the live DOM. UI deadlock symptom: buttons stuck disabled; root cause: unreachable button reference.

**Supervisor owns squash-merge.** Commits: 40af2bc + 560df95 + 94e8912. After merge, master should be GREEN.
...
## 2026-07-15T00:26:22Z — supervisor tick mr-sup-cowork-20260715T000546Z-1839983-2109
MERGED PR #178 (m16c PvP evals) -> master 6111bc2; review pass VERDICT FIXED (df60c76). ADR-0111 index chore PR #179 merged -> master 564c473, CI GREEN. **M16 CLOSED.** 6111bc2 push-CI e2e failure = dialogue-M13.5c flake (untouched by diff; green on 564c473) — queued deflake follow-up. Composite-launched **m16.5a** (battle-trade interlock both ways + m7b_2 real body + revival-gate assert check; ADR 0112 reserved; leader 1841473, claude 1841476, sonnet, detached). Worktree/branch m16c cleaned. Next: watch m16.5a -> merge on VERDICT/green.

## 2026-07-15T02:22Z — supervisor mr-sup-cowork-20260715T020622Z-1926666-23706 — m16.5a MERGED
- PR #180 (feat/m16.5a-battle-trade-interlock) squash-merged → master 4ea2f67; ADR-0112 index chore PR #181 merged → master 3c5efbe (auto-merge rejected "clean status", merged manually per fallback). Worktree + branches cleaned.
- Audits: orchestration CLEAN (sonnet-4-6; tester/reviewer/red-team/verifier all present, 4 Agent invocations); gating-test CLEAN (zero removed tests/asserts; #[ignore] hits were additions strengthening the revival gate). Cost $11.30, attempts 1.
- Touches overrun (known recurring): docs/knowledge/reducers/*.md + trading_tests.rs undeclared. SOLO serial → no collision → merged. Brief-tightening follow-up remains queued.
- master CI after merge: `ci` green, `e2e` RED on 4ea2f67 = known dialogue-M13.5c toBeHidden 20s flake (untouched by this slice). Rerun of failed job triggered (run 29383438514); fresh run also in flight on 3c5efbe. NEXT TICK: verify green; if red again treat as fix-red-master target (deflake M13.5c is already a queued follow-up — consider promoting it).
- ANOMALY: orphaned unmerged (UU) scripts/adr-digest.mjs in the MAIN checkout, mtime 2026-07-15T00:42Z, no MERGE_HEAD. NOT resolved/discarded: working-tree conflicted copy + all 3 index stages preserved at memory/projects/strays-m16.5a-merge-20260715/; index entry reset to HEAD to unblock --ff-only. Human: if that edit was yours, recover it from there. Untracked strays .claire/ and docs/memory-cards/ left untouched.
- No composite launch (doubt rule: e2e red pending rerun + stray-conflict anomaly). Next eligible: 16.5b per SS6 sequencing.

## 2026-07-15T03:52Z — interactive session (Drew-directed) — all m16.5a tick risks RESOLVED
- **M13.5c e2e deflake MERGED:** PR #182 → master a2b0a91. Root cause: hard in-loop toBeHidden(20s) threw before the advance-retry loop could absorb slow row-delete propagation (both 07-15 failures at line 406). Slow attempt now = failed attempt (continue); choice-click softened vs overlay vanishing mid-attempt. Teeth intact (post-loop advanced===true + final toBeHidden). Verified: e2e 3× green on the PR (attempts 1–3), tsc+biome clean.
- **UU adr-digest.mjs anomaly ROOT-CAUSED + CLOSED:** the m16.5a run ran `git -C <main-checkout> stash`/`stash pop` while diagnosing pre-existing biome noise; the pop conflicted on stash@{0} (supervisor-stray 2026-07-13: ADR 0091/0092/0097 header edits + adr-digest comment). ALL stash content verified already superseded in master (m-infra-d2 backfill). Stash archived (strays-m16.5a-merge-20260715/stash0-supervisor-stray-20260713.patch) then dropped (dangling 45e2a361 until gc). Older supervisor stashes (5) left untouched.
- **Brief template hardened** (mr-brief-template.md): (1) always-in-scope companions defined — test-file siblings of declared code + docs/adr/** (reserved number)/docs/knowledge/**/CHANGELOG/ARCHITECTURE; (2) runs must list a `touches-delta:` heading in the PR body; (3) NEVER run mutating git against the main checkout (read-only ok). Supervisors: declare test siblings + docs/knowledge/** in touches:.
- Next: verify a2b0a91 master run green → launch 16.5b (ADR 0113 next free).

## 2026-07-15T04:12:18Z — supervisor tick mr-sup-cowork-20260715T040525Z-1930541-7955 — IN-PROGRESS: launched m16.5b
Master a2b0a91 CI verified GREEN (run 29387430960). No open PRs, nothing parked. Launching m16.5b (conservation at receiver caps — reject-not-clamp in confirm path; ADR reserved 0113) per state-file directive from the 2026-07-15 interactive session. Brief: /tmp/mr_pass_m16.5b.md. Touches: server-module/src/trading.rs, game-core/src/trading/rules.rs, trading tests, evals/trade-conservation.eval.mjs.

## 2026-07-15T06:20Z — mr-sup-cowork-20260715T060601Z-2021037-10017 — m16.5b MERGED

- PR #183 (feat/m16.5b-receiver-cap-headroom, ADR-0113) squash-merged at 06:10Z → master 5188db2; post-merge master CI GREEN (run success on 5188db2).
- Audits: orchestration CLEAN (tester/reviewer/red-team/verifier all invoked, model claude-sonnet-4-6, cost $11.15, 1 wrapper attempt); gating-test CLEAN (+13 #[test], 0 removed, no skips/ignores).
- Touches-delta beyond declared set: game-core/src/lib.rs (re-export), server-module/src/inventory.rs (MAX_ITEM_STACK SSOT move), docs/knowledge companions — all documented in PR body; no in-flight siblings, so no collision risk; merged rather than parked.
- ADR index reconciled via doc-only chore PR #184 (merged 06:19Z; next-free bumped to 0114).
- Cleanup done: remote+local branch deleted, worktree .claude/worktrees/m16.5b removed, main checkout ff'd to 5188db2.
- Two DC session deaths mid-tick (during merge + during chore-PR auto-merge); both reconciled from live GitHub state, no repo damage.
- Next: composite launch m16.5c (per queue: 16.5c -> 16.5d -> 16.5e -> 16.5f -> 16.5g), ADR reserve 0114.

## 2026-07-15T06:30:58Z — IN-PROGRESS breadcrumb: launching m16.5c (trade client completion; ADR 0114 reserved; touches client/src/** + client/e2e/trade.spec.ts). Composite launch after m16.5b merge.
...
## 2026-07-17T12:40:35Z — supervisor tick mr-sup-cowork-20260717T120940Z-1720978-15309 — m17c MERGED, M17 slices complete
- PR #198 (feat/m17c-ranked-evals-tail) was CONFLICTING after m17b's merge. Conflict set exactly {ARCHITECTURE.md, docs/adr/DIGEST.md} = doc-aggregation set -> supervisor resolved deterministically: union/append (M17b para then M17c para; 0120 row then 0121 row), then regenerated DIGEST.md via scripts/adr-digest.mjs after the adr-digest eval flagged staleness (header count 86->87). One remote red fix-cycle.
- Squash-merged #198 at 12:27Z (522ee4a). Gating-test audit CLEAN; orchestration audit CLEAN (fable-5, 7 agents). Chore PR #202 (ADR index 0121 row, range 0035–0121; next-free stays 0122) merged 12:35Z (69e3706). Master CI GREEN on both. m17c worktree + local/remote branch removed.
- M17a/b/c all merged. NEXT TICK: M17 post-integration verification per spec section-5 tail -> declare M17 CLOSED; then M17.5 per the 2026-07-17 playtest-first replan (see mr-state queue).
- No composite launch: heavy DC session churn this tick (5 shell deaths); conservative record-and-exit.

## 2026-07-17T14:16:05Z — supervisor tick mr-sup-cowork-20260717T141159Z-1758559-7094 — M17 CLOSED (verification passed) · standdown before m17.5a launch
- Fast-path: no live runs, no .done, no locks. Master GREEN @ 69e3706; no open PRs; single clean worktree; remote mapping OK.
- **M17 post-integration verification (spec §5 tail): PASSED.** just-ci green-and-meaningful on integrated tip (m17c squash 522ee4a + chore 69e3706, both CI success); nightly SUCCESS on 79d26b0 (validates #194 fix); bindings-drift eval in ci target => drift 0; schema snapshot includes `profile`; cross-slice contracts CI-enforced (RL-10 apply_pvp_rating single-call-site test in pvp_tests.rs; guard-string + PvE-exclusion evals in evals/); ranked-forfeit e2e (RL-18) in m17c CI. **M17 CLOSED** (m17a #196, m17b #199, m17c #198 + chores #201/#202).
- Next target selected: **m17.5a** (side-B battle-guard unification; SERIAL structural, no sibling; ADR 0122 to be assigned at launch). Launch ABORTED at final re-probe: untracked `specs/monster-realm-v2/mechanics-survey.md` mtime 14:09:16Z (<6 min) — active spec curation in the harness repo (likely Drew, playtest replan). consecutive_standdowns=1.
- Stray-branch note: local chore/m15c-adr-index + chore/m16c-adr-index and worktree-agent-* branches remain (no attached worktrees); left untouched.
- Next tick: launch m17.5a per M17.5 spec §2/§5 (17.5a-1..5, decision-A-amended eval instead of BattleKind column), brief from template, ADR_NUM=0122.

## 2026-07-17T16:13:56Z — supervisor tick mr-sup-cowork-20260717T160832Z-1795149-28968 — IN-PROGRESS: launching m17.5a
Gates clean (no live runs, master GREEN @ 69e3706, no open PRs, probe clean x2). M17 closed last tick. Launching m17.5a (M17.5 §2 17.5a-1..5, side-B ongoing-battle guard unification; SERIAL structural, no sibling; ADR reserved 0122; touches server-module/src/{battle,raising,evolution,guards}.rs + tests + optional evals/battle-reducer-security.eval.mjs). Brief /tmp/mr_pass_m17.5a.md. Detached via mr-launch.sh.

## 2026-07-17T~13:4xZ — m17.5a TERMINAL: PR #203 OPEN, local full `just ci` green — supervisor to merge

- Branch `feat/m17.5a-sideb-battle-guards` @ bf0011e (pushed). PR https://github.com/mdrewt/monster-realm/pull/203. Local full `just ci` EXIT=0 on PR head (288 module + full-workspace Rust tests; all evals incl. the fixed npc-dialogue-quest-security; clippy/fmt clean; client typecheck+vitest); remote ci+e2e running (run 29600931691).
- Delivered EARS 17.5a-1..5 (ADR-0122): hoisted `is_in_ongoing_battle` into guards.rs as SSOT two-function split (pure iterator-core `is_in_ongoing_battle_either_role` + thin ctx wrapper; WILD refinement verbatim; player-iter first/opponent-iter second, eval order-pinned); rewrote 4 PvE sites to `if is_in_ongoing_battle(...)` (byte-stable strings); chained opponent_identity in evolve(×1)/fuse(×2) reject_if_in_battle; DELETED pvp.rs private copy + imports shared. Classification eval (17.5a-5) pins is_ranked_pvp negative-space via per-site (file,fn,RHS-form) allowlist over exactly-3 insert sites — replaces deferred BattleKind column (spec Decision A amended).
- Gates: 7 pure-core unit tests + 4 side-B seam tests + eval C1-C4 (bad/good teeth). Red-team ran **5 live mutation spot-checks — ALL BIT**. Reducer-security review-lens CLEAN. **mutate-server re-baselined 308→299** (513 mutants, ADR-0118 §4).
- Orchestration: planner → plan-fan (reviewer BLOCKER B-1 seam-parity + red-team 6 findings → all dispositioned) → tester RED → test-fan (reviewer B-1 naming + red-team 4 BLOCKER gate-holes: discarded/reversed iterator, wrong-identity, chain-structure/fuse-count → all hardened) → implementer (5 prod files, gating tests untouched) → impl-fan (reviewer+red-team SOUND+CLEAN, reducer-security review-lens CLEAN) → doc-keeper (ADR-0122, ARCHITECTURE, DIGEST+knowledge) → verifier FAIL-caught the npc-dialogue eval break → tester fixed C6 → full ci green. Model claude-fable-5.
- **touches-delta (supervisor audit):** `server-module/src/pvp.rs` (hoist-required, expected); **`evals/npc-dialogue-quest-security.eval.mjs`** (NOT declared — heal_party refactor blast-radius; C6 needle taught the SSOT form + GOOD_HEAL_PARTY_WITH_SSOT_GUARD fixture, bad-fixture teeth preserved; SERIAL slice so no sibling owns it); `justfile` (cap 308→299); standard doc outputs (ADR-0122, DIGEST, 11 knowledge cards, ARCHITECTURE, docs/specs/m17.5a-plan.md). CHANGELOG + adr-README untouched. ADR next-free = 0123.
- **Ops note:** fresh `git worktree add` has NO client/node_modules → `just ci` lint step fails "biome not found" until `cd client && npm install --include=dev`. Done this run. Also: background-bash "exit code 0" notification was UNRELIABLE for `just ci` (reported 0 while `recipe eval failed`); trust the recipe-failed line / an explicit `EXIT: $?` echo, not the harness code.
- Residuals in ADR-0122 (candidate follow-ups, NOT fixed): D4 warp-guard player-only (side-B warps mid-PvP); D7 care/train no battle guard (not laundering today — HP snapshotted, weaponized if write-back recomputes from live stats); D2 movement pre-check player-only wasted-RNG-draw.
- Progress memo: memory/projects/monster-realm-m17.5a-progress.md. Codebase-memory graph: main checkout unchanged (worktree slice) — reindex after merge. Worktree `.claude/worktrees/m17.5a` removable after merge.
- NEXT: supervisor polls remote CI on #203 → squash-merge to master; then 0122 ADR-index chore. Next M17.5 slices per spec §5: 17.5b (trade same-item near-cap conservation), 17.5c/d/e (server-module), 17.5f (evals/e2e), 17.5g (docs) — 17.5g + much of 17.5f disjoint/opportunistic.

## 2026-07-17T18:54:41Z — supervisor tick mr-sup-cowork-20260717T184810Z-2472641-12351 — m17.5a MERGE RECONCILED (takeover)
- **Takeover of a dead-owner completed merge.** Fast-path: no live rooted-run pids (1797683/1797689 dead), chain-owner lock STALE (owner pid 2459193 dead, heartbeat 18:22:17Z ~ -31min) mid `merge-chore-204`. No active human session (no resident IDE claude; zero writes <6min). Took over the mutex.
- **Live ground truth:** PR #203 (feat/m17.5a-sideb-battle-guards) MERGED 18:11:32Z → 9ef0b03; chore PR #204 (chore/m17.5a-adr-index, ADR-0122 row) MERGED 18:22:20Z → **292961c = master tip**. No open PRs. The prior tick (2459193) landed BOTH merges on the remote, then died before recording/cleanup.
- **master CI GREEN @ 292961c** (check-runs ci=success, e2e=success). Worktree clean at master; local m17.5a branches already gone.
- **Audits (post-hoc, merge already gated by prior tick):** gating-test integrity CLEAN — no removed assertions / skip / `#[ignore]` / `.only` across guards_tests.rs (+244 new), evolution_tests.rs (+415), battle-reducer-security.eval.mjs (+1226); npc-dialogue eval change is fixture-only (SSOT needle). Orchestration CLEAN — model claude-fable-5; planner/tester/reviewer/red-team/verifier/doc-keeper/review-lens all invoked; run cost $54.39, ATTEMPTS=1.
- **Cleanup done:** removed per-run lock .harness-runner.m17.5a.lock; deleted merged remote branch feat/m17.5a-sideb-battle-guards (chore branch already gone); cleared stale /tmp/mr_pass_m17.5a.done + stop flags. mr-state: inflight=[], master=292961c, adr_next_free=123, consecutive_standdowns=0.
- **Strays (untouched, human):** untracked `.claire/` and `docs/memory-cards/` in the checkout — left as-is (not supervisor-owned; no --ff-only needed, already at tip).
- **No composite launch** — heavy DC session churn this tick (3 shell deaths during git/gh calls); the merge was completed by a prior tick, so per rule 1 this tick records the reconcile and exits. **NEXT TICK: launch m17.5b** (M17.5 spec §5: trade same-item near-cap conservation; ADR_NUM=0123; brief from template). 17.5c/d/e server-module; 17.5f evals/e2e + 17.5g docs partly disjoint (opportunistic fan-out later).

## 2026-07-18T03:25:49Z — supervisor tick mr-sup-cowork-20260718T032549Z-2545275-21235 — IN-PROGRESS: launching m17.5b
Gates clean (fast-path: no live runs/locks/.done; master GREEN @ 292961c, ci+e2e success live; no open PRs; single clean worktree; remote mapping OK; active-session re-probe clean x2 — no writes <6min, no resident IDE). m17.5a MERGED+reconciled last tick. Launching **m17.5b** (M17.5 spec section 17.5b: HIGH->MEDIUM trade same-item near-cap conservation — debits-before-credits in confirm_trade + netted currency headroom; proof-of-teeth 9999/9998 conservation + genuine-over-cap Err). SERIAL server-module, no sibling; ADR reserved **0123**; touches server-module/src/trading.rs + game-core/src/trading/rules.rs + trading_tests.rs + one eval. Brief /tmp/mr_pass_m17.5b.md. Model fable (Drew 2026-07-15 directive; mr-launch.sh default). Detached via mr-launch.sh under setsid.
  - **LAUNCH CONFIRMED 2026-07-18T03:30:52Z:** leader pid 2547560 (SESS==PID, detached from supervisor shell), claude child 2547565, model `claude-fable-5` asserted from log. Per-run lock `.harness-runner.m17.5b.lock` written; mr-state inflight=[m17.5b], adr_next_free 123->124, consecutive_standdowns=0; ledger `launched` line appended. Supervisor shell died twice this tick (DC churn) during network/poll waits — run unaffected (setsid-detached). Chain-owner mutex released. NEXT TICK: fast-path standdown while leader alive & no .done; then poll remote CI on the m17.5b PR -> squash-merge -> ADR-0123 index chore.

## 2026-07-18T~06:1xZ — m17.5b TERMINAL: PR #205 OPEN, local full `just ci` green — supervisor to merge

- Branch `feat/m17.5b-trade-conservation` @ f85b43a (pushed). PR https://github.com/mdrewt/monster-realm/pull/205. Local full `just ci` EXIT=0 on PR head (1282 Rust + 1001 client tests; 61/61 evals incl. trade-conservation 8-criteria); remote ci+e2e running (run 29631274446). Verifier verdict APPROVE; gating-test RED→GREEN integrity audit CLEAN.
- Delivered EARS 17.5b-1/2/3 (ADR-0123, amends ADR-0113): `SwapPlan::ordered_steps()` game-core debits-before-credits SSOT (credit variants `to_initiator`, inverted once at emission); confirm_trade single exhaustive 4-arm match, legacy loops deleted; currency headroom netted INLINE in check_headroom args (cap-headroom-only; broke sender → spend_currency Err + rollback, documented).
- Gates: 9 new executed game-core tests (@9999/@9998 conservation per-party, over-cap reject, netting-sensitivity flip, broke-sender boundary, constructive proptest, partition/parity/zero-currency teeth) + EA-CONSERVATION-ORDER-01 (arg-span netting needles + discard/split-loop/dead-var teeth) + EA-CONSERVATION-ORDER-INLINE-01 + eval APPLY_ORDER (8th criterion, string-strip added to shared pipeline). **Mutation: ordered_steps targeted 0 missed; full mutate-server 299/513 = EXACTLY the ADR-0118 cap — no re-baseline needed** (all 8 confirm_trade survivors pre-existing ==→!= class).
- Orchestration (model claude-fable-5): planner → plan-fan (reviewer+red-team, 20 findings adopted) → tester RED → test-fan (2 BLOCKERs: proptest plan/headroom divergence ~17% false-panic; netting dead-var bypass → arg-span) → implementer GREEN (tests untouched) → impl-fan (reviewer+red-team+reducer-security CLEAN ×6) → doc-keeper → verifier APPROVE. **Two harness process deaths recovered from on-disk state + wip checkpoints; desync-guard skipped (no netcode surface, deliberate).**
- **touches-delta (supervisor audit):** declared set + `game-core/src/{lib,trading/mod}.rs` (ApplyStep re-export, mechanical) + standard doc outputs (ADR-0123, DIGEST, 2 knowledge cards regen, ARCHITECTURE M17.5b para, docs/specs/m17.5b-plan.md). CHANGELOG/adr-README/run.mjs untouched. **Gating-test edits post-lock (all verifier-audited):** 2f71683 orchestrator concat-split (byte-identical needles) + 0400d87 good-fixture strengthened to arg-span + red-team-added INLINE-01 test.
- **Ops notes:** doc-keeper wrote ADR/ARCH to the MAIN checkout (m17a trap repeat) + fabricated details (inverted root cause, "synthetic ledger", 286-char Decision line — adr-digest gate caught it); recovered via plain file copies (NO git mutations on main checkout), ADR rewritten from ground truth. Main checkout verified clean (only pre-existing strays .claire/, docs/memory-cards/). Shell cwd resets recurred — absolute paths mandatory.
- Residuals: 17.5c shop buy/sell headroom (next slice); consume_one O(qty) → consume_qty candidate; ordered_steps↔check_headroom pairing type-unenforced (ADR-0123 D6). ADR next-free = 0124. Memory card memory/projects/monster-realm-m17.5b.md. Worktree `.claude/worktrees/m17.5b` removable after merge; codebase-memory graph reindex after merge.
- NEXT: supervisor polls remote CI on #205 → squash-merge → 0123 ADR-index chore. Then 17.5c/d/e per M17.5 spec §5.

## 2026-07-18T06:33Z — m17.5b MERGED (PR #205) + ADR-index chore (PR #206); master GREEN — supervisor tick

- **Merged m17.5b** PR #205 squash -> master `18bf4e6`; master CI run 29633602479 **SUCCESS**. Trade same-item near-cap conservation: debits-before-credits ordering + netted currency headroom (ADR-0123, amends ADR-0113).
- **Pre-merge audits all CLEAN:** orchestration (13 subagents: planner/tester/reviewer/red-team/verifier/doc-keeper/review-lens; model claude-fable-5) CLEAN; gating-test integrity (no `#[ignore]`/`.skip`/`.only`, rust `#[test]` 31->33, 2112 ins / 5 non-assertion del, 0 assertions removed) CLEAN; touches over-declared by game-core `lib.rs`+`trading/mod.rs` (mechanical ApplyStep re-export, verifier-audited), no structural-set files, single in-flight so no sibling collision (NOT a park).
- **ADR-index chore** PR #206 squash -> master `90090fc` (doc-only, merged on green): added 0123 row, range 0035-0123, next-free **0124**. Merged manually on green per branch-protection (--auto rejected).
- Cleanup: m17.5b worktree removed, local branch `feat/m17.5b-trade-conservation` deleted, per-run lock cleared, `/tmp/mr_pass_m17.5b.done` removed. Main checkout at `90090fc [master]`; only pre-existing strays (.claire/, docs/memory-cards/) untracked.
- **Robustness:** 3 supervisor-shell deaths this tick (DC session churn); each reconciled from live `gh`/`git` ground truth. Both merge commands timed out but merges (#205, #206) landed. m17.5b run cost $39.9986, 3 wrapper attempts.
- NEXT: next tick launches M17.5 sec5 **17.5c** (shop buy/sell headroom) fresh slice, reserve ADR 0124 on launch; assert model post-launch (last run claude-fable-5 per Drew 2026-07-15). Remaining M17.5: 17.5c/d/e server-module, 17.5f evals/e2e, 17.5g docs. Confirm chore `90090fc` master CI green next tick.

## 2026-07-18T08:17:35Z — m17.5c LAUNCHING (fresh slice) — supervisor tick
- **Action:** launch M17.5 §17.5c (shop buy/sell reject-not-destroy at receiver caps; reuse game_core::check_headroom). Fresh worktree off origin/master `90090fc`. ADR **0124** reserved (adr_next_free 124→125). Serial (N=1) — no clean disjoint fan-out partner (17.5f/17.5g both overlap rules.rs/evals).
- **Pre-launch gates:** master CI GREEN @ `90090fc` (CI run 29633... success; check-runs ci+e2e success; #205 m17.5b + #206 ADR-index chore both confirmed merged). No open PRs, no inflight/awaiting-merge, no parked slices. Active-session probe clean (no IDE claude, no foreign writes) at gate top + pre-launch re-probe.
- **Model:** mr-launch.sh default `fable` (Drew 2026-07-15 directive; last run m17.5b confirmed claude-fable-5). Will assert post-launch.
- run_id `mr-sup-cowork-20260718T081442Z-3252440-29576`; brief /tmp/mr_pass_m17.5c.md.

## 2026-07-18T~10:2xZ — m17.5c TERMINAL: PR #207 OPEN, local full `just ci` green — supervisor to merge

- Branch `feat/m17.5c-shop-headroom` @ db50e18 (pushed). PR https://github.com/mdrewt/monster-realm/pull/207. Local full `just ci` EXIT=0 on PR head (1,296 Rust + 1,001 client tests; 61+ evals incl. shop-reducer-security 7/7; clippy/fmt/semgrep/wasm clean); remote ci+e2e running (run 29639527821). Verifier verdict APPROVE; gating-test RED→GREEN integrity audit CLEAN.
- Delivered EARS 17.5c-1/-2 (ADR-0124, amends ADR-0113/ADR-0082): `check_item_headroom`/`check_currency_headroom` pure primitives; `check_headroom` delegates UNCONDITIONALLY (incoming>0 gate inside the currency primitive, anti-normalization pinned); `buy` raw-count check before spend, `sell` raw-balance check before consume (RAW not escrow-netted — spend-lock vs receive-lock, documented).
- Gates: 11 game-core boundary tests + delegation SSOT tooth + 2 hardened source-scans (paren-anchored split-literal needles, string-literal stripping, `?`-propagation windows, provenance + argument-identity + lookup-filter pins, cfg-forbidden) + eval 5→7 criteria with 7-bad/1-good teeth each. **Mutation: rules.rs targeted 60 mutants 0 missed (zero-tolerance holds); economy.rs 15 missed ALL pre-existing baseline class — the one net-new survivor killed by the lookup-filter pin (include_str! sees mutated source), mutate-server stays at the 299 cap, NO re-baseline.**
- Orchestration (model claude-fable-5): planner → plan-fan (reviewer B-1/B-2 + red-team 12 findings, F3 refuted via validate_shops) → tester RED (verified red on all 3 surfaces) → test-fan (4 gate-hole classes closed) → implementer GREEN (production only) → impl-fan (reviewer SOUND + red-team 6 live mutation spot-checks + reducer-security CLEAN 6/6) → doc-keeper → verifier APPROVE. desync-guard skipped (no netcode surface, deliberate).
- **touches-delta (supervisor audit):** declared set + `game-core/src/{lib,trading/mod}.rs` (mechanical re-exports) + `docs/adr/DIGEST.md` (adr-digest regen) + `docs/specs/m17.5c-plan.md` + 2 knowledge cards (buy/sell) regen. CHANGELOG/adr-README/run.mjs untouched. Gating-test edits post-lock all verifier-audited (strengthening/doc-only; listed in PR body).
- **Ops notes:** doc-keeper wrote ADR+ARCHITECTURE to the MAIN checkout AGAIN (3rd repeat) + fabricated ADR details (invented HeadroomError type, wrong signatures) — recovered via plain file copies (`git show HEAD:` redirect + rm; NO git mutations on main checkout), ADR rewritten from ground truth by orchestrator; main checkout verified clean (only pre-existing .claire/, docs/memory-cards/ strays). adr-digest 240-char Decision gate bit once (242→trimmed). guard-bash hook blocked an `rm -rf /tmp/...` + worktree-add compound — split commands to avoid.
- Residuals (ADR-0124 D5): client UX for the two cap errors (named deferral); consume_qty batching; cfg-ban is per-body mechanical only. Spec §17.5c delivery note updated with PR #207 (harness repo). ADR next-free = 0125. Memory card memory/projects/monster-realm-m17.5c.md. Worktree `.claude/worktrees/m17.5c` removable after merge; codebase-memory graph reindex after merge (main checkout unchanged).
- NEXT: supervisor polls remote CI on #207 → squash-merge → 0124 ADR-index chore. Then M17.5 remaining per spec §5: 17.5d (ranking profile.name mirror), 17.5e (challenge TTL, schema-additive), 17.5f (PvP runtime coverage, client/evals), 17.5g (docs-only, runnable anytime).

---

## 2026-07-18T10:25Z — mr-sup-cowork-20260718T101128Z-3480061-13166 — MERGED m17.5c
- **Action:** merged slice **m17.5c** — PR #207 `feat(m17.5c): shop buy/sell reject-not-destroy at receiver caps (ADR-0124)` squash-merged to master @ **84db318**, branch deleted.
- **Pre-merge gates (all pass):** mergeStateStatus CLEAN / MERGEABLE; CI green (ci + e2e, run 29639527821). Orchestration audit **CLEAN** — model `claude-fable-5`; subagent evidence: tester + reviewer + verifier + red-team + review-lens + planner + doc-keeper all present (14 Agent invocations). Gating-test integrity **CLEAN** — no removed/skipped/`#[ignore]`d tests, no removed assertions. Diff files ⊆ declared `touches` + trivial `game-core` re-export wiring (`check_item_headroom`/`check_currency_headroom` added to lib.rs & trading/mod.rs `pub use`); no structural-set files; no in-flight siblings.
- **Doc-aggregation:** ADR-0124 index reconciled via **doc-only chore PR #208** (`chore/m17.5c-adr-index`) — next-free `0124`→`0125`, 0124 index row added. `--squash --auto` accepted (allow_auto_merge working) and **landed @ a0d5743**. Index verified on master.
- **master CI after:** **GREEN @ 84db318** (slice CI success). Head now **a0d5743** (chore doc-only); its CI `in_progress` — doc-only, expected green (next tick reconfirm).
- **Cleanup:** merged m17.5c worktree + local branch `feat/m17.5c-shop-headroom` removed; temp ADR worktree removed; main checkout fast-forwarded to a0d5743.
- **Cost:** run total_cost_usd ≈ $55.10, attempts=1, 101 turns.
- **No composite launch** this tick — 2 Desktop-Commander shell deaths occurred mid-tick (during foreground `git fetch`); switched to detached-background network commands. Conservative: recorded and left next eligible slice for the next tick.
- **NEXT:** launch first unfinished non-`blocked:` M17.5 slice per PLAN §9 (17.5d/e server-module → 17.5f evals/e2e → 17.5g docs); serial N=1 unless a clean fan-out partner; reconfirm a0d5743 master CI green first. adr_next_free=125.
- **MODEL note (still open for Drew):** rooted runs on **fable** per Drew's 2026-07-15 durable directive + mr-launch.sh default; SKILL.md task text says "Opus 4.8" — conflict persists, followed the durable directive, flagged.

## 2026-07-18T12:19:47Z — mr-sup-cowork-20260718T121857Z-3497796-26522 — LAUNCHING m17.5d (IN-PROGRESS)
- **Action:** launching rooted run for slice **m17.5d** (leaderboard profile.name drift; EARS 17.5d-1/2) — serial N=1, no clean fan-out partner (17.5e schema-serial, 17.5f trips module_bindings/evals structural set, 17.5g docs+rules.rs).
- **Pre-launch gates:** fast-path standdown = no live run; rate-limit gate clear (resets_at null); active-session probe clean (gate-top + pre-launch re-probe); both repos fetched; master **GREEN @ a0d5743** (chore #208 CI now completed/success, nightly green @90090fc); no open PRs / inflight / parked / awaiting_merge.
- **Touches (declared):** server-module/src/ranking.rs (+ranking_tests.rs). **ADR reserved:** 0125 (README next-free confirmed 0125). **Model:** fable (mr-launch.sh default per Drew 2026-07-15 durable directive).
- **MODEL CONFLICT (open for Drew):** SKILL.md task text says "Opus 4.8"; durable directive + launch tooling default = fable-5. Followed the durable directive (consistent with prior ticks); flagged.
- **State:** brief /tmp/mr_pass_m17.5d.md written; per-run lock + chain mutex held. Detached launch next; session_leader to be asserted.
- **[2026-07-18T12:26:25Z] ASSERTED:** launch healthy — leader pid **3501040** alive (Ss), claude child 3501046 (`--model fable`), codebase-memory-mcp 3501090 loaded (rooted). Model **claude-fable-5** confirmed in log init event (not haiku). Detachment confirmed (run session 3501040 ≠ supervisor shell session). Supervisor DC shell died mid-assert; detached run survived — reconnected from a fresh shell to finish assert+record. Per-run lock + ledger(outcome=launched) + mr-state(inflight=[m17.5d], adr_next_free=126) written. Chain mutex released. NEXT TICK: fast-path standdown while 3501040 live & no .done; merge-verify when PR opens.

## 2026-07-18T~13:3xZ — m17.5d TERMINAL: PR #209 OPEN, local full `just ci` green — supervisor to merge

- Branch `feat/m17.5d-profile-name-mirror` @ bb04273 (pushed). PR https://github.com/mdrewt/monster-realm/pull/209. Local full `just ci` EXIT=0 explicit-echo (1,308 Rust + 1,001 client tests; 63/63 evals; clippy/fmt/semgrep/wasm clean); remote ci+e2e running (run 29646058562). Verifier verdict APPROVE; gating-test RED→GREEN integrity audit CLEAN (assertions 36→39 monotone, post-lock edits doc-only + one additive test).
- Delivered EARS 17.5d-1/-2 (ADR-0125, amends ADR-0119): get_or_init_profile Some arm composes `refresh_profile_name(existing, live_player_name(ctx, identity))` — in-memory only, no eager write; None→keep last-known name (disconnect race, `.map` chain load-bearing); persistence rides apply_pvp_rating's untouched `..winner`/`..loser` spreads (both roles refresh; rename staleness ≤1 rated game). set_profile_name NOT added (D-17.5-C stays parked). All eight pre-existing RL-5/RL-7/RT-M17-01 byte-pins survive.
- Gates: 5 executed pure-core + 7 source-scan tests in ranking_tests.rs with NEW squash_ws scan machinery (strings→comments→whitespace-squash, concat!-split needles, 3-fixture BAD/GOOD/EVASION self-teeth). **String-literal stripping was red-team-mandated — comment-only stripping was live-demonstrated evadable.** Red-team live mutations 7/7 ALL-BIT (incl. real-file literal evasion). **Targeted cargo-mutants on ranking.rs: 0 missed (10 caught/2 unviable) — 3 pre-existing nightly-baseline survivors in apply_pvp_rating (delete-! guard, delete-rating-field ×2) killed by new survivor-pin needles.** mutate-server 299 cap untouched.
- Orchestration (model claude-fable-5): planner → plan-fan (reviewer NEEDS-REVISION W-5 + red-team F1–F8 → dispositioned, 2 refuted) → tester RED (verified compile-fail + needle-absence) → test-fan red-team (HIGH string-literal gate-hole → closed by tester) → implementer GREEN (production only; same_item_push allow workaround properly re-fixed via tester machinery refactor, allow removed) → impl-fan (reviewer SOUND + red-team ALL-BIT + reducer-security review-lens CLEAN 6/6) → doc-keeper (worktree-only — main-checkout trap NOT repeated; but misplaced memory card into project repo → removed, and mis-measured ADR Decision line 249>240 → adr-digest gate bit, trimmed) → verifier APPROVE. desync-guard skipped (no netcode surface, deliberate).
- **touches-delta (supervisor audit):** declared set exactly + standard doc outputs only (ADR-0125, DIGEST regen 90→91, docs/specs/m17.5d-plan.md, ARCHITECTURE.md one para). CHANGELOG/adr-README/evals/run.mjs untouched. No hidden dependencies; no game-core edits (unlike b/c slices).
- Residuals (recorded in PR, not fixed): eval C1b + rl2 never-deleted scan exclude ranking.rs internals (this slice's in-file T2 scans now cover the new code); extract_pvp_fn_body pub(crate) offset quirk (latent-harmless); doc_lazy_continuation trap recurred (fixed inline).
- Memory card (auto-memory) + MEMORY.md line written. Spec §17.5d delivery note added. Worktree `.claude/worktrees/m17.5d` removable after merge; codebase-memory graph reindex after merge (main checkout unchanged, verified clean). Scratch tree /tmp/m17.5d-mutcheck left behind (rm -rf guard) — safe to reap.
- NEXT: supervisor polls remote CI on #209 → squash-merge → 0125 ADR-index chore (next-free → 0126). Then M17.5 remaining per spec §5: 17.5e (challenge TTL, schema-additive/serial), 17.5f (PvP runtime coverage, client/evals), 17.5g (docs-only, runnable anytime).

---

## 2026-07-18T14:27:51Z — MERGED m17.5d (PR #209) + ADR-index chore (PR #210) — run mr-sup-cowork-20260718T141423Z-3607559-6860

Tick opened with m17.5d run finished (`.done` EXIT=0 ATTEMPTS=1, leader pid 3501040 dead) and PR #209 open/CLEAN/MERGEABLE — chain not live, so proceeded to merge-verify.

**Pre-merge audits (all CLEAN):**
- ci + e2e green; PR CLEAN/MERGEABLE.
- Orchestration: orchestrator on `claude-fable-5` (per Drew's durable 2026-07-15 fable directive — NOT haiku). Roles present: tester 335, reviewer 146, verifier 99, red-team 312, review-lens 130, planner 42, doc-keeper 110.
- Gating-test integrity (`server-module/src/ranking_tests.rs`): 0 removed/skipped/ignored `#[test]`, +12 new `#[test]`, 0 removed asserts, 0 `#[ignore]`; RED→GREEN lifecycle + proof-of-teeth comments intact.
- Touches: code/test diff == declared `touches` (`ranking.rs` + `ranking_tests.rs`). **FLAG:** PR also carried `docs/specs/m17.5d-plan.md` (outside declared touches AND outside doc-agg set) and edited `docs/adr/DIGEST.md` on the slice branch — benign here (N=1, no in-flight sibling, non-code), but the brief's `touches:` should have listed the plan doc.

**Action:** squash-merged #209 → master c10772e (master CI run 29647674523 = success). Removed worktree `.claude/worktrees/m17.5d` + local branch; main ff to c10772e. Supervisor-owned ADR-index reconciled via doc-only chore PR #210 (bump next-free 0125→0126, add 0125 row) → merged 7a6cd17. `gh pr merge --auto` was **rejected while the PR was already CLEAN**, then **accepted once it went UNSTABLE** (checks pending) — enable auto-merge only after checks start.

**Master now 7a6cd17, CI GREEN.** No composite launch this tick — the DC shell died twice mid-tick again; per rule 1 ("any doubt → next tick launches") I finished the merge cleanly and deferred.

**NEXT TICK:** no live run, nothing awaiting merge. Next eligible = **m17.5e** (adds a schedule table = structural/serial, N=1 — no clean fan-out partner: 17.5f trips `client/src/module_bindings` + `evals` structural set, 17.5g is docs+rules.rs doc-comments). Launch m17.5e on FABLE (assert claude-fable-5 post-launch), reserve ADR-0126 (next-free→0127). Model conflict (SKILL "Opus 4.8" vs durable fable directive) still open for Drew.

## 2026-07-18T16:24:01Z — mr-sup-cowork-20260718T162401Z-3632288-10177 — LAUNCHING m17.5e
- Gates clean: master GREEN @ 7a6cd17 (live CI success), no open PRs, nothing awaiting merge, no active human session (re-probed).
- Action: launching rooted run for slice **m17.5e** (battle_challenge TTL reaper + anti-spam cooldown; EARS 17.5e-1/-2/-3) — SERIAL N=1 (schema-additive schedule table = structural set; no clean fan-out partner).
- ADR-0126 reserved (adr_next_free 126->127). Model: FABLE (mr-launch.sh default; continuing durable Drew 2026-07-15 directive). MODEL CONFLICT vs SKILL 'Opus 4.8' still OPEN for Drew.
- LAUNCHED status=LAUNCHED session_leader=3632336 claude_pid=? model=claude-fable-5 at 2026-07-18T16:24:04Z.
- NEXT TICK: fast-path standdown while leader 3632336 alive + no .done; when .done appears -> verify PR/CI -> squash-merge -> ADR-0126 index chore -> then m17.5f (client/e2e + evals + rowConvert, structural) or m17.5g (docs-only).

## 2026-07-18T~19:2xZ — m17.5e TERMINAL: PR #211 OPEN, local full `just ci` green — supervisor to merge

- Branch `feat/m17.5e-challenge-ttl` @ 5c53112 (pushed). PR https://github.com/mdrewt/monster-realm/pull/211. Local full `just ci` EXIT=0 explicit-echo (1,321 Rust nextest + 1,001 client tests; 64/64 evals incl. new pvp-challenge-reaper 7-criteria; clippy/fmt/biome/semgrep/wasm clean); remote CI running (run 29655509621). Verifier: gating-test RED→GREEN integrity audit PASS (all post-lock edits enumerated strengthenings; live teeth spot-check bit); initial REJECT on ONE mechanical blocker (knowledge drift from doc-keeper's comment-only pvp.rs banner — regen-after-source-edit trap AGAIN) → `just knowledge` fix 5c53112 → full ci green.
- Delivered EARS 17.5e-1/-3 (ADR-0126): battle_challenge_reaper_schedule PRIVATE one-shot schedule table + schedule/disarm helpers + scheduler-only battle_challenge_reaper in pvp.rs (reducer placed BEFORE pvp_deadline_reaper — EA-PVP-02 suffix-scan vacuity avoided + gate re-bounded brace-wise in-slice); CHALLENGE_TTL_MS=120_000 (**D-17.5-1: 2 min**) + is_challenge_stale in game-core combat::pvp (existing module — no new root module); arm post-insert in challenge_pvp (guard 9); disarm at ALL FOUR deletion sites. **17.5e-2 cooldown DEFERRED to M19 decline-and-mute (ADR-0126 D3, spec Delivered line updated)**.
- Gates: 7 game-core boundary tests + EA-CHR-01..06 (arg-identity pins EVERYWHERE after red-team live probes: arm args, stale-recheck arg ORDER + block-open shape, delete arg, per-site disarm args + disarm-body survivor-pins) + eval 7 criteria × bad+good teeth. Red-team live mutations 9/9 BIT post-hardening. Targeted cargo-mutants: game-core 0 missed; server challenge-region 0 NEW-code missed (7 pre-existing challenge_pvp guard mutants on byte-unchanged lines, baseline class); mutate-server 299 cap untouched.
- Orchestration (model claude-fable-5): planner → plan-fan (reviewer B1 BLOCKER suffix-scan/placement + W1-W4; red-team F1-F12 incl. 2 HIGH gate-holes) → tester RED (verified empirically) → test-fan red-team (HIGH transposed-args + MEDIUM empty-block → tester closed) → implementer GREEN (production only; escalated the orphan-/* glob comment blocker instead of touching locked tests — tester fixed) → impl-fan (reviewer SOUND docs-only + red-team 2 more arg-pin holes → tester closed + reducer-security lens CLEAN 7/7) → doc-keeper (WORKTREE-ONLY — main-checkout trap NOT repeated; ADR-0126 237-char Decision, ground-truth verified) → verifier REJECT(mechanical)→fixed→green. desync-guard skipped (no netcode surface, deliberate).
- **touches-delta (supervisor audit; full list in PR body):** declared set + game-core/src/combat/{pvp.rs,mod.rs}+lib.rs (TTL home + mechanical re-exports) + client/src/module_bindings/types.ts (regen, schema-additive as briefed) + evals/baselines/table-schemas.json (append-only) + docs/knowledge regen + standard doc outputs (ADR-0126, DIGEST 92 ADRs, ARCHITECTURE one line, docs/specs/m17.5e-plan.md). CHANGELOG/adr-README/run.mjs untouched. Gating-test edits post-lock: ALL tester-authored strengthenings, verifier-enumerated (PR body section).
- Ops notes: cargo-mutants `-f` needs WORKSPACE-relative paths (crate-relative silently matches 0). Comment-only .rs edits trip the knowledge drift gate — `just knowledge` in the same commit. Orphan-`/*` from a literal glob in a test comment blinded npc/raising evals (~60KB eaten) — globs as prose in .rs comments. Red-team left 3 /tmp scratch dirs (mr175e, mr175e-target, okf-probe) + /tmp/m175e-teeth-UOtv + mutants.out dirs in the worktree root (gitignored) — reap after merge.
- Residuals (ADR-0126 a-g): active-attacker re-challenge → M19; early-fire Pending-leak accepted (no re-arm); runtime reaper-firing coverage → 17.5f; republish survival of pending ScheduleAt::Time rows not doc-confirmed (durable-row reasoning; applies to trade reaper); trade-side deadline-expression scan-pin gap; silent-expiry UX for playtest; knowledge date-rollover will recur if squash lands on a later UTC day.
- Memory card memory/projects/monster-realm-m17.5e.md + MEMORY.md line written (auto-memory). Spec §17.5e Delivered line added (harness repo, orchestrator-written per R7). Worktree `.claude/worktrees/m17.5e` removable after merge; codebase-memory graph reindex + manage_adr memo refresh after merge (main checkout unchanged).
- NEXT: supervisor polls remote CI on #211 → squash-merge → ADR-0126 index chore (next-free → 0127). Then M17.5 remaining per spec §5: 17.5f (PvP runtime coverage, client/evals, structural), 17.5g (docs-only, runnable anytime).

---

## 2026-07-18T20:28Z — SUPERVISOR (cowork tick, run mr-sup-cowork-20260718T200825Z-3772129-21523)

**Action: MERGED m17.5e.** Prior tick (…162401Z) launched m17.5e on FABLE and died before reconciling; this tick found session-leader 3632336 dead, `.done` = `EXIT=0 ATTEMPTS=1`, and PR #211 OPEN+CLEAN awaiting merge.

- Squash-merged **PR #211** (`feat/m17.5e-challenge-ttl`) → master **0a2d039** @ 20:13:27Z. battle_challenge TTL reaper: 2-min expiry, private one-shot schedule table in pvp.rs, `CHALLENGE_TTL_MS=120000` + `is_challenge_stale` in game-core combat::pvp, disarm at all 4 challenge-deletion sites; decline-cooldown deferred to M19. ADR-0126.
- ADR index reconciled via doc-only chore **PR #212** → master **1e0c270** @ 20:22:13Z (`--auto` rejected: branch protection auto-merge not configured → merged manually on green). Next-free `0126`→`0127`.
- **Audits CLEAN.** Orchestration: fable-5, 11 Agent invocations, roles tester=428/reviewer=311/verifier=152/red-team=504/review-lens=67/doc-keeper=83/planner=39. Gating-test: +14 `#[test]`, 0 removed, no skip/xit/.only/#[ignore], no removed assertions. Cost $94.43, attempts 1.
- **FLAG (benign, N=1):** actual diff 89 files/+2070 is broader than declared `touches` — codegen `client/src/module_bindings/types.ts` + full `docs/knowledge/**` regen + out-of-touches `docs/specs/m17.5e-plan.md`. No in-flight siblings → no fan-out collision; consistent with m17.5d precedent.
- Master **GREEN** (0a2d039 slice CI success; 1e0c270 doc-chore CI success). Worktree `.claude/worktrees/m17.5e` + branches `feat/m17.5e-challenge-ttl` and local `chore/m17.5e-adr-index` removed; strays (`.claire/`, `docs/memory-cards/`) stash-restored. Older stashes @{1}(2026-07-04)/@{2}(M12.5c) left untouched.
- **Resilience note:** 2 DC shell deaths mid-tick (both during `sleep` polls); reattached each time, mutex held throughout, merge finished fully. Avoid long in-shell sleeps.

**No composite launch this tick (deferred, deliberate).** Next eligible = **m17.5f** (structural/serial: trips `client/src/module_bindings/**` + evals; no clean fan-out partner), then m17.5g. Deferred launch pending: (a) **OPEN MODEL-DIRECTIVE CONFLICT for Drew** — this SKILL says "Opus 4.8" but durable directive (Drew 2026-07-15) says rooted runs on FABLE; last 4 slices ran fable-5. (b) shell instability this tick. Next tick: no live run, nothing awaiting merge → launch m17.5f (reserve ADR-0127→next-free 0128 at launch; assert model post-launch per whichever directive Drew confirms). Pre-gate roadmap unchanged (playtest-replan).


<!-- archived 2026-07-19T16:19:57Z by supervisor mr-sup-cowork-20260719T160610Z-4304-6378: entries through pt-a1 merge / pre-pt-a2-merge -->
---

## [2026-07-18T22:18:19Z] IN-PROGRESS — LAUNCH m17.5f (mr-sup-cowork-20260718T220411Z-3790677-28448)
Launching m17.5f (PvP runtime coverage + SDK-boundary enum safety; EARS 17.5f-1/-2/-3). Fresh serial slice off origin/master 1e0c270 (master GREEN). ADR-0127 reserved. touches: client/e2e/**, client/src/main.ts, client/src/net/rowConvert.ts, evals/** (module_bindings/**, evals/run.mjs structural=serial). Model: fable (script default, per Drew 2026-07-15 durable directive; SKILL 'Opus 4.8' conflict still OPEN for Drew). Single launch — no fan-out (17.5f touches structural set; 17.5g docs-only is the next candidate).

## 2026-07-18T~23:4xZ — m17.5f TERMINAL: PR #213 OPEN, local full `just ci` green — supervisor to merge

- Branch `feat/m17.5f-pvp-runtime-coverage` @ 5479e9f (pushed). PR https://github.com/mdrewt/monster-realm/pull/213. Local full `just ci` EXIT=0 explicit-echo (1,018 client tests / 38 files; 65/65 evals incl. new sdk-enum-exhaustiveness 8-teeth; clippy/fmt/biome/tsc/semgrep/wasm clean) + local `just e2e` EXIT=0 (37 passed, 1 pre-existing recruit-R4 fixme skip). Remote ci+e2e running (run 29665759533). Verifier APPROVE; gating-test RED→GREEN integrity CLEAN (locked tests byte-identical since lock 2ab4ad3; impl commits touched zero test files).
- Delivered EARS 17.5f-1/-2/-3 + D-17.5-E (ADR-0127, amends ADR-0115 — reverses D1): DEV-gated `__game`/`__mrTrade`/`__mrPvp` (one `import.meta.env.DEV` block; minifier dead-branch, `--minify false` caveat); `__mrPvp` 8 methods incl. role-agnostic battleById (side B invisible to store.ongoingBattle — cross-page battleId passing); pvp-full.spec.ts strict turn-exchange (`turnNumber===prev+1`, NO terminal escape — deadline forfeit would fake coverage) + server-sql turn assert + disconnect-forfeit zero-sum; trade-interlock.spec.ts paired positive-control (propose-succeeds→cancel→in-battle propose→pair-scoped sql ABSENCE; filename sorts after golden.spec deliberately); sdk-enum-exhaustiveness eval (C1 min-8/C2/C3/C4 narrowTag-ADJACENCY pin) + HANDLED_ENUM_VARIANTS 8-enum registry + fail-soft narrowTag at rowConvert.ts:525 only (registry↔tsc widening ratchet; type-replacement REJECTED in ADR).
- Gates: red-team live mutations 4/4 BIT; adversarial tester 7× e2e repeat-run SOUND (191ms flow-time verified honest — sql ≈20ms local, setup in beforeAll); privilege-escalation refuted (ctx.sender all 5 reducers). Orchestration (fable-5): planner → plan-fan (2 BLOCKER + 10 findings) → tester RED → test-fan (1 BLOCKER vacuous-sql + 4 HIGH closed) → implementer GREEN (production only) → impl-fan (reviewer SOUND + red-team + adversarial tester) → doc-keeper WORKTREE-ONLY (trap NOT repeated) → verifier APPROVE. desync-guard + reducer-security skipped DELIBERATE (no netcode/reducer surface).
- touches-delta (in PR body): declared set + rowConvert.test.ts (sibling) + ADR-0127 + DIGEST regen + ARCHITECTURE 2 lines + docs/specs/m17.5f-plan.md. CHANGELOG/adr-README/run.mjs/module_bindings untouched.
- Ops notes: NEW trap signature — concurrent `just eval` mid-e2e clobbers wasm pkg AND vite full-reloads pages MID-TEST (kills in-flight specs, not just startup); never overlap eval+e2e in one worktree. Local spacetimedb had to be started (was down; `spacetime start` detached, still running). Residuals in ADR-0127: console.warn unbounded per unknown-tag row; deadline-elapse forfeit e2e deferred; StatusEffect eval-excluded.
- Memory card memory/projects/monster-realm-m17.5f.md + MEMORY.md line written. Spec §17.5f Delivered note = PR #213. ADR next-free = 0128. Worktree `.claude/worktrees/m17.5f` removable after merge; codebase-memory graph reindex after merge (main checkout unchanged, verified clean).
- NEXT: supervisor polls remote CI on #213 → squash-merge → ADR-0127 index chore (next-free → 0128). Then M17.5 remaining per spec §5: 17.5g (docs-only ledger reconciliation, runnable anytime) closes M17.5.

## 2026-07-19T00:30Z — SUPERVISOR (cowork tick, run mr-sup-cowork-20260719T000857Z-3877309-29849)

**Action: MERGED m17.5f.** Fast-path found no live rooted run (leader 3793582 dead) + `.done`=`EXIT=0 ATTEMPTS=1` + PR #213 OPEN → proceeded to merge-verify (not a live-chain standdown).

- Squash-merged **PR #213** (`feat/m17.5f-pvp-runtime-coverage` @ 5479e9f) → master **80fcacb** @ 00:20:19Z. PvP runtime e2e coverage + DEV-gated `__game`/`__mrTrade`/`__mrPvp` hooks (ADR-0127 amends/reverses ADR-0115 ungated decision) + `sdk-enum-exhaustiveness` eval + `HANDLED_ENUM_VARIANTS` registry + fail-soft `narrowTag` at rowConvert cast.
- ADR index reconciled via doc-only chore **PR #214** → master **78ec655** @ ~00:26Z (`--auto` ACCEPTED this time — merged immediately; branch auto-deleted). Next-free `0127`→`0128`.
- **Audits CLEAN.** Orchestration: model `claude-fable-5` (durable Drew 2026-07-15 fable directive — NOT silent-haiku), 13 Agent invocations, roles tester/reviewer/verifier/red-team/planner/doc-keeper all present. Gating-test integrity: additive-only — 2 NEW e2e specs (pvp-full 6-block/22-expect; trade-interlock 5-block/8-expect) + +369 rowConvert narrowTag/registry tests + new eval; **0** removed/`skip`/`.only`/`xit`/`#[ignore]`; the lone `rowConvert.test.ts` "deletion" = adding `vi` to a vitest import. Cost **$75.58**, attempts 1, remote_red_fix_cycles 0.
- **Touches (supervisor audit):** actual diff (10 files) ⊆ declared set (`client/e2e/**`, `main.ts`, `rowConvert.ts`, `evals/**`) + doc-agg (`ARCHITECTURE.md`,`docs/adr/**`, supervisor-owned) + benign `docs/specs/m17.5f-plan.md` (plan doc, precedent-consistent flag) + `rowConvert.test.ts` sibling. **No in-flight sibling; NO structural-set file touched** (`evals/run.mjs` & `module_bindings/**` untouched despite serial declaration).
- **Master CI:** slice head **80fcacb CI run 29666783038 = SUCCESS** (GREEN). Chore head 78ec655 CI in_progress at record time (doc-only, expected green). Main checkout restored to master@78ec655; worktree `.claude/worktrees/m17.5f` + branches (local+remote `feat/m17.5f-*`, `chore/m17.5f-adr-index`) removed. Untracked strays `.claire/`, `docs/memory-cards/` left untouched.
- **Resilience note:** DC shell died 3× early this tick (all during network `git fetch`/poll ops). Switched to detached-bg + artifact-marker polling with NO in-shell `sleep` — stable thereafter. Mutex acquired ONLY at merge time (pid 3877309); merge→cleanup→chore→record all completed under it.
- **Self-inflicted note:** a stray `git checkout 5479e9f --` (empty pathspec, during the gating audit) detached the main checkout to the slice tip; caught it, restored to master via checkout + `--ff-only`. No harm (working tree had only known untracked strays).

**NO composite launch this tick (DEFERRED, deliberate).** Next eligible = **m17.5g** (docs-only ledger reconciliation, closes M17.5). Defer reasons: (a) **OPEN MODEL-DIRECTIVE CONFLICT for Drew** — this SKILL says "Opus 4.8" but the durable directive (Drew 2026-07-15) + last 4 slices = FABLE; (b) shell instability early this tick. Per rule-1 "any doubt → next tick launches." Next tick: no live run, nothing awaiting merge → launch m17.5g (reserve nothing new — docs-only; assert model post-launch per whichever directive Drew confirms). Pre-gate roadmap unchanged (playtest-replan): after M17.5 closes, pre-gate block M-playtest-a→d then PLAYTEST-READY blocker (no M18+).

## [2026-07-19T02:23:48Z] IN-PROGRESS — LAUNCH m17.5g (mr-sup-cowork-20260719T022135Z-3895429-19882)
Launching **m17.5g** (docs-only ledger reconciliation, spec §17.5g-1..-4) — the FINAL M17.5 slice; its merge CLOSES M17.5. Fresh serial slice (N=1, no clean fan-out partner — the pre-gate M-playtest-a..d block is downstream of M17.5) off origin/master **78ec655** (master CONFIRMED GREEN, CI run 29666950074 success). **No ADR** (docs-only; adr_next_free stays 128). touches: docs+comments only — CHANGELOG.md (git-cliff regen), ARCHITECTURE.md module map, docs/adr/README.md (ADR-0107 row label→KeyU), specs/monster-realm-v2/PLAN.md §Status + M17 §5 merge-states (harness repo), game-core/src/trading/rules.rs test doc-comments (assertions untouched), 4× server-module RLS deferral comments. **Model: fable** (mr-launch.sh default per Drew 2026-07-15 durable directive; matches last 5 M17.5 slices). MODEL-DIRECTIVE resolution: the SKILL's "Opus 4.8" prose is stale (it even mis-claims the launch script pins --model opus, but the script Drew edited 2026-07-15 defaults to fable); operative model = fable per Drew's maintained config + durable directive + 5-slice precedent. Docs-only, so low-stakes regardless — Drew reviews the PR before merge.

## [2026-07-19T03:45:39Z] PARKED m17.5g (fable usage-limit) + MODEL SWITCH fable->opus — Drew interactive (mr-sup-cowork-20260719T022135Z-3895429-19882)
Drew reached his **fable usage limit** and directed the loop onto **Opus**. Actions:
- **mr-launch.sh: default model flipped `fable`->`opus`** (line 25 `MODEL="${2:-opus}"`; comment updated; `bash -n` clean; backup `mr-launch.sh.bak-20260719`). The 2026-07-15 fable directive is **superseded**; model-directive conflict is now closed = **OPUS**.
- **m17.5g PARKED (not failed):** the fable run I launched at 02:24Z hit the fable `seven_day_overage_included` rejection (`status:rejected`, resetsAt 1784851200 ~2026-07-23), EXIT=1 ATTEMPTS=1 (wrapper correctly did NOT auto-resume a usage-limit), cost ~$18.33. It got FAR first: **6 wip commits pushed to `origin/docs/m17.5g-ledger-reconciliation` @ fc07004**, clean worktree `.claude/worktrees/m17.5g` (PRESERVED). Deliverables all committed: g1 CHANGELOG git-cliff regen · g2 ARCHITECTURE module-map (use_battle_item, is_ranked_pvp) + coverage-exclusion + PR#187 fix · g3 RLS deferral comments reworded · g4 ADR-0107 README label->KeyU + rules.rs test doc-comments past-tense + knowledge-bundle regen. Orchestration: planner+reviewer+verifier (no tester = correct for docs-only). **No PR opened** (died before it); no progress memo (usage-limit bypassed cooperative park). 
- **Stale per-run lock removed; mr-state reconciled** (inflight=[], m17.5g under parked[], model_directive=OPUS). Worktree/branch KEPT.
- **RESUME (on opus):** `git worktree list` -> `.claude/worktrees/m17.5g` on branch `docs/m17.5g-ledger-reconciliation` (already pushed @ fc07004). Remaining = run full `just ci` green in the worktree, then open the slice PR on mdrewt/monster-realm; supervisor merges -> **CLOSES M17.5** -> then pre-gate M-playtest-a. master GREEN@78ec655.

## [2026-07-19T04:44:12Z] SUPERVISOR (cowork tick, run mr-sup-cowork-20260719T042821Z-3938174-17557) — MERGED m17.5g -> M17.5 CLOSED

**Action: MERGED m17.5g (PR #215).** Fast-path found no live rooted run + no chain-lock; `.done`=`EXIT=1 ATTEMPTS=1` (fable usage-limit park). Ground-truth probe CORRECTED the park breadcrumb: mr-state said `pr:null`/pre-PR, but **PR #215 was live OPEN + mergeStateStatus CLEAN** on `docs/m17.5g-ledger-reconciliation` @ fc07004 (the fable run opened it before the usage-limit stop, or Drew did) — so merged, not relaunched.
- Squash-merged **PR #215** -> master **b31eeab** @ 04:28:56Z. Diff = doc-agg (CHANGELOG git-cliff regen thru #214, ARCHITECTURE module-map, docs/adr/README ADR-0107 KeyU label) + docs/knowledge/** bundle regen + **3 comment-only .rs** (game-core/src/trading/rules.rs test doc-comments; server-module/src/inventory.rs + schema.rs RLS-deferral wording "M16 residual"->"deferred until per-row RLS lands").
- **Audits CLEAN.** Verified all 3 .rs diffs are 100% comment/doc-comment-only — zero behavior change; rules.rs `#[test]` attrs + assertions byte-untouched. Gating-test integrity: **CLEAN** (no deleted/skip/xit/.only/#[ignore]'d test, no removed assertion). Orchestration: docs-only -> exempt (park line: planner+reviewer+verifier). No new ADR file (README = label fix) -> **no ADR-index chore-PR needed**; adr_next_free stays 128.
- **Master CI:** run 29673379734 @ b31eeab = **SUCCESS (GREEN)** (PR's own ci+e2e already passed on fc07004). Local master ff-only advanced to b31eeab. Worktree `.claude/worktrees/m17.5g` + local & remote branch `docs/m17.5g-ledger-reconciliation` removed. Untracked strays (.claire/, docs/memory-cards/) left untouched. Stale local chore branches (chore/m15c/m16c/m17.5c-adr-index) left (pre-existing cruft, harmless).
- **m17.5g merge CLOSES M17.5** (all slices a-g merged).
- Resilience: DC shell churned once early (during git fetch) — restarted, mutex acquired only at merge time (pid 3938174).
- **COMPOSITE LAUNCH follows this entry** (Drew present, explicitly requested launching M-playtest-a now). See next IN-PROGRESS breadcrumb.

## [2026-07-19T04:47:13Z] IN-PROGRESS — LAUNCH pt-a1 (composite, run mr-sup-cowork-20260719T042821Z-3938174-17557)
Launching **pt-a1** — M-playtest-a's first sub-slice (client-side local-playtest build hygiene), the first pre-gate playtest slice. **COMPOSITE launch** immediately after the m17.5g merge (Drew present ~04:2xZ, explicitly requested kicking off M-playtest-a this tick rather than waiting for the next scheduled tick). Fresh serial slice (N=1; pre-gate block builds a->b->c->d serial, no fan-out partner) off origin/master **b31eeab** (master GREEN). **Model: OPUS** (Drew 2026-07-19 fable->opus directive; mr-launch.sh default opus). **ADR-0128 reserved.**
- M-playtest-a is a design SKETCH (specs/monster-realm-v2/M-playtest-a-deployment.spec.md, harness repo). Brief instructs the run to do the **build-time slicing pass** FIRST (reconcile the sketch against master b31eeab, finalize EARS + touches, write a per-slice plan doc under docs/specs/ per the mNN-plan.md convention), THEN ship pt-a1 ONLY: (1) env-driven client config (VITE_STDB_URI / VITE_STDB_DB; playtest DB name monster-realm-playtest, never the dev-default); (2) git-SHA + build-time version stamp visible in-client and exposed for the M-playtest-b F9 bug-bundle; (3) honest production-mode client build/serve path (vite build + preview/static, minified, DEV hooks absent). RECONCILE (do NOT re-implement) the __game/__mrTrade/__mrPvp DEV-gating already landed in ADR-0127/m17.5f — verify it is absent from the minified prod build (ADR-0127 noted a --minify false caveat).
- **Out of scope, park as the NEXT slice:** pt-a2 (just playtest-up/down + release-publish with dev_reducers-absent proof against the PUBLISHED module + wipe/republish flow + docs/playtest-ops.md) and ALL hosted deploy (re-booked M-playtest-a2). touches PROVISIONAL (sketch slice — the slicing pass finalizes them); structural set excluded (client/src/module_bindings/**, Cargo.lock, package-lock.json, evals/run.mjs, server-module/**, game-core/**).
- Precondition satisfied: spec requires landing after M17.5's HIGH slices so the playtest build cannot carry the side-B exploit — M17.5 is now fully CLOSED (a-g merged).
- Per-run lock memory/projects/.harness-runner.pt-a1.lock written (session_leader/claude_pid filled post-launch). Detachment + Opus-model asserted after launch.

## [2026-07-19T~01:5xZ] pt-a1 TERMINAL — PR #216 OPEN, local full `just ci` green — supervisor to merge

- Branch `feat/pt-a1-playtest-build-hygiene` @ **807a629** (pushed). PR https://github.com/mdrewt/monster-realm/pull/216 (base master). **Local full `just ci` EXIT=0** twice (FINAL_CI_PASS): cargo fmt/clippy/check/nextest/doctest on the workspace, 65/65 evals (incl. dom-shell-coverage-exclusion + gate-hardening-config GREEN — the vite.config `define` did NOT trip them), biome 180 files, tsc, 1065 client vitest tests, security, wasm. Remote ci+e2e will run on push. **Verifier PASS on all 5 gates**; RED→GREEN integrity CLEAN (impl commit ece79ba touched ZERO test files; test files byte-identical since the RED checkpoint a2f3001; no skip/only/xit/#[ignore]; F-2a/F-5e teeth bite).
- **Delivered (ADR-0128):** (D1) pure `resolveConnectionConfig(env,isDev)` in `client/src/net/connectionConfig.ts` — prod FAILS LOUD (throw) when trimmed db is unset/empty/whitespace or case-insensitively `monster-realm`; dev keeps `ws://127.0.0.1:3000`/`monster-realm`; **guards DB not URI**; case-preserving trimmed db; wired at MODULE scope in main.ts (replaces inline URI/DB consts). (D2) `BUILD_INFO`+`formatBuildStamp` in `client/src/net/buildInfo.ts` + `vite.config.ts` `define` (git short-SHA/time, env-overridable, 'unknown' fallback); rendered in `#build-stamp` + UNGATED `window.__mrBuild` for the M-playtest-b F9 bundle. (D3) DEV-hooks-absent reconciliation of ADR-0127 (empirically verified minified `vite build` = 0 window bindings; refined the `--minify false` caveat — Rollup DCE drops bindings even unminified) + `main.wiring.test.ts` F-5 source-scan regression guard.
- **Orchestration (Opus):** planner → plan-fan (reviewer + red-team, F-1..F-7 folded into EARS/tests) → tester RED (connectionConfig/buildInfo + main.wiring; tester self-caught the comment-mention needle bug, corrected to `).__game =`) → implementer GREEN → impl-fan (reviewer: B-1/H-2 REFUTED false-positives, H-1 mitigated by toEqual shape-pin, M-1 declined; red-team: all F-1..F-7 CLOSED, LOW-1 ZWSP + LOW-2 single-DEV-block documented as ADR residuals) → verifier PASS. Domain auditors (reducer-security/desync-guard) DELIBERATELY skipped (no reducer/netcode surface) — matches m17.5f precedent. doc-keeper: memory card + MEMORY.md + spec delivery note (in flight).
- **touches-delta (in PR body):** all within declared set (`client/src/net/**`, `main.ts`, `vite.config.ts`, `index.html`) + standard companions (ADR-0128, DIGEST, ARCHITECTURE, docs/specs/pt-a1-plan.md). Notable: `buildInfo.ts` in `net/` (declared set allowed net/ or ui/); `main.wiring.test.ts` = source-scan test of touched main.ts (sibling-ish). **NO structural-set file touched** (no module_bindings/Cargo.lock/package-lock.json/package.json/evals/run.mjs/server-module/game-core). CHANGELOG not hand-edited.
- **Ops notes:** (1) `vite build` static analysis needs a FRESH wasm pkg — a stale `client-wasm/pkg/client_wasm.js` (clobbered mid-session by eval/test runs) breaks the build with "step_ms not exported"; `just wasm` fixes it; `just ci`'s wasm step avoids it (vitest tolerates the stale pkg, `vite build` does not). (2) do NOT write literal zero-width chars into docs (LOW-1 discussion) — use `U+200B` escape notation. (3) foreground sleep is blocked — use Monitor/run_in_background.
- **NEXT (supervisor):** poll remote CI on #216 → squash-merge to master → **codebase-memory graph reindex AFTER merge** (main checkout still master@b31eeab; pt-a1 code lands on master only post-merge — graph refresh deferred). Then **pt-a2** (M-playtest-a second sub-slice): `just playtest-up/down` + release-publish with mechanical `dev_reducers`-absent proof vs the PUBLISHED module + automated build-output DEV-hooks-absent grep guard + wipe/republish + `docs/playtest-ops.md`; structural/justfile-touching → SERIAL. Hosted deploy = M-playtest-a2. **ADR next-free = 0129.** Worktree `.claude/worktrees/pt-a1` removable after merge; branch `feat/pt-a1-playtest-build-hygiene` deletable after merge.

---
## 2026-07-19T06:15:45Z · supervisor tick · mr-sup-cowork-20260719T060552Z-4005670-20954 · pt-a1 REMOTE-RED FIX launched (IN-PROGRESS)
pt-a1 run finished EXIT=0 and opened PR #216, but remote CI `ci` job failed at the **SAST (Semgrep)** step: 4 blocking `javascript.lang.security.detect-insecure-websocket` findings on the NEW file `client/src/net/connectionConfig.test.ts` (intentional localhost `ws://` dev-config test fixtures). `e2e` job + all other `ci` steps GREEN. **Root cause of miss:** local `just ci`'s `security` recipe runs only `check-secrets.mjs` — it does NOT run Semgrep; the remote `ci.yml` "SAST (Semgrep)" step (`semgrep scan --config auto --error`) is remote-only, so no local run can pre-catch these (same class as the twice-biting `detect-non-literal-regexp`). Launched a bounded remote-red FIX run (`--model opus`, detached via mr-launch.sh) to annotate the intentional fixtures with `// nosemgrep` (teeth-preserving, comment-only), push to the existing branch (updates #216), and stop at terminal state for supervisor merge on remote green. Systemic follow-up flagged: wire Semgrep into local `just ci` (or pin ruleset). Master unchanged @ b31eeab (M17.5 CLOSED).

## 2026-07-19T~06:25Z · pt-a1 REMOTE-RED FIX TERMINAL — fix pushed to PR #216, remote CI re-running (supervisor to merge on green)
- **Fixed + pushed.** Commit `ee9d4bd` on `feat/pt-a1-playtest-build-hygiene` (807a629→ee9d4bd, `git push` EXIT=0). PR **#216** https://github.com/mdrewt/monster-realm/pull/216 head now `ee9d4bd`, state OPEN, mergeStateStatus UNSTABLE (CI running). New CI run **29676410704** in_progress on ee9d4bd (old failing run 29675421351 was on 807a629). **DID NOT merge / did not wait on remote — supervisor polls + merges.**
- **What changed (COMMENT-ONLY, 1 file, +6/-0):** `client/src/net/connectionConfig.test.ts` only. Added targeted `// nosemgrep` suppressions on the immediately-preceding line of each of the 4 flagged `ws://` dev-fixture finding lines (the `0.0.0.0` and `local` host cases in the two dev-mode tests) + one plain-English rationale comment per block. **Zero** assertion/expected-value/`it()`-name/fixture-string/behavior change; no test skipped/deleted/weakened. `touches-delta:` = `client/src/net/connectionConfig.test.ts` (declared sibling of the pt-a1 source file — in scope).
- **Reproduced + verified locally.** `semgrep scan --config auto --error --exclude '.claude'` (v1.168.0, network-reachable this session): BEFORE = 4 blocking `javascript.lang.security.detect-insecure-websocket`; AFTER = **0 blocking** per-file AND full-repo (exact remote command, EXIT=0). Full `just ci` = **EXIT=0** (cargo fmt/clippy/check/nextest/doctest, 65/65 evals, security, wasm, tsc, 1065 client vitest / 41 files). Independent verifier PASS ×4 checks (diff comment-only; `expect(`/`it(` counts identical =46; 23/23 connectionConfig tests bite; no `ws://` token in added comments).
- **TRAP (load-bearing, cost me 2 iterations) — scoped nosemgrep needs the DOUBLED rule id.** `// nosemgrep: javascript.lang.security.detect-insecure-websocket` SILENTLY FAILS to suppress (semgrep reports "No ignore information available"). The registry rule id is `javascript.lang.security.detect-insecure-websocket.detect-insecure-websocket` (last segment doubled) — the scoped comment MUST use the full doubled id, else it no-ops. Bare `// nosemgrep` works but is over-broad. BOTH same-line and preceding-line placement work once the id is right (empirically probed). ALSO: the rule matches the `ws://` token in COMMENT text too — a rationale comment containing `ws://` re-triggers the finding on itself; keep the literal scheme out of prose comments.
- **`127.0.0.1` is EXEMPT** — the rule's loopback allowlist means `ws://127.0.0.1:3000` (test line 34 DEV_URI + source `connectionConfig.ts:12`) is NOT flagged; only `0.0.0.0`/`local`/non-loopback hosts fire. So the source file needed NO change — fix confined to the test file.
- **SYSTEMIC FOLLOW-UP (future infra slice, NOT pt-a1 — out of touches):** local `just ci`'s `security` recipe = `node scripts/check-secrets.mjs .` ONLY; it does NOT run Semgrep, so no local gate can pre-catch Semgrep findings (this is now the 2nd such miss, cf. `detect-non-literal-regexp`). Add a `just semgrep` recipe (`semgrep scan --config auto --error --exclude '.claude'`) wired into `just ci`, and/or pin the ruleset for determinism. Touches the justfile/CI config (shared/structural) → its own slice.
- **Risks:** LOW. Suppressions are per-line + per-rule (not blanket) so a genuinely-insecure production `ws://` elsewhere still fires. Only residual risk = if a future Semgrep ruleset renames/re-doubles the rule id, the scoped suppression could silently lapse (bare `nosemgrep` fallback would avoid that but is broader). Remote CI (ci+e2e) will confirm; supervisor merges on green. **Master unchanged @ b31eeab.**

## 2026-07-19T08:23Z · supervisor tick · mr-sup-cowork-20260719T081201Z-4035601-7612 · MERGED pt-a1 (PR #216)

**Action: MERGED pt-a1.** Fast-path: recorded run pids (leader 4008016 / claude 4008022) both DEAD + `.done`=`EXIT=0 ATTEMPTS=1` + PR #216 OPEN → not a live-chain standdown → proceeded to merge-verify. Gate 1 clear (rate_limit_resets_at null). Active-session probe CLEAN (no resident IDE claude, zero harness/proj writes <6min). Chain mutex acquired (holder 4035601).

- Squash-merged **PR #216** (`feat/pt-a1-playtest-build-hygiene` @ **ee9d4bd**) → master **05d0781** @ 08:12:28Z. pt-a1 = M-playtest-a's 1st sub-slice (client local-playtest build hygiene, ADR-0128): (D1) pure `resolveConnectionConfig(env,isDev)` in `client/src/net/connectionConfig.ts` — prod FAILS LOUD on unset/empty/dev-default DB; dev keeps `ws://127.0.0.1:3000`/`monster-realm`. (D2) `BUILD_INFO`+`formatBuildStamp` (`buildInfo.ts` + vite `define`, git short-SHA/time) → `#build-stamp` + ungated `window.__mrBuild` for the M-playtest-b F9 bundle. (D3) DEV-hooks-absent reconciliation of ADR-0127 vs the minified prod build.
- Remote CI at merge: **ci PASS + e2e PASS** (run 29676410704 on ee9d4bd), mergeStateStatus CLEAN, MERGEABLE.
- **Audits CLEAN.** Touches: merged diff (12 files) ⊆ declared set (`client/src/net/**`, `main.ts`, `vite.config.ts`, `index.html`) + companions (ADR-0128, DIGEST, ARCHITECTURE, `docs/specs/pt-a1-plan.md`) + `main.wiring.test.ts` (source-scan sibling of main.ts). **NO structural-set file** (no module_bindings/Cargo.lock/package-lock.json/evals/run.mjs/server-module/game-core). Gating-test integrity: test changes = 3 NEW test files + comment-only nosemgrep on connectionConfig.test.ts; **0** removed assertions, **0** skip/only/xit/#[ignore]. Orchestration: current `/tmp/mr_pass_pt-a1.log` is the remote-red-FIX run (comment-only) — verifier-role present, model **claude-opus-4-8** ✓; the original pt-a1 BUILD run's full planner/tester/reviewer/red-team/verifier orchestration is documented in the prior handoff entries. remote_red_fix_cycles **1**.
- **Remote-red fix (already landed pre-tick, verified):** commit ee9d4bd added 4 `// nosemgrep` suppressions (+ rationale) on intentional `ws://` dev-fixtures in connectionConfig.test.ts — Semgrep SAST is a remote-only gate (local `just ci` security recipe = check-secrets.mjs only). **SYSTEMIC follow-up STILL OPEN** (2nd remote-only-SAST miss, cf detect-non-literal-regexp): add a `just semgrep` recipe wired into `just ci` / pin ruleset — own infra slice (touches justfile).
- **ADR index reconciled** via supervisor-owned doc-only chore **PR #217** (`chore/pt-a1-adr-index` @ b9f144c) → master **d423d0d** @ ~08:2xZ (`--auto` accepted, merged fast-forward, branch auto-deleted): added the ADR-0128 index row after 0127 + bumped README "Next free number" **0128→0129**. adr_next_free now **0129**.
- **Master CI:** slice head **05d0781 = CI run 29679423254 SUCCESS (GREEN)** ✓. Chore head d423d0d = run 29679599570 in_progress (doc-only, expected green). Local master ff-advanced to d423d0d; worktree `.claude/worktrees/pt-a1` removed, local+remote branch `feat/pt-a1-playtest-build-hygiene` deleted. Pre-existing untracked strays (`.claire/`, `docs/memory-cards/`) stashed for the ff then popped back (restored); older July-4 stashes left untouched.
- Cost (fix run) **$3.34**, attempts 1.
- **Resilience:** DC shell died once early (during the first `git fetch`) — restarted, switched to detached-bg + marker-file polling (no in-shell sleep) for all network ops; stable thereafter. Mutex held only across merge→cleanup→chore→record.

**NO composite launch this tick (DEFERRED, deliberate).** Next eligible = **pt-a2** (M-playtest-a 2nd sub-slice): `just playtest-up/down` + release-publish with mechanical `dev_reducers`-absent proof vs the PUBLISHED module + automated build-output DEV-hooks-absent grep guard + wipe/republish + `docs/playtest-ops.md`; **SERIAL** (justfile/structural). Defer reasons: shell churned once early this tick + chore CI still settling → per rule-1 "any doubt → next tick launches." **Model: OPUS** (Drew 2026-07-19 durable directive; mr-launch.sh default opus). ADR next-free **0129** (reserve at launch). Next tick: no live run, nothing awaiting merge, master confirmed green → launch pt-a2 off master d423d0d. Pre-gate roadmap unchanged: M-playtest-a(pt-a1✓→pt-a2)→b→c→d then PLAYTEST-READY blocker (no M18+).

---
## 2026-07-19T10:14:12Z  IN-PROGRESS (launching)  run_id=mr-sup-cowork-20260719T101220Z-4052602-9122
slice **pt-a2** (M-playtest-a 2nd sub-slice) launching off master d423d0d (CI GREEN). SERIAL (justfile/structural). ADR 0129 reserved (adr_next_free 129->130). Model OPUS (Drew 2026-07-19 directive; fable usage limit reached). Scope: just playtest-up/down + release-publish dev_reducers-absent proof + build-output DEV-hooks-absent grep guard (parked from pt-a1 §F) + wipe/republish + docs/playtest-ops.md. Supervisor owns the merge.
  · rate-limit watch: seven_day window at utilization 0.68, status=allowed_warning, isUsingOverage=false (resetsAt 1784851200) — ADVISORY, NOT a trip (five_hour window still allowed; run progressing normally). Next tick: watch seven_day climb.

## 2026-07-19T~11:14Z — pt-a2 TERMINAL: PR #218 OPEN, local full `just ci` green — supervisor to merge

- Branch `feat/pt-a2-playtest-ops` @ **3c7e5c9** (pushed; 5 wip commits to squash). PR https://github.com/mdrewt/monster-realm/pull/218 (base master). **Local full `just ci` EXIT=0** (verifier-confirmed: lint, typecheck, 1321 Rust nextest, **66/66 evals** incl. new playtest-verify, security, wasm, client-typecheck, 1065 client vitest). Remote CI **run 29684757208 in_progress** (mergeStateStatus UNSTABLE). Separate `semgrep scan --config auto --error --exclude '.claude'` (remote-only SAST) = **0 findings**; biome clean. **DID NOT wait on remote CI / did not merge — supervisor owns it.**
- **Delivered (ADR-0129), EARS pt-a2-1..-6:** `justfile` 5 recipes `playtest-up/down/verify-release/verify-build/wipe` (bash-shebang, `set -euo pipefail`, case-insensitive reject-not-clamp DB guard, honest DEFAULT release publish to isolated `monster-realm-playtest`, output-checked `sync_content`, `exec`+PID-file+`disown` vite preview, wipe re-verifies) — NOT in `just ci`. `scripts/verify-release-reducers.mjs` (pure `parseReducerNames` throws on empty/unparseable/zero/all-nameless + `findForbiddenReducers` exact-match, forbidden=`start_wild_battle`,`grant_bait` ONLY — `grant_item` is a pub(crate) helper NOT a reducer; main-guarded `execFileSync spacetime describe --json` driver, fail-loud). `scripts/verify-build-hooks.mjs` (pure `findDevHooks` binding-form + defineProperty fingerprints, NOT bare token, NOT `__mrBuild`; main-guarded `client/dist/**/*.js` scan, fail-loud on absent/empty dist). `evals/playtest-verify.eval.mjs` (touches-delta; auto-discovered CI gate; proof-of-teeth + recipe/script structural scans). `docs/playtest-ops.md` runbook.
- **Empirically validated LIVE** (local instance running, `monster-realm-playtest` published from worktree): release describe → 38 reducers, dev-absent, exit 0 / nonexistent-db exit 1; real minified dist (22 js) hook-bindings absent exit 0 / planted `window.__mrPvp=1` exit 1 / absent-dir exit 1; `vite preview` serves `http://localhost` + PID teardown works.
- **Orchestration (Opus):** planner → plan-fan (reviewer 3 BLOCKER + red-team `grant_item`-not-a-reducer BLOCKER + fingerprint/DB findings, folded into plan §K) → tester RED (eval, byte-locked f0d7601) → implementer GREEN (production only, eval untouched) → impl-fan (reviewer HIGH export-env/normalization + red-team HIGH **vite double-`client` path bug** `client/client/...` fixed to `./node_modules/.bin/vite`+exec+disown; BOTH confirmed guards SOUND — no vacuous-green) → verifier PASS (full ci + integrity: eval byte-identical since lock, 0 test files modified, teeth bite) → doc-keeper (ARCHITECTURE worktree-only ✓ trap avoided + memory card + MEMORY.md + spec Delivered note). Domain auditors (reducer-security/desync-guard) DELIBERATELY skipped (no reducer/netcode surface).
- **touches-delta (in PR body):** declared set (`justfile`, `scripts/verify-*.mjs`, `docs/playtest-ops.md`) + `evals/playtest-verify.eval.mjs` (SAFE: SERIAL slice, `evals/run.mjs` untouched) + doc companions (ADR-0129, DIGEST regen, ARCHITECTURE 3 lines, docs/specs/pt-a2-plan.md). NO client/server-module/game-core/lockfile/CHANGELOG/adr-README/run.mjs.
- **Ops notes:** (1) `spacetime describe` REQUIRES `--json` in 2.6.0; reducer names at flat `j.reducers[i].name`. (2) fresh worktree lacks `client/node_modules` → `vite: not found`; ran `npm ci` (lockfile clean). (3) `vite build` emits no sourcemaps by default; minified dist has 0 hook bindings (ADR-0127/0128 hold). (4) local spacetimedb instance + `monster-realm-playtest` module LEFT PUBLISHED (harmless; it IS the playtest DB). (5) destructive-cleanup shell commands are blocked by the guard-bash hook — used mktemp scratch dirs, left them in place.
- **ADR next-free = 0130** (pt-a2 used 0129). Worktree `.claude/worktrees/pt-a2` + branch removable after merge. codebase-memory graph reindex AFTER merge (main checkout still master@d423d0d; pt-a2 lands on master only post-merge).
- **NEXT (supervisor):** poll remote CI on #218 → squash-merge → ADR-0129 index chore (next-free → 0130). **pt-a2 completes M-playtest-a's core** (pt-a1 + pt-a2). Remaining: **pt-a3** = live nightly `playtest-smoke` recipe (DEFERRED/optional — the pt-a2 analogue of smoke-republish; adds a nightly.yml job). Hosted deploy = **M-playtest-a2**. Next pre-gate milestone per replan = **M-playtest-b** (playtest observability/feedback; the M20 pull-forward). Pre-gate roadmap: M-playtest-a(pt-a1✓→pt-a2→[pt-a3 opt])→b→c→d then PLAYTEST-READY blocker (no M18+).

---
## 2026-07-19T12:12Z — supervisor tick mr-sup-cowork-20260719T120656Z-4102691-1836 — MERGED pt-a2 → M-playtest-a core CLOSED
- **Action:** merged pt-a2 (PR #218, feat/pt-a2-playtest-ops) squash → `04c2df5`; supervisor ADR-index chore PR #219 (ADR-0129 row + next-free 0130) merged → master `abafb54`. Worktree + branch cleaned.
- **Audits:** orchestration CLEAN (tester 94 / reviewer 116 / red-team 141 / verifier 90 / planner / doc-keeper; model claude-opus-4-8). gating-test CLEAN (0 removed/skipped tests). Diff-vs-touches within declared + doc-aggregation set; benign extras `evals/playtest-verify.eval.mjs` (slice's own eval) + `docs/specs/pt-a2-plan.md`; no in-flight sibling → zero collision.
- **CI:** master GREEN @ 04c2df5 (pt-a2 substantive merge, run 29686478338 SUCCESS). abafb54 (ADR-index doc-only) CI run 29686615505 in_progress — expected green.
- **Milestone status:** **M-playtest-a core CLOSED** (pt-a1 #216 + pt-a2 #218). Per updated spec: `pt-a3` (nightly `playtest-smoke` recipe) = OPTIONAL; `M-playtest-a2` (hosted deploy / external testers) = DEFERRED.
- **NEXT (next tick):** advance to **M-playtest-b** (observability/feedback — the M20 pull-forward: client error overlay + event ring + F9 bug-report bundle + additive `playtest_event` table + `just playtest-report`). Needs a **build-time slicing pass** to define pt-b1/pt-b2 + finalized `touches:` before launch; **pt-b2 SERIAL** (additive schema → structural set). Composite launch NOT taken this tick — M-playtest-b unsliced, avoiding mispredicted-touches waste.
- **Anomaly noted:** PLAN/specs live in HARNESS repo `$HARNESS/specs/monster-realm-v2/` (not the project repo); SKILL's `specs/monster-realm-v2/PLAN.md` path is harness-relative. No action needed — just recording so future ticks don't `find`-hunt.
- **SYSTEMIC still-open:** local `just ci` `security` recipe runs only check-secrets.mjs, NOT Semgrep (repeat remote-only-SAST miss) — own infra slice (touches justfile) still unbooked.
- adr_next_free = 130. consecutive_standdowns = 0.

---
### IN-PROGRESS 2026-07-19T14:12:41Z — mr-sup-cowork-20260719T141107Z-4120556-18698
LAUNCHING slice **pt-b1** (M-playtest-b: client error overlay + event ring + F9 bug-report bundle) off master abafb54 (CI green). Model=opus. ADR 0130 reserved. Client-only; pt-b2 (playtest_event schema) stays SERIAL for a later tick. Brief /tmp/mr_pass_pt-b1.md.

---
### LAUNCHED 2026-07-19T14:15:49Z — mr-sup-cowork-20260719T141107Z-4120556-18698
Slice **pt-b1** launched & asserted: detached rooted run (session_leader=4122423, claude_pid=4122429), model claude-opus-4-8, off master abafb54 (CI GREEN). ADR **0130** reserved (adr_next_free->131). Scope: client error overlay + event ring + F9 bug-report bundle (client-only). pt-b2 (playtest_event additive schema + reducer hooks + reaper + just playtest-report) remains SERIAL/structural for a later tick; M-playtest-d is out of build-order so NO fan-out this tick. Supervisor released chain-owner mutex; next tick's fast-path will see the live pid.

## 2026-07-19T~11:2xZ — pt-b1 TERMINAL: PR #220 OPEN, local full `just ci` green — supervisor to merge

- Branch `feat/pt-b1-observability` @ **a8f460e** (pushed; wip commits to squash). PR https://github.com/mdrewt/monster-realm/pull/220 (base master). **Local full `just ci` EXIT=0** (verifier-confirmed: cargo fmt/clippy, **1321 Rust nextest**, cargo doctest, **66/66 evals**, check-secrets, wasm, tsc, **1125 client vitest / 46 files**). Separate **Semgrep** `--config auto --error` (remote-only SAST) = **0 findings**; biome exit 0. Remote CI **run 29692881760** (ci+e2e) pending. **DID NOT wait on remote CI / did not merge — supervisor owns it.**
- **Delivered (ADR-0130), EARS pt-b1 U-1..6/E-1..11/S-1..3:** client-only observability on the M13.5 seam. 5 new pure/view modules under `client/src/ui/`: `eventRing.ts` (FIFO cap 256, monotonic tSeq-from-1 never-reset, injected clock; 14-variant PlaytestEvent taxonomy + make* + `isPvpBattle` party-guard helper), `errorRing.ts` (cap 64/msg 512, `normalizeError` TOTAL incl hostile-toString), `bugBundle.ts` (PURE — no net/* import; bigint-total JSON replacer; sha-sanitized filename; `BugBundleInput=Omit<BugBundle,'schema'>`), `errorOverlayModel.ts`, `errorOverlayView.ts` (self-mount `#mr-error-overlay`, pointer-events:none, total render, textContent-only). `main.ts` wiring (coverage-excluded, guarded by `main.wiring.test.ts` source-scans): `pushError` re-entrancy latch; window error/unhandledrejection listeners; augmented `reportError` (keeps #status+console); F9 Blob download (sentinel `// F9-BUNDLE-BEGIN/END`, NO network, CSP fallback); F8 dismiss; disconnect gated `where==='link'` ONLY; connect on onReady+onReconnect; battleStart/End + rankedMatch in DEDICATED unconditional onBatchApplied listeners with latches (`activeBattleId`/`lastOwnRating`/`battleReseedPending`) reset in resetPredictionState.
- **CORE+PARK right-size:** 6 events wired (connect/disconnect/zoneChange/battleStart/battleEnd/rankedMatch); 8 handler-wired PARKED to **pt-b1b** (full 14-variant taxonomy pre-committed in ADR-0130). **pt-b2** = server playtest_event schema (SERIAL/structural), still a later tick.
- **Orchestration (Opus):** planner → plan-fan (reviewer BLOCKER B-1 rankedMatch-visibility-gate + red-team B-1 disconnect-where/B-2 error-PII, folded into ADR pre-impl) → tester RED (6 files, 64 teeth) → **specialist (general-purpose) process DIED mid-run** but files landed on disk → orchestrator committed partial (039a189), verified green → impl-fan (reviewer H-1 isPvp-wild-mislabel + red-team M-1 reconnect-dup/M-2 rankedMatch-battleId/L-1 serialize-in-try/L-2 keystore-tooth + determinism-lens NO-findings) → orchestrator folded all fixes + added T-ISPVP/W-KEYSTORE-NOPII tests → /simplify (BugBundleInput→Omit) → doc-keeper (memory card + MEMORY.md + ARCHITECTURE + spec note) → verifier PASS (full ci + semgrep + gating-integrity additive-only 0-deletions).
- **touches-delta (in PR body):** declared set (`client/src/ui/*`, `main.ts`) + `main.wiring.test.ts` (sibling) + doc companions (ADR-0130, DIGEST regen, `docs/specs/pt-b1-plan.md`, ARCHITECTURE 8 lines). store.ts declared-but-UNMODIFIED (observed only). NO structural-set file (vite.config/index.html/net-except-store/evals/module_bindings/lockfiles/CHANGELOG/adr-README). Out-of-PR: harness spec delivery note (#220) + ~/.claude memory card.
- **Ops notes:** (1) **specialist async-agent process death** during the long impl run — files survived on disk; orchestrator recovered by committing + verifying (lesson: push after every phase; agent process-death ≠ file loss). (2) fresh worktree needs `npm ci` + `just wasm` (tsc/vite need client-wasm/pkg). (3) biome `useConst` auto-fix flipped a momentarily-unreassigned `let`→`const` mid-edit → re-assert `let` after adding reassignments. (4) `just knowledge` was a no-op (client-only, no reducers/modules indexed). (5) coverage/mutation are NIGHTLY not in `just ci`; new modules 100% (eventRing/errorRing/bugBundle), aggregate 97.8% (>96).
- **ADR next-free = 0131** (pt-b1 used 0130). Worktree `.claude/worktrees/pt-b1` + branch removable after merge. codebase-memory graph reindex AFTER merge (main checkout still master@abafb54; pt-b1 lands on master only post-merge).
- **NEXT (supervisor):** poll remote CI on #220 → squash-merge → ADR-0130 index chore (next-free 0130→0131). Then **pt-b1b** (the 8 parked handler-wired emit-sites + delta-tracking; client-only, likely fan-out-eligible) OR **pt-b2** (server playtest_event additive schema — SERIAL/structural). Pre-gate roadmap: M-playtest-b(pt-b1✓→pt-b1b→pt-b2)→M-playtest-c→d then PLAYTEST-READY blocker (no M18+).

---
## 2026-07-19T16:17Z — supervisor tick mr-sup-cowork-20260719T160610Z-4304-6378 — MERGED pt-b1 (PR #220) → M-playtest-b advancing

**Action:** merged the finished pt-b1 run (`.done` EXIT=0 ATTEMPTS=2; both run pids dead; no live chain).

- **Merge:** PR #220 `feat/pt-b1-observability` squash-merged → `e56983e` (client error overlay + event ring + F9 bug-report bundle, ADR-0130). CI: ci+e2e green pre-merge; master CI @ e56983e = success.
- **Audits:** model `claude-opus-4-8` ✓ (OPUS directive); orchestration CLEAN (tester 148 / reviewer 312 / verifier 110 / red-team 336 / review-lens 92 invocations); gating-test integrity CLEAN (no skip/only/ignore/xit; +1249 test lines, 0 deleted). Pre-merge diff ⊆ declared touches; no structural-set files; no in-flight siblings.
- **ADR index reconciliation:** doc-only chore PR #221 `chore/pt-b1-adr-index` squash-merged → `67ab5c0` (added 0130 catalog row + bumped Next-free → 0131). Doc-only chore CI @ 67ab5c0 still in_progress at tick end (expected pass; next tick verifies).
- **Cleanup:** pt-b1 worktree removed, branch `feat/pt-b1-observability` deleted, per-run lock removed. Local master ff → `67ab5c0`.
- **State:** `adr_next_free=131`; inflight/awaiting_merge cleared; consecutive_standdowns=0.
- **NEXT:** pt-b2 (M-playtest-b: playtest_event additive schema + reducer hooks + reaper + `just playtest-report`) — SERIAL/structural, NEVER fanned out. Deferred to next tick to launch off a confirmed-green master (chore CI was mid-flight; "any doubt → next tick"). Then pt-c, pt-d, then PLAYTEST GATE (blocked:playtest-gate; do NOT start M18+). Model = OPUS (fable limit reached 2026-07-19).

---

## 2026-07-19T18:16Z — supervisor tick mr-sup-cowork-20260719T181549Z-23599-8218 — LAUNCHING pt-b2 (M-playtest-b server capture) [IN-PROGRESS]

**Gates:** master CI CONFIRMED GREEN @ 67ab5c0 (chore CI `success`, resolves last tick's mid-flight note); no open PRs; no stray worktrees/locks; no active human session (no resident IDE claude, no recent writes); harness+proj fetch RC=0. Mutex acquired.
**Action:** launching `pt-b2` (SERIAL/structural — additive `playtest_event` table + reducer hooks + reaper + `just playtest-report` GDD §4 proxy report). ADR reserved **0131**. Model=OPUS. Touches: server-module/src/*, game-core(helper-only), scripts/*, justfile. Not fanned out (structural/schema).
**Resume point if this breadcrumb is the last entry:** check `gh pr list` for `feat/pt-b2-*`, `/tmp/mr_pass_pt-b2.done`, and the run session-leader pid before re-launching.

---

## 2026-07-19T~19:4xZ — pt-b2 TERMINAL: PR #222 OPEN, local full `just ci` GREEN + Semgrep clean + verifier PASS — supervisor to merge

- Branch `feat/pt-b2-playtest-capture` @ **9fe947e** (pushed; 4 wip commits to squash). PR **https://github.com/mdrewt/monster-realm/pull/222** (base master). **Local full `just ci` EXIT=0** (lint/typecheck, **1349 Rust nextest**, doctests, **68/68 evals**, check-secrets, wasm, client tsc, **1125 client vitest / 46 files**). Separate **Semgrep** `--config auto --error` on changed files = **0 findings**. **DID NOT wait on remote CI / did not merge — supervisor owns it.**
- **Verifier PASS** (gating-integrity + teeth-bite): my review-gate edits to gating tests are doc-comment/needle-wrapper(useless_concat)/scan-order-correction(ADR-0125)/header/additive-gate ONLY — **zero assertions removed, zero tests deleted, zero skip/ignore/only**. All 5 spot-checked proof-of-teeth BITE on deliberate violations then revert green (cap-eviction-by-contents, scheduler-guard, emit-exactly-once, privacy-public, ORDER-BY).
- **Delivered (ADR-0131):** additive PRIVATE `playtest_event` table + **first interval-singleton `ScheduleAt::Interval` TTL+cap reaper** (`server-module/src/playtest.rs`; pure `plan_reap`/`plan_reaper_arm`/`hp_permille`/`build_playtest_event` seams; scheduler-guard-first; cap 20k/TTL 7d/5min/batch 8192) + **single-site emit** in `attempt_recruit` (after roll, before branch; pre-roll HP; infallible; zero determinism perturbation; game-core untouched) + `just playtest-report`/`scripts/playtest-report.mjs` (pure `aggregateReport` H1 weaken-first + H2 re-catch; execFileSync array-args; ORDER BY event_id ASC; PII-firewall rates-only). `kind` = u16 code (internal exhaustive `PlaytestKind`, NOT SpacetimeType). NO CONTENT_VERSION bump.
- **Right-size:** H1 recruit spine only (H2 derived from recruit species repeats). **Parked pt-b2b:** H3 emit sites (session/trade/ranked) + battle-end + report H3 sections — pure-additive (call-sites + additive columns, reserved kind codes 2–5).
- **touches-delta (in PR body):** `client/src/module_bindings/types.ts` (mechanical `spacetime generate` regen — **private tables still register their row TYPE in the shared types.ts**, only the `_table.ts` accessor is omitted; only types.ts differs; SERIAL → collision-free) + `evals/baselines/table-schemas.json` (additive rebaseline) + 2 slice-own auto-discovered evals (`playtest-event-privacy`, `playtest-report`) + doc companions (ADR-0131, DIGEST, knowledge/**, ARCHITECTURE). NO game-core, NO CHANGELOG (git-cliff; specialist stash left it UU → restored to HEAD), NO adr/README.
- **Ops notes:** (1) orphan-`/*` in a test doc comment blinded the schema-snapshot comment-stripper (dropped pvp.rs scheduled tables via cross-file `*/` pairing) — reworded. (2) `clippy::useless_concat` (Rust 1.96) flags single-arg `concat!` — collapsed to literals. (3) **async subagent output files buffer** — red-team+verifier output sat at 151 bytes for min looked dead but completed (68/80 tool calls). (4) worktree needs `npm ci` for client checks; wasm pkg already present.
- **ADR next-free = 0132** (pt-b2 used 0131). Worktree `.claude/worktrees/pt-b2` + branch removable after merge.
- **NEXT (supervisor):** poll remote CI on #222 → squash-merge → ADR-0131 index chore (next-free 0131→0132). Then **pt-b2b** (H3 server emit sites, SERIAL) OR **pt-b1b** (8 parked client emit-sites) → M-playtest-c → d → PLAYTEST GATE (do NOT start M18+).

---

## 2026-07-19T20:22Z — supervisor tick mr-sup-cowork-20260719T201030Z-146606-2155 — MERGED pt-b2

**Action:** merged pt-b2 (PR #222) + ADR-0131 index reconcile chore (PR #223). No launch this tick (established rhythm: next tick launches pt-c off confirmed-green master, mirroring pt-b1→pt-b2).

- **pt-b2 run:** EXIT=0 ATTEMPTS=1, clean .done, empty .err. session_leader 25273 dead. Cost $58.41. Orchestrator model claude-opus-4-8 ✓ (subagents on sonnet-4-6, fine).
- **Audits — both CLEAN:** orchestration (tester+reviewer+verifier+red-team+planner all invoked); gating-test (25 rust #[test] intact, no #[ignore]/skip/only/xit, no removed assertions, evals fail-loud with process.exit(1)).
- **PR #222** CLEAN/MERGEABLE, ci+e2e green → squash-merged → **master d0b1ef2** (CI **success**, verified green). Worktree `.claude/worktrees/pt-b2` removed, local+remote branch deleted.
- **Touches note (non-blocking):** actual diff exceeded declared `touches:` (server-module/src/*, game-core helper, scripts/*, justfile) — also added `client/src/module_bindings/types.ts` (structural, expected from schema-additive table), `docs/knowledge/**`, `evals/**`. Slice ran SERIAL (zero in-flight siblings) so no collision — safe. Consider declaring these in future schema-slice briefs.
- **ADR-0131 index reconcile:** doc-only chore PR #223 (`chore/pt-b2-adr-index`) added the 0131 row after 0130 + bumped README "Next free number" 0131→0132; `--squash --auto --delete-branch` merged → **master f82fbd7**. Chore CI in_progress at record time (doc-only, expected pass; next tick verifies before launching pt-c).
- **State:** adr_next_free=132, inflight cleared, no parked slices, no rate-limit, consecutive_standdowns=0.

**NEXT tick:** confirm f82fbd7 CI green, then launch **pt-c** (client-observability/UX per replan; subsumes m17b-2 set_profile_name park) off confirmed-green master. Order remaining: pt-c → pt-d → **PLAYTEST GATE** (blocked:playtest-gate; do NOT start M18+). Model=OPUS.

## 2026-07-19T22:15:50Z — mr-sup-cowork-20260719T220705Z-146606-27432 — LAUNCH pt-c1 (IN-PROGRESS)
Master green @ f82fbd7 (pt-b2 merge d0b1ef2 + ADR-0131 index chore f82fbd7, both CI success). No live run, no rate-limit park, no active human session (clean re-probe). Launching **pt-c1** SERIAL off f82fbd7: server-side `set_profile_name` reducer + RL-7 tooth amendment (D-17.5-C) + leaderboard/profile rename UI; subsumes parked m17b-2. ADR-0132 reserved. Structural (module_bindings regen) → no fan-out. Model=OPUS. pt-c2 (trade-propose UI/help/PLAYTEST.md) is the next serial slice. Strays in project checkout: untracked `.claire/`, `docs/memory-cards/` (left as-is, not mine to commit).

## 2026-07-19T~19:2xZ — pt-c1 TERMINAL: PR #224 OPEN, local full `just ci` GREEN + Semgrep clean + verifier PASS — supervisor to merge
- Branch `feat/pt-c1-profile-rename` @ **261064c** (pushed; 5 wip/feat/docs commits to squash). PR **https://github.com/mdrewt/monster-realm/pull/224** (base master). **Local full `just ci` EXIT=0** (lint/typecheck, Rust nextest, doctests, **68/68 evals** incl. refined `ranking-security`, check-secrets, wasm, client tsc, **1125 client vitest / 46 files**). Separate **Semgrep** `--config auto --error` on changed source = **0 findings**. **DID NOT wait on remote CI / did not merge — supervisor owns it.**
- **RIGHT-SIZED:** shipped SERVER-ONLY (reducer + RL-7 tooth + bindings + docs). **Client rename UI PARKED to pt-c1b** — it CANNOT live in `client/src/ui/leaderboard*` (RL-15 source-scans forbid `set_profile_name`/`reducers.` there), so it needs a new ui file + `main.ts` + `index.html` (main.ts = pt-c2 serial-overlap file); bindings must exist first. m17a→m17b rhythm.
- **Delivered (ADR-0132, EARS pt-c1-1..6):** `set_profile_name` reducer in `ranking.rs` — the FIRST reducer in the previously reducer-free module-write-only module. **Option (a): writes `player.name` ONLY** (reject-not-clamp via `guards::validate_name`, same rules as join_game; `match`-form joined-guard rejects "not joined"); the ADR-0125 mirror surfaces the rename on the leaderboard next rated game. NEVER touches `profile` (no rating/W/L, no row create). RL-7 tooth refined (`ranking-security.eval.mjs` A1 + `pvp_tests.rs` (i)) from "zero reducers" → "exactly one reducer, name-extracted==set_profile_name, profile-untouching ALLOWLIST" — monotone-stronger (verifier: +4 Rust assertions/+4 eval fixtures, 0 removed; all prior bites preserved; teeth bite-verified via inject-profile-access→RED + .unwrap→RED spot-checks).
- **Design forks resolved:** (b) eager profile-mirror REJECTED (breaks the `==2` update pin = ADR-0125 D1 invariant); (c) profile.name-only REJECTED (mirror silent-revert). Red-team F1/F2/F3 (blocklist-evasion via `p.rating=`/helper/get_or_init-injection) closed by the allowlist + whole-file `update==2`/`insert==1` backstops. tests-review MAJOR (pt-c1-3 not-joined had no pin) closed by `ptc1_scan_body_rejects_when_not_joined` (find(+Err(, bans `.unwrap`). Homoglyph/dup names + no-cooldown ACCEPTED for closed test (D4, SSOT parity with join_game).
- **Orchestration (Opus orch):** planner → plan-fan (reviewer BLOCKER: b breaks ==2 pin → switch to a; red-team F1-F6 → allowlist teeth) → ADR-0132 + plan checkpoint → tester RED (pvp_tests i/ranking_tests scans/eval A1) → tests-review (pt-c1-3 gap) → tester resume (+not-joined pin) [orch fixed 3 E0716 + 1 fmt-arg + 1 `.unwrap(`→`.unwrap` needle bug — mechanical, no assertion change] → specialist (general-purpose, Opus) red→green [21/21, only ranking.rs] → `just gen` bindings + `just knowledge` → impl-fan (reviewer clean/red-team SECURE 0-findings) → doc-keeper (ARCHITECTURE + memory card in ~/.claude + harness spec note) → verifier PASS.
- **touches-delta (in PR body):** declared set was `ranking.rs`+tests / `client/src/ui/leaderboard*` / evals needle. Actual: `ranking.rs`(+39) + `ranking_tests.rs`(+526) + **`pvp_tests.rs`(+87, the RL-7 tooth = named deliverable)** + `ranking-security.eval.mjs`(+333) + `client/src/module_bindings/**`(spacetime generate regen) + docs(ADR-0132/DIGEST/knowledge/pt-c1-plan/ARCHITECTURE 1-line). **`client/src/ui/leaderboard*` NOT touched** (UI parked → reduction). NO game-core, NO schema, NO CHANGELOG hand-edit, NO adr/README. All in sanctioned set (verifier-confirmed).
- **ADR next-free = 0133** (pt-c1 used 0132). Worktree `.claude/worktrees/pt-c1` + branch removable after merge.
- **NEXT (supervisor):** poll remote CI on #224 → squash-merge → ADR-0132 index chore (0132→0133). Then **pt-c1b** (client rename UI: new ui file + main.ts + index.html → `reducers.setProfileName` + leaderboard-reflects e2e; client-only but main.ts SERIAL vs pt-a1/pt-b1/pt-c2) OR **pt-c2** (trade-propose UI + help overlay + docs/PLAYTEST.md). Then M-playtest-d (content) → PLAYTEST GATE (do NOT start M18+).

---
## 2026-07-20T00:18Z — pt-c1 MERGED (run mr-sup-cowork-20260720T001051Z)
- **Action:** merged pt-c1 PR #224 (squash → master `6310126`); server `set_profile_name` reducer (validate_name + player.name-only write, Option a) + RL-7 module-write-only tooth refinement (ADR-0132, amends ADR-0119). Subsumes m17b-2 set_profile_name park.
- **Gates:** PR #224 CLEAN/MERGEABLE, ci+e2e pass. Master CI on merge-base f82fbd7 confirmed success (the prior pt-b2 ADR-index chore CI passed). Active-session probe clean (no IDE, no recent writes) at gate-top AND pre-merge re-probe.
- **Orchestration audit: CLEAN** — model Opus (claude-opus-4-8); tester+reviewer+red-team+verifier+planner+doc-keeper roles all present.
- **Gating-test audit: CLEAN** — the RL-7 tooth (ranking-security.eval A1 + pvp_tests) was AMENDED not weakened: "zero reducers in ranking.rs" → "exactly one profile-untouching set_profile_name reducer" (validate_name + player().identity().update, bans profile().insert/delete/get_or_init_profile). Security invariant preserved; closes red-team F1/F2/F3 (rating-1000 leaderboard-row injection, wrong-named-reducer evasion). Documented in ADR-0132. 7 new #[test] added.
- **ADR index:** reconciled via doc-only chore PR #225 (chore/pt-c1-adr-index; 0132 row + next-free→0133); `--auto` squash-merge armed (EXIT=0). adr_next_free now 133.
- **Cleanup:** pt-c1 worktree removed, local+remote feat branch deleted. Main checkout restored to master 6310126.
- **No composite launch this tick:** post-merge CI on 6310126 still in_progress, AND next-slice ambiguity — ADR-0132 states the client rename UI is parked to a NEW slice **pt-c1b** (needs main.ts/index.html; can't live under client/src/ui/leaderboard* per RL-15), whereas earlier queue notes referenced "pt-c2". NEXT TICK: (1) verify master CI green on 6310126; (2) confirm chore PR #225 merged + reconcile adr_next_free; (3) resolve pt-c1b-vs-pt-c2 naming against PLAN §9 and launch the next eligible pre-gate slice (pt-c1b client rename UI is the immediate spine continuation). Cost this pass $33.62, attempts 1.

### 2026-07-20T00:22Z — ADDENDUM (same run): RED-TIP on ADR-index chore
- ADR-index chore PR #225 auto-merged → master tip **68b4176**. Its CI conclusion = **failure**, BUT: (a) change is DOC-ONLY (README.md, 1 file); (b) GitHub Actions API was returning **HTTP 503** at ~00:20Z (job-step logs unfetchable to classify); (c) the pt-c1 CODE merge **6310126 is GREEN**. → Highest-probability transient/infra or semgrep-ruleset-drift, NOT a real code red — but NOT verified.
- **NEXT TICK MUST:** re-fetch gh run for 68b4176 once GitHub recovers; inspect failed step. Semgrep-only/5xx → re-run CI (doc-only, safe) to green. Real red → fix/revert-to-green FIRST. **Do not launch any slice until master tip is confirmed green.**
- Mutex released; per-run lock + pt-c1.done cleaned; no live runs. consecutive_standdowns=0.

---
## 2026-07-20T02:19Z — master tip 68b4176 VERIFIED GREEN (run mr-sup-cowork-20260720T021208Z-240173-12293)
- **Priority target (from prior tick's RED-TIP addendum):** verify/fix master tip `68b4176` (doc-only ADR-index chore #225 that showed CI=failure during the ~00:17Z GitHub Actions 503 outage). No live runs at gate-top; active-session probe clean (no IDE, no recent writes).
- **Diagnosis (GitHub recovered → logs fetchable):** failed job = `ci`, failed step = `gitleaks/gitleaks-action`. Log: `Get user [mdrewt] failed with error [HttpError: No server is currently available...]` → license-key validation forced → `🛑 missing gitleaks license`. i.e. the action's license-validation GitHub API call hit the 503 outage and fell into enforcement mode. NOT a secrets leak, NOT a code red — identical CI was GREEN on the pt-c1 code merge `6310126` 5 min earlier and on every prior commit.
- **Action:** `gh run rerun 29709277995 --failed` (only the `ci` job had failed; `e2e` already green). Post-recovery re-run → **completed success** (ci + e2e). Master tip 68b4176 now CONFIRMED GREEN. origin/master unchanged @ 68b4176.
- **No slice launched this tick:** single-step model — a CI re-run is not the sanctioned merge→launch composite, and the DC shell churned twice mid-tick (recovered via mutex-ownership refresh to the live pid). Master is now green + unblocked.
- **NEXT TICK:** launch the next eligible pre-gate slice. Resolve **pt-c1b vs pt-c2** naming against PLAN §9 — ADR-0132 parks the client rename UI to NEW slice **pt-c1b** (main.ts/index.html + new ui file → `reducers.setProfileName` + leaderboard-reflects e2e; client-only, main.ts SERIAL vs pt-a1/pt-b1/pt-c2 per RL-15; NOT client/src/ui/leaderboard*). pt-c2 = trade-propose UI + help overlay + docs/PLAYTEST.md. Then M-playtest-d → PLAYTEST-READY gate (do NOT start M18+). adr_next_free=133. consecutive_standdowns=0.
- Mutex released; no per-run locks; no live runs; no `.done` pending.

## 2026-07-20T04:20:44Z — tick mr-sup-cowork-20260720T041858Z-259386-4887 — LAUNCH pt-c1b (IN-PROGRESS)
- master tip 68b4176 GREEN, no open PRs, no active human session (watchdog tick).
- Next pre-gate slice = pt-c1b (client rename UI): main.ts/index.html + new client/src/ui rename model/view -> reducers.setProfileName (merged in pt-c1/ADR-0132), + rename->leaderboard e2e. Client-only, main.ts SERIAL. No fan-out (single serial slice).
- ADR reserved: 0133. adr_next_free -> 134.
- Launching detached via mr-launch.sh (--model opus). Supervisor owns the merge.

## 2026-07-20T~05:35Z — pt-c1b TERMINAL: PR #226 OPEN, local full `just ci` GREEN + Semgrep clean + verifier PASS + e2e green — supervisor to merge
- Branch `feat/pt-c1b-rename-ui` @ **44de275** (pushed; 6 wip/feat/test/fix/chore/docs commits to squash). PR **https://github.com/mdrewt/monster-realm/pull/226** (base master, MERGEABLE, mergeStateStatus UNSTABLE = remote CI run 29719445042 in_progress). **Local full `just ci` EXIT=0** (lint, 1356 rust nextest, doctests, **68/68 evals** incl. adr-digest, check-secrets, wasm, client tsc, **1181 client vitest / 48 files**). Separate **Semgrep** `--config auto --error` on 7 changed files = **0 findings**. **`client/e2e/rename.spec.ts` run locally = GREEN** against the live local spacetime (server-truth SQL round-trip). **DID NOT wait on remote CI / did not merge — supervisor owns it.**
- **Delivered (ADR-0133, EARS PTC1B-1..9):** client rename UI. `KeyN` overlay wiring the merged `set_profile_name` (pt-c1). `renameModel.ts` (pure VM: trim+non-empty ONLY, server validate_name is SSOT) + `renameView.ts` (**FIRST text-input overlay**; fully happy-dom-covered → NOT in coverage.exclude; input+button `stopPropagation`, deferred `setTimeout(focus,0)`, `hide()` resets input/feedback/`#pending`, single `#submit()` Enter+click with `.finally().catch()` lock, live button-enable via `input` listener, textContent-only). `main.ts`: KeyN handler (11-sibling guard, `held.clear()` on open, `e.preventDefault()`, current name from `store.player(identity)?.name`), async onSubmit→setProfileName (frozen-gate + `reduceErrorMessage` no-leak), **20-site `renameView?.visible` fan-out**. `index.html` shell.
- **RIGHT-SIZED:** shipped the lighter single-context server-truth-SQL e2e. **Parked → pt-c1b2:** the full `rename → ranked game → leaderboard-DOM reflects` two-context e2e (heavy disconnect-forfeit apparatus; the player.name→profile.name mirror is already proven by ADR-0125/pt-c1).
- **Orchestration (Opus orch):** planner → plan-fan (reviewer 2 BLOCKERs: deferred-focus + currentName-from-player; red-team RT-RN-01 CRITICAL held-key-corruption → held.clear()) → ADR-0133 + plan checkpoint → tester RED (renameModel/View units + main.wiring per-context fan-out scans + rename e2e) → specialist (general-purpose, Opus) red→green → impl-fan (reviewer BLOCKER B-1 hide()-#pending; red-team F1 button-stopPropagation/F2 stale-feedback; **netcode/desync review-lens CLEAN**) → orchestrator folded all fixes + **found 2 impl bugs the unit suite missed** (vitest exits 1 on unhandled rejection → `.catch()`; button never re-enables on type — real chromium vs happy-dom → `input` listener; both caught by running the FULL vitest exit-code + the e2e) + added 3 additive regression tests + fixed e2e sql string-quote stripping → doc-keeper (ARCHITECTURE + memory card) → verifier PASS (gating additive-only, teeth bite).
- **touches-delta (in PR body):** declared set (main.ts, index.html, ui/rename*.ts, e2e/rename*.spec.ts) + siblings (main.wiring.test.ts, renameModel/View.test.ts) + docs (ADR-0133, DIGEST regen via `just adr-digest`, docs/specs/pt-c1b-plan.md, ARCHITECTURE 5-line). **NO module_bindings, NO leaderboard*, NO vite.config, NO evals/**, NO CHANGELOG hand-edit, NO adr/README.** All in sanctioned set (verifier-confirmed).
- **Ops notes:** (1) fresh worktree needs `npm ci` + `just wasm`. (2) **`spacetime sql` wraps STRING columns in double-quotes** — the e2e parser must strip them (ranked-forfeit never hit this: numeric cols only). (3) **vitest exits 1 on ANY unhandled rejection even when all tests pass** — the shopView-family `void Promise.resolve(cb).finally()` needs a trailing `.catch()` when a test hands it a rejecting callback. (4) ADR subsystem vocab is controlled: `movement-netcode` not `netcode-input`; regen DIGEST via `just adr-digest`. (5) real chromium won't fire click on a disabled button (happy-dom does) → live button-enable needed an `input` listener; caught only by the real-browser e2e.
- **ADR next-free = 0134** (pt-c1b used 0133). Worktree `.claude/worktrees/pt-c1b` + branch removable after merge.
- **NEXT (supervisor):** poll remote CI on #226 → squash-merge → ADR-0133 index chore (0133→0134). Then **pt-c1b2** (parked leaderboard-DOM e2e) OR **pt-c2** (trade-propose UI + help overlay + `docs/PLAYTEST.md`) → M-playtest-d → PLAYTEST GATE (do NOT start M18+). Memory card written: `monster-realm-pt-c1b.md`.

## 2026-07-20T06:31:16Z — LAUNCHING pt-c2 (composite after pt-c1b merge) — mr-sup-cowork-20260720T060703Z-333797-22957
- pt-c1b MERGED (PR #226 → master a63724f) + ADR-0133 index chore (PR #227 → master 0421f2c, auto-merged). master GREEN.
- Composite launch: pt-c2 (M-playtest-c UX) — trade PROPOSE UI core (H3/D-17.5-D); help overlay + docs/PLAYTEST.md right-size/park to pt-c2b. ADR-0134 reserved. Client-serial (main.ts), no fan-out.
- Launching detached opus via mr-launch.sh. session_leader TBD post-launch.


<!-- archived 2026-07-21T17:19:30Z by corpus-tidy (native-cutover follow-up) -->
## 2026-07-20T~23:10Z — ptc5d TERMINAL: PR #234 OPEN, local full `just ci` GREEN — supervisor to merge
- Branch `feat/ptc5d-okf-tests-filter` @ **fa69c91** (pushed; commits: plan+ADR `5f90eab`, RED teeth `3a8a40f`, impl `80b8c76`, docs+review-fixes `fa69c91`). PR **https://github.com/mdrewt/monster-realm/pull/234** (base master). **Local full `just ci` EXIT=0** (clippy `-D warnings`, biome, **1360 nextest / 0 skipped**, **68/68 evals** incl. knowledge-bundle-conformance + nightly-smoke-wiring + movement/prediction-parity, check-secrets clean, wasm-pack, client tsc, **1323 vitest / 52 files**) + **Semgrep `--config auto` 0 findings** on the 3 changed source files (local ci lacks Semgrep; ran manually). **DID NOT wait on remote CI / did not merge — supervisor owns it.**
- **Delivered (ADR-0137, tooling/eval hardening — no schema/reducer/game change; EARS ptc5d-1..5):** (1) `collectRsFiles` (okf-export.mjs) excludes `*_tests.rs` → bundle regen (exactly **5** files; **49→42** reducers; `set_profile_name`/`playtest_reaper` re-sourced to `ranking.rs#L139`/`playtest.rs#L158`; **tables stay 34** — filter table-set-neutral); SSOT comments (:59/:70) reconciled (deliberate divergence from `battle-schema-snapshot`). (2) conformance eval `findTestSourcedPages` scans `reducers/`+`tables/` `resource:`/`source:` lines for `_tests.rs` (**synthetic** teeth, NOT `runExport` — would self-defeat post-fix). (3) RT-M14.5A-02 `return;`→`assert!(status_applied)` (deterministic: wild.atk40 vs active.def200, power=1→2dmg, 3→1HP survives→StatusApplied→chip kills). (4) mutate-server wiring ceiling **340→299** via `MUTATE_SERVER_CAP_BASELINE` (==committed justfile cap; **amends ADR-0118 §3/A3** — every cap move now eval-visible, re-baseline bumps both justfile cap + const); L-recap flipped(309 rejected)+L-overcap(300)+L-299(accepted).
- **Orchestration (Opus orch):** planner → reviewer+red-team (plan; caught L-recap-flip + ceiling-vs-cap ADR-0118 amendment need + synthetic-fixture requirement) → ADR-0137 + plan checkpoint → **tester** (separate agent) RED teeth (all 3 surfaces confirmed RED/GREEN) → orchestrator implemented (did NOT edit teeth) → reviewer+red-team impl (**both ran the evals, confirmed all 3 teeth BITE**) → **verifier PASS** (gating diff `3a8a40f..HEAD` no weakened/skipped/deleted assertion; 340→299 is a *correction* aligning ceiling with the already-declared RED tooth; touches ⊆ declared, 13 files).
- **touches-delta (in PR body):** declared set + `docs/adr/DIGEST.md` (`just adr-digest` regen) + `docs/specs/ptc5d-plan.md` (plan doc). NOT touched: justfile, ADR-0118/0050 bodies, CHANGELOG (git-cliff), adr/README, server/game-core prod code, module_bindings, Cargo.lock.
- **Ops notes:** fresh worktree needed `npm ci` (biome/vitest) AND the **first `wasm-pack build` flaked on wasm-bindgen install** (`invalid type: sequence, expected a string`) until `just wasm` installed wasm-bindgen → then movement-parity passed. Not a code issue.
- **HARNESS bookkeeping done (supervisor to commit these harness files):** spec `M-playtest-c.5-...spec.md` ptc5d **DELIVERED note** added; memory card `monster-realm-ptc5d.md` (auto-memory dir) + MEMORY.md pointer written.
- **Residuals → ptc5f (pre-existing, out of ptc5d touch-set):** ADR-0118 A2 body still says cap=309 (justfile=299 per §4); ADR-0050 A3 still says ceiling=340 / "bumps inside ceiling don't require eval edit" (now false under D4). **Supervisor owns** `Amended-by: 0137` backlink on ADR-0118 at ADR-index reconciliation.
- **ADR next-free = 0138** (ptc5d used 0137). Worktree `.claude/worktrees/ptc5d` + branch removable after merge. Graph reindex AFTER merge (main checkout still master).
- **NEXT (supervisor):** poll remote CI on #234 → squash-merge → ADR-0137 index chore (README next-free 0137→0138 + `Amended-by: 0137` on ADR-0118). Then remaining M-playtest-c.5: **ptc5b** (wild-battle disconnect+GC, server SERIAL) · **ptc5c** (overlay mutual-excl, client main.ts SERIAL) · **ptc5e** (SSOT/content/dedup) · **ptc5f** (ledger recon incl. the ADR-0118 A2 / ADR-0050 A3 residuals above + ADR-0090 Decision-A) → **M-playtest-d** (content) → **PLAYTEST GATE** (do NOT start M18+).

## 2026-07-20T~21:10Z — ptc5a TERMINAL: PR #232 OPEN, local full `just ci` GREEN — supervisor to merge
- Branch `feat/ptc5a-care-train-battle-guard` @ **3473c8c** (pushed; commits: plan+ADR `21d8753`, RED tests `3879b54`, impl `7a6737f`, polish+bundle `9683961`, ARCHITECTURE `3473c8c`). PR **https://github.com/mdrewt/monster-realm/pull/232** (base master). **Local full `just ci` EXIT=0** (fmt+clippy `-D warnings`, cargo check, **rust nextest incl. 24 raising tests**, **68 evals** incl. battle-reducer-security C1 **6 reducers** + knowledge-bundle-conformance, check-secrets/semgrep, wasm-pack, client tsc, **1323 client vitest / 52 files**). **DID NOT wait on remote CI / did not merge — supervisor owns it.**
- **Delivered (ADR-0136 amends ADR-0122 §D7; EARS ptc5a-1..3):** `if is_in_ongoing_battle(ctx, ctx.sender) { return Err }` added to `care`+`train` (raising.rs) immediately after `require_owner` — reuses the caller-scoped, both-role, `!= WILD_IDENTITY`-refined SSOT helper `heal_party` calls (NOT monster-scoped `reject_if_in_battle`; ADR §D1). No new predicate/schema/import (already imported). Closes the bounded mid-battle HP-laundering path (mid-battle `train` raises live `ev_hp` → level-up heal battle.rs:1179 folds live max-HP into current_hp). §D7's "no vector" claim corrected.
- **Proof-of-teeth (all in raising_tests.rs — harness has NO in-process reducer DB):** `care_battle_guard_wired`/`train_battle_guard_wired` source-scans (needle `ifis_in_ongoing_battle(ctx,ctx.sender){` — trailing `{` beats a string-literal fake per red-team F2; eval C1 is the authoritative string-stripped gate), `both_role_predicate_scenarios` (wild side-A / PvP side-B / no-battle / WILD-sentinel refinement), differential `level_up_healed_hp` test. eval C1 `BOTH_ROLE_GUARD_REDUCERS` 4→6. Verifier CONFIRMED teeth bite (delete guard → RED, tree restored clean).
- **Orchestration (Opus orch):** planner (validated the fixed design) → orchestrator adversarial plan-pass → ADR-0136 + plan checkpoint → **tester** (sonnet) RED (4 tests + eval needle; orchestrator confirmed RED) → **orchestrator implemented** the 2-line guard directly (tester≠implementer preserved; right-sized to avoid subagent-crash risk) → **reviewer + red-team + review-lens(reducer-security)** parallel → **verifier** PASS. All lenses clean: reviewer NO blockers; reducer-security CLEAN (server-auth/reject-never-burns/RLS/SSOT/determinism); red-team NO exploitable finding (1 LOW string-literal note → addressed by needle `{` strengthening). **NOTE:** `reducer-security-auditor`/`desync-guard` agent-types are NOT in this harness's available set → used `review-lens` with a reducer-security prompt; desync-guard moot (no netcode/movement surface).
- **touches-delta (in PR body):** declared set + sibling `raising_tests.rs` + ADR-0136 + `DIGEST.md` regen (`just adr-digest`) + `docs/knowledge/reducers/{train,heal_party}.md` (**`resource:` LINE-POINTER only** — my guard added lines above them; sanctioned "docs/knowledge entries for reducers/modules touched"; regen via `just knowledge`; the broad ptc5d bundle fix is separate) + `ARCHITECTURE.md` 1-line. NOT touched: guards.rs, game-core, battle.rs, xp.rs, evolution.rs, client/**, schema/migrations, Cargo.lock, module_bindings, evals/run.mjs, CHANGELOG (git-cliff), adr/README, raising-reducer-security.eval.mjs.
- **Deferred (documented in PR):** reviewer **W-3** (consolidate the 2 pre-existing `care_reducer_*` brace-walks into the new `reducer_body` helper — kept the security diff focused); reviewer **W-1** DECLINED (battle-reducer-security C1 is the SSOT gate for the both-role guard across all 6 reducers; duplicating into raising-reducer-security would violate SSOT); ADR-0122 §D7 **`Amended-by: 0136` backlink** → supervisor ADR-index reconciliation (this slice does not edit 0122, to stay in-touch-set).
- **HARNESS bookkeeping done (supervisor to commit these harness files):** spec `M-playtest-c.5-pregate-review-residuals.spec.md` ptc5a **DELIVERED note** added; **MEMORY.md compacted 19.9KB→16.1KB** (all 104 pointers preserved, per read-limit hook) + `monster-realm-ptc5a.md` slice card written (auto-memory dir).
- **ADR next-free = 0137** (ptc5a used 0136). Worktree `.claude/worktrees/ptc5a` + branch removable after merge. **Graph reindex AFTER merge** (main checkout still master).
- **NEXT (supervisor):** poll remote CI on #232 → squash-merge → ADR-0136 index chore (README 0136 row + `Amended-by: 0136` backlink on ADR-0122 + next-free→0137). Then **ptc5b** (resolve caller's Ongoing WILD battle on disconnect + reaper — the stale-wild-battle soft-lock deferred here) ‖ **ptc5d** (knowledge-bundle `_tests.rs` filter — will re-regen the whole bundle, absorbing my train/heal_party line-pointer changes) → ptc5c (client main.ts, SERIAL) / ptc5e / ptc5f → **PLAYTEST GATE** (do NOT start M18+).

---

## 2026-07-20T~11:55Z — pt-c2b TERMINAL: PR #230 OPEN, local full `just ci` GREEN (×2) — supervisor to merge
- Branch `feat/pt-c2b-help-overlay` @ **43b3af7** (pushed; commits: plan+ADR `add02d9`, RED tests `2ecaaa3`, impl `b8770aa`, ARCHITECTURE `16957af`, comment-clarify `43b3af7`). PR **https://github.com/mdrewt/monster-realm/pull/230** (base master). **Local full `just ci` EXIT=0 run TWICE** (after impl, and after ARCHITECTURE+comment edits): cargo fmt+clippy `-D warnings`, cargo check, **1356 rust nextest**, evals, security/semgrep clean (no `new RegExp` — indexOf/includes/split + textContent/createElement only), wasm-pack, client-typecheck, **1323 client vitest / 52 files**. **DID NOT wait on remote CI / did not merge — supervisor owns it.**
- **Delivered (ADR-0135, EARS PTC2B-1..12):** in-client **help overlay** (`?` = `e.key`, sole non-`e.code` branch; `F1` rejected) — display-only (controls key→action + session goals). `helpModel.ts` pure SSOT VM (typed const, NOT RON; fresh copy; display-only structural guard) + fully happy-dom-covered `helpView.ts` (textContent-only XSS firewall, `replaceChildren` rebuild, zero-arg, no lock). `main.ts` **19-site** mutual-exclusion fan-out. **THE deliberate asymmetry:** help NOT hidden on onReconnect (discriminator = static-content vs store-derived, NOT just "no lock" — leaderboard has no lock yet IS hidden); pinned by two-endpoint-bounded negative tooth. `docs/PLAYTEST.md` tester runbook. **M-playtest-c now COMPLETE** (pt-c1/pt-c1b/pt-c2/pt-c2b).
- **Orchestration (Opus orch):** planner → reviewer+red-team plan review (count-floor=19 not 21, rAF forward-anchor, onReconnect two-endpoint, battle tooth, drop e2e, hand-write PLAYTEST) → ADR-0135 + plan checkpoint → tester RED (13 wiring teeth bite + helpModel/helpView units) → impl → reviewer+red-team+desync-guard impl review (ALL green: desync CLEAN 3 movement sites; reviewer no blockers; red-team defends all attacks, proved each edited tooth bites on a broken variant). **⚠ NOTE: subagent runs (tester, specialist) CRASHED mid-run twice (parent process ceiling) — their file writes PERSISTED; orchestrator recovered from disk + completed residual main.ts fan-out (11→19) + PLAYTEST.md INLINE.** Verifier role satisfied by reviewer+red-team test-edit audit + green ci + orchestrator RED→HEAD gating-diff (no skip/only/ignore, no deleted tests, count tooth `>=19` unchanged).
- **⚠ Test-window edits (impl correct; supervisor audit):** 3 `main.wiring.test.ts` edits — CONFIRMED NON-WEAKENING by reviewer+red-team: (a) `W-RN-HELD` KeyN window 600→720 (help guard grew region, held.clear() 12-char string straddled 600); (b) count-floor self-check leaderboard 19→20 (help's `?` guard adds a leaderboard ref; real count tooth `>=19` untouched); (c) `W-HELP-FANOUT-PVP` anchor `'anyOverlayVisible'`→`'const anyOverlayVisible ='` (comment captured bare substring). PR body documents all 3.
- **touches-delta (in PR body):** declared set + siblings (`main.wiring.test.ts`) + doc companions (ADR-0135, DIGEST regen, ARCHITECTURE) PLUS: `client/index.html` (#help-overlay shell, SERIAL-safe) + `docs/specs/pt-c2b-plan.md`. NOT touched: vite.config, dom-shell eval, evals/**, server/game-core/module_bindings, CHANGELOG (git-cliff), adr/README (supervisor), docs/knowledge (client-only → no reducers).
- **HARNESS bookkeeping done (supervisor to commit these harness files):** spec `M-playtest-c-ux-completion.spec.md` pt-c2b DELIVERED note added (M-playtest-c COMPLETE); memory card `memory/projects/monster-realm-pt-c2b.md` + MEMORY.md pointer written. **⚠ MEMORY.md is ~19.6KB (approaching 24.4KB read limit) — needs a proper compaction pass (maintenance follow-up; I trimmed my own + 2 longest lines but did not derail the slice to hit 17.1KB).**
- **ADR next-free = 0136** (pt-c2b used 0135). Worktree `.claude/worktrees/pt-c2b` + branch removable after merge. Graph reindex AFTER merge (main checkout still master).
- **NEXT (supervisor):** poll remote CI on #230 → squash-merge → ADR-0135 index chore (README 0135 row + next-free→0136). Then **pt-c2c** (auto-show [needs localStorage seam] + trade item-offer rows) OR **M-playtest-c.5** (ptc5a..f pre-gate residuals — ptc5c is SERIAL vs main.ts, sequence after pt-c2b help lands per §3 Decision B) OR **M-playtest-d** (content, fan-out-friendly) → **PLAYTEST GATE** (do NOT start M18+).

## 2026-07-20T~08:37Z — pt-c2 TERMINAL: PR #228 OPEN, local full `just ci` GREEN + Semgrep clean + e2e green + verifier PASS — supervisor to merge
- Branch `feat/pt-c2-trade-propose` @ **9945ec9** (pushed; 6 wip/feat/fix/docs commits to squash). PR **https://github.com/mdrewt/monster-realm/pull/228** (base master, MERGEABLE, mergeStateStatus UNSTABLE = remote CI run 29728459980 in_progress). **Local full `just ci` EXIT=0** (lint, **1356 Rust nextest**, doctests, **68/68 evals**, check-secrets, wasm, client tsc, **1284 client vitest / 50 files**). Separate **Semgrep** `--config auto --error` on 4 changed source files = **0 findings** (literal regexes only, no `new RegExp`). **`client/e2e/trade-propose.spec.ts` run locally = GREEN** (1 passed, 3.8s) against live local spacetime (module republished --delete-data; UI-driven propose → specific-monster transfer verified). **DID NOT wait on remote CI / did not merge — supervisor owns it.**
- **Delivered (ADR-0134, EARS PTC2-1..16):** in-client trade-PROPOSE overlay on **KeyO** ("Offer") — first human entry point to `reducers.proposeTrade` (KeyU only RESPONDS). `tradeProposeModel.ts` (pure/TOTAL: `buildProposeLists`/`buildProposeSubmission`/`parseCurrency` digit-scan never Number/parseInt) + fully happy-dom-covered `tradeProposeView.ts` (mirrors pt-c1b renameView: textContent-only XSS firewall, stopPropagation on every focusable incl. dynamic checkboxes, deferred focus, hide()-resets-all + #pending dead-button guard, single #submit .finally().catch()) + `index.html` `#tradepropose-overlay` shell + `main.ts` (KeyO handler w/ `identity!==''` guard + held.clear + preventDefault; onSubmit frozen-gate-FIRST → new Identity + bigint arrays → reduceErrorMessage no-leak; **~24-site mutual-exclusion fan-out** incl. easy-miss **KeyN guard** + **PvP anyOverlayVisible** + **onReconnect dead-#pending-lock hide** + battle force-hide + movement/reconcile/rAF suppression + Escape branch). Counterparty side = **currency-only** (RLS: peer roster not client-enumerable, ADR-0015). Two-context UI-driven e2e asserts SPECIFIC monsterId transfer (identity, not just conservation).
- **RIGHT-SIZED:** shipped trade-propose CORE only. **Parked → pt-c2b:** item-offer rows (qty inputs); requesting SPECIFIC counterparty monsters/items (RLS-IMPOSSIBLE from client — ADR rationale, not merely deferred); in-client help/controls overlay; `docs/PLAYTEST.md` tester runbook.
- **Orchestration (Opus orch):** planner → plan-fan (reviewer B-1 KeyN-guard-miss + red-team C-1 PvP-anyOverlayVisible/C-2 onReconnect-deadlock/H-2 select-stopPropagation/H-3 currency-parse/H-5 e2e-specific-monster — all folded into ADR-0134's enumerated fan-out checklist + digit-scan parser) → ADR-0134 + plan checkpoint → tester RED (model/view units + main.wiring D7 scans + e2e; 4 files) → impl (specialist general-purpose/Opus, red→green, **edited 0 gating tests**) → impl-fan (reviewer + red-team both confirm fan-out COMPLETE + all plan findings CLOSED; desync-guard: all 4 movement sites PRESENT+CORRECT; **one `#readDraft` BigInt-guard hardening applied** for TOTAL view) → doc-keeper (ARCHITECTURE + memory card + spec note) → **verifier PASS** (gating-test diff RED→HEAD **empty**; 3 proof-of-teeth bite; 139 tests green; touches ⊆ declared; coverage policy correct).
- **touches-delta (in PR body):** declared set + siblings (`main.wiring.test.ts`, `tradeProposeModel/View.test.ts`) + doc companions (ADR-0134, DIGEST regen via `just adr-digest`, ARCHITECTURE 2-line, `docs/specs/pt-c2-plan.md`). **NO help*.ts / docs/PLAYTEST.md (parked), NO docs/knowledge/** (just knowledge no-op), NO CHANGELOG hand-edit, NO adr/README, NO module_bindings/vite.config/lockfile/server/game-core/evals.** 12 files total, all sanctioned (verifier-confirmed).
- **Ops notes:** (1) fresh worktree needed `npm ci`; wasm pkg present. (2) `just ci` fast (~90s) — target warmer than expected (specialist's earlier `just wasm`+tsc warmed clippy/nextest). (3) e2e ran clean after `just wasm` (anti-clobber) — local spacetime already up @127.0.0.1:3000. (4) async subagent output buffers at 151B (not death) — polled via size-stability/timed waits.
- **ADR next-free = 0135** (pt-c2 used 0134). Worktree `.claude/worktrees/pt-c2` + branch removable after merge. Graph reindex AFTER merge (main checkout still master).
- **NEXT (supervisor):** poll remote CI on #228 → squash-merge → ADR-0134 index chore (0134→0135). Then **pt-c2b** (item-offer rows + help overlay + `docs/PLAYTEST.md`; client-only, main.ts SERIAL) OR M-playtest-d (content) → PLAYTEST GATE (do NOT start M18+). Memory card written: `monster-realm-pt-c2.md`.
### 2026-07-20T06:36:21Z — TICK COMPLETE — pt-c1b MERGED, pt-c2 LAUNCHED
- pt-c1b: PR #226 squash-merged → a63724f; ADR-0133 index reconciled (doc-only chore PR #227 → 0421f2c, auto). master **GREEN @ 0421f2c**. worktree+branch cleaned. cost $37.25, attempts 1. audits: orchestration CLEAN, gating-test CLEAN.
- pt-c2: LAUNCHED detached opus (leader 339193 / claude 339198), ADR-0134 reserved. Terminal-state expected: PR open + local ci green → next tick merges.
- No active human session; no rate-limit trip. consecutive_standdowns=0. Mutex released.

---
## 2026-07-20T09:11Z — WEEKLY REVIEW (generate-improvement-plan, read-only) — NEW MILESTONE M-playtest-c.5 INSERTED
- **Source:** weekly read-only multi-lens review, pinned isolated `--no-hardlinks` clone @ `0421f2c` (2026-07-20T09:11Z). 7 lenses → aggregate → independent verifier fleet (17/17 findings verified, 0 dropped). Counts: 0 Critical, 0 High, 6 Medium, 11 Low. Review clone torn down (no trace; runner git state untouched).
- **Action (harness only — NO project-repo writes):** wrote new milestone spec `specs/monster-realm-v2/M-playtest-c.5-pregate-review-residuals.spec.md` and inserted its PLAN.md §9 bullet **between M-playtest-c and M-playtest-d**. Please include both harness files in your next git commits alongside your normal ADR/spec bookkeeping.
- **Milestone M-playtest-c.5 (pre-gate residuals, no new game-design surface), slices ptc5a–f:**
  - **ptc5a (MED, server, SERIAL):** extend the both-role ongoing-battle guard (`guards::reject_if_in_battle`, already on evolve/fuse) to `care`/`train` (raising.rs:68,133) — closes a bounded mid-battle HP-laundering path (mid-battle `train` → live-EV max-HP folded into level-up heal at battle.rs:1170/xp.rs:97) that **ADR-0122 §D7 explicitly claims is impossible**; ADR amends §D7.
  - **ptc5b (MED, server, SERIAL):** resolve the caller's Ongoing WILD battle on disconnect + GC `battle_wild` (forfeit_on_disconnect excludes WILD @ pvp.rs:576,588; no reaper covers wild rows → returning player soft-locked by is_in_ongoing_battle + row leak). Playtest-facing.
  - **ptc5c (MED, client main.ts, SERIAL vs pt-c2/pt-c2b):** KeyB/KeyI/KeyE omit dialogue/questLog/heal from their overlay guards (main.ts:487/506/525) → overlays stack; fix + optional overlay registry (Decision B).
  - **ptc5d (MED/LOW, tooling/evals — disjoint):** okf-export `collectRsFiles` has no `*_tests.rs` filter → knowledge bundle counts fixture reducers (49 vs 42) and clobbers set_profile_name.md/playtest_reaper.md with test-file sources; conformance gate blind. Also RT-M14.5A-02 vacuous `return;` (redteam_m14_5a_tests.rs:451) + mutate-server wiring ceiling `cap>340` vs committed 299 (nightly-smoke-wiring.eval.mjs:318).
  - **ptc5e (LOW):** CARE_BOND_AMOUNT → game-core SSOT; heal_location stale-reap; isPvpBattle dedup; stale resetCharacters comment.
  - **ptc5f (LOW, docs):** CHANGELOG regen (missing ADR-0128–0133) + PLAN §Status advance.
- **§3 decisions for Drew** (record choice in slice ADRs): (A) ADR-0090 synthetic burst-spread inert at STEP_MS=200 — fix vs annotate; (B) overlay registry now vs cheap patch; (C) CARE_COOLDOWN_MS to content vs shell literal; (D) client coverage exclusions; (E) warp-path predictor epoch guard (Low-Med, DEFERRED by default — not in the slices).
- **Build-order suggestion:** ptc5a→ptc5b (highest ROI, playtest-facing) ‖ ptc5d (disjoint); ptc5c after M-playtest-c client work; ptc5e/ptc5f last. ptc5a/ptc5b pair cleanly with M-playtest-d content. **Runner retains best-judgement on chaining/interleaving; this milestone does not reorder the PLAYTEST GATE beyond preceding M-playtest-d.** adr_next_free was 0135 at review time — verify before reserving.

### 2026-07-20T09:2xZ — ADDENDUM: M-playtest-c.5 §3 decisions A/B/C RESOLVED (Drew delegated)
- **A (ADR-0090 burst-spread inert):** RESOLVED → option (b). Amend ADR-0090 (mechanism is small-step-only; ring-buffer depth carries production) + pin the `stepMs < 2·BURST_EPSILON_MS` reachability bound in a code comment + a small test asserting the branch is unreachable at STEP_MS=200. Do NOT retune netcode pre-gate (anti-YAGNI on a determinism-sensitive path; option (a) kept as a named exception if playtest shows burst pops). Fold into ptc5f/ptc5e.
- **B (overlay-guard):** RESOLVED → ptc5c-1 patch + ptc5c-3 source-scan gate ship NOW (gate is the non-negotiable primary deliverable, mechanical-enforcement-first); ptc5c-2 registry adopted but SEQUENCED after pt-c2b help overlay lands (SERIAL main.ts), or opportunistic/post-gate if pt-c2b slips.
- **C (CARE_COOLDOWN_MS):** RESOLVED → move to **game-core `pub const`** (NOT content, NOT shell literal), folded into ptc5e-1 alongside CARE_BOND_AMOUNT. Precedent: global time/rate magnitudes are game-core SSOT here (CHALLENGE_TTL_MS, RECRUIT_BASE_RATE, EV caps, MAX_BALANCE); content-driven cooldowns exist only as per-entity data (heal_location_row.cooldown_ms). Check stays in shell via a pure `is_care_ready` evaluator mirroring `is_challenge_stale`.
- D (coverage exclusions) and E (warp predictor epoch) remain OPEN/deferred by default per §3.

## 2026-07-20T12:32Z — TICK COMPLETE — pt-c2 MERGED (PR #228) + ADR-0134 index reconciled (PR #229) — mr-sup-cowork-20260720T121459Z-444499-27800
- **pt-c2 MERGED:** PR #228 squash-merged → master **97decf1** (regular CI: ci pass + e2e pass on the PR; post-merge master CI 97decf1 = success). Worktree `.claude/worktrees/pt-c2` + branch `feat/pt-c2-trade-propose` removed. cost **$28.21**, attempts **1**.
- **ADR-0134 index chore:** doc-only PR #229 (`chore/pt-c2-adr-index`, README 0134 row + Next-free→0135) merged → master **3a9cde4** (CI in_progress at tick end; doc-only). master HEAD = **3a9cde4**.
- **Audits:** orchestration **CLEAN** (opus-4-8; tester/reviewer/red-team/verifier/planner/doc-keeper all invoked). gating-test **CLEAN** (RED→merged diff: no skip/only/ignore added, no assertions/tests removed, structural set untouched). Pre-merge scope: touches slightly under-declared (`main.wiring.test.ts`, `docs/specs/pt-c2-plan.md` beyond declared set) — serial/no-sibling merge, verifier-sanctioned, structural set clean → merged (not parked).
- **⚠ Nightly RED (non-gating, pre-existing):** Nightly run 29734207854 on **0421f2c** (ran 10:11Z, pre-dates pt-c2) — `mutation-server` job FAILED; `mutation`/`coverage`/`smoke-republish` PASSED. 0421f2c was doc-only (ADR-0133 index chore) → surviving-mutant/infra flake per ADR-0118 nightly-triage lineage, NOT caused by pt-c2. Regular CI (the merge gate) is green. Recommend next tick confirm it's flake vs. a real server surviving-mutant.
- **NO composite launch this tick** (deferred). Reasons: handoff was >40KB (archived this tick); **M-playtest-c.5 was inserted** into PLAN §9 between M-playtest-c and M-playtest-d (by the 2026-07-20 weekly review @ 0421f2c) — ptc5a..ptc5f pre-gate review residuals — so the next-eligible-slice pick needs a clean queue re-derive rather than a rushed merge→launch.
- **NEXT (supervisor):** M-playtest-c is NOT closed — remaining = **pt-c2b** (item-offer rows [RLS-limited] + in-client help overlay + `docs/PLAYTEST.md`; client-only, main.ts SERIAL, no fan-out). Then M-playtest-c.5 (ptc5a..f residuals; some disjoint/fan-out-friendly) → M-playtest-d (content, ADR-0057, fan-out-friendly) → **PLAYTEST GATE** (do NOT start M18+, blocked:playtest-gate). ADR next-free = **0135**.

## 2026-07-20T14:17:49Z — SUPERVISOR LAUNCH pt-c2b (M-playtest-c residual)
- run_id: mr-sup-cowork-20260720T141749Z-463327-25370
- Launching rooted headless run for slice **pt-c2b**: item-offer rows (RLS-limited) + in-client help overlay + docs/PLAYTEST.md. Client-only, main.ts SERIAL, no fan-out. ADR-0135 reserved (adr_next_free->136).
- Base: master 3a9cde4 (regular CI GREEN). Model: opus. Spec: M-playtest-c-ux-completion.spec.md.
- CONFIRMED launched: leader pid 464760, claude pid 464762, model claude-opus-4-8, log /tmp/mr_pass_pt-c2b.log (streaming). Chain-owner mutex released. Next tick: fast-path standdown while alive.

## 2026-07-20T16:13:24Z — mr-sup-cowork-20260720T160613Z-530949-10912 — RESUME-LAUNCH pt-c2b (fix red CI)
Tick action: RESUME pt-c2b. Prior run (leader 464760, ATTEMPTS=3, EXIT=0) opened PR #230 (`feat/pt-c2b-help-overlay`, MERGEABLE) but remote **ci is RED**: Semgrep SAST 2 blocking findings, both false-positives on the slice's own adversarial XSS proof-of-teeth test `client/src/ui/helpView.test.ts` (~L248 `querySelector('script')`, ~L255 `textContent?.includes(XSS)`, rule `unknown-value-with-script-tag`). e2e green; master 3a9cde4 CI green. Supervisor does NOT fix code → relaunching rooted run to suppress the 2 FPs via inline `// nosemgrep` WITHOUT weakening the test, run full `just ci` green, push → PR #230 re-runs CI. No merge this tick. Nightly still RED @0421f2c (non-gating known flake). run_id=mr-sup-cowork-20260720T160613Z-530949-10912.

---
## 2026-07-20T18:20Z — tick mr-sup-cowork-20260720T180546Z-562866-19136 — MERGED pt-c2b (M-playtest-c CLOSED)
- **Action:** Merged PR#230 `feat(pt-c2b): in-client help overlay (?) + docs/PLAYTEST.md (ADR-0135)` — squash → master `edeb7c4`. Prior tick's rooted run (leader 532998) resumed to fix red Semgrep CI (2 XSS false-positives on the slice's own adversarial proof-of-teeth `client/src/ui/helpView.test.ts`); it exited EXIT=0/ATTEMPTS=1, PR#230 came back CLEAN/MERGEABLE with ci+e2e SUCCESS.
- **Audits (both CLEAN, pre-merge):**
  - Orchestration: model opus-4-8; subagent roles present — tester(7), reviewer(44), verifier(35), red-team(132). Not flagged; no extra review pass needed.
  - Gating-test integrity: XSS teeth INTACT — `querySelector('script').toBeNull()`, `textContent.includes(XSS).toBe(true)`, `<img onerror>` firewall all preserved. Fix added ONLY two `// nosemgrep: ...unknown-value-with-script-tag` inline comments (L250,L260) with false-positive justifications. No skip/only/xit/todo/removed-assertion anywhere across helpView/helpModel/main.wiring test files.
- **Doc-aggregation:** ADR-0135 DIGEST rows already in the slice PR; README "Next free number" still read 0135 → reconciled via doc-only chore **PR#231** (`chore/pt-c2b-adr-index`, 0135→0136), auto-squash-merged → master `ba07d0e`. `adr_next_free` stays 136.
- **master CI after:** GREEN — `edeb7c4` CI completed SUCCESS. `ba07d0e` (doc-only chore) CI in_progress, expected green.
- **Cleanup:** worktree `.claude/worktrees/pt-c2b` removed, local+remote branches deleted. No open PRs. Nightly `0421f2c` still RED (known non-gating flake, unchanged).
- **Note on shell churn:** my DC bash shell died once mid-`gh pr merge` (DC session churn). Reconciled from LIVE PR state — merge had in fact completed (mergedAt 18:11:43Z). No split-brain; single supervisor chain throughout (mutex held).
- **NOT launched (deferred to next tick):** M-playtest-c is now CLOSED (pt-c2b was its last slice). Next up = M-playtest-c.5 (ptc5a–f). Did NOT composite-launch: (a) one shell death already this tick → elevated risk, prefer clean handoff; (b) c.5 has 5 open decisions for Drew in replan §3 — next tick must read `specs/monster-realm-v2/playtest-replan-2026-07.md §3` + PLAN §9 to determine which ptc5 slices are launchable vs blocked-on-decision before selecting.
- **consecutive_standdowns:** 0 (a launch/merge tick).

## 2026-07-20T20:27:59Z — SUPERVISOR LAUNCH ptc5a (M-playtest-c.5 pre-gate residual)
- run_id: mr-sup-cowork-20260720T202518Z-582870-14678
- Launching rooted headless run for slice **ptc5a**: extend both-role Ongoing-battle guard to care/train (reuse guards::reject_if_in_battle SSOT; close mid-battle HP-laundering; ADR-0136 amends ADR-0122 §D7). Server-only, SERIAL. Base master ba07d0e (CI GREEN). Model opus. ADR-0136 reserved (adr_next_free->137).
- Decisions: A/B/C RESOLVED, D/E deferred+out-of-slice → NO ptc5 slice decision-blocked. Launched ptc5a ALONE (highest-ROI serial); deferred ptc5d fan-out to next tick (cleaner after ptc5a merges — ptc5d regen absorbs care/train reducer changes, avoids docs/knowledge overlap).
- CONFIRMED launched: leader pid **585443** (own session — detached, SIGHUP-safe), claude pid 585450, model **claude-opus-4-8** (Opus-class asserted from log init). Log /tmp/mr_pass_ptc5a.log streaming. Per-run lock written. Chain-owner mutex released. **Next tick: fast-path standdown while 585443 alive + no .done.** adr_next_free->137.

## 2026-07-20T22:23Z — tick (mr-sup-cowork-20260720T220717Z) — MERGED ptc5a (PR#232) + LAUNCHED ptc5d
- MERGED ptc5a PR#232 (ADR-0136, care/train both-role ongoing-battle guard) squash → master 53069b0; CI GREEN. Audits: orchestration CLEAN (opus-4-8; planner/red-team/review-lens/reviewer/tester/verifier all present), gating-test integrity CLEAN (guard coverage expanded 4→6 reducers, +12 assertions, no skip/ignore/removed teeth). Minor: 2 knowledge-doc files (heal_party.md, train.md, 1 line each) outside declared touches — benign, no in-flight sibling.
- ADR index reconciled: chore PR#233 (README next-free 0136→0137), --auto squash enabled (lands on green).
- Worktree/branch ptc5a removed; main checkout on master 53069b0.
- COMPOSITE LAUNCH ptc5d (knowledge-bundle *_tests.rs exclusion + 2 degraded test teeth; ADR-0137 reserved). Decision-unblocked (Decisions D/E are client-coverage/netcode, deferred, NOT ptc5d; A/B/C resolved→ptc5c/e/f). Tooling/eval slice, disjoint. See mr-state.json for details.

## 2026-07-21T02:11:16Z — GOVERNANCE (Drew-directed, interactive): folded in the remaining open decisions
- **NOT a tick / no code, no merge, no mutex.** Drew delegated resolution of all remaining open cross-milestone decisions; recorded the resolutions in the authoritative harness specs + PLAN §9 (harness-side only; project repo untouched). The implementations flow through the normal slice loop with full DoD/proof-of-teeth — supervisor does not implement.
- **Audited every milestone's decisions.** All prior review-milestone decisions (M10.5/M12.5/M13.5/M14.5/M16.5/M17.5, M8.9, M-infra-d) were already resolved. Four remained open; now resolved (Drew-delegated 2026-07-20):
  1. **M-playtest-c.5 Decision D (client coverage exclusions)** → keep exclusions for the gate; booked post-gate slice **M-postgate-client-coverage** + a pre-gate mechanical fence (fail if vite.config exclusion set grows). Recorded in c.5 §3 + PLAN Phase D.
  2. **M-playtest-c.5 Decision E (warp predictor epoch)** → DEFER fix; **ptc5f** now also carries an ADR-0085 amend (accept-risk for closed playtest) + a reachability-pin test; fix booked post-gate as **M-postgate-netcode-hardening**. Recorded in c.5 §3/§4 + PLAN Phase D.
  3. **M10.5 D-render-snap (position-divergence snap)** → FIX pre-gate (its "until warps exist" trigger fired at M11; verified renderResolver.ts on master 53069b0 still lacks a position-divergence branch). Booked as **new slice ptc5g** in M-playtest-c.5 §2 (client render, disjoint from main.ts). Recorded in M10.5 §5 + c.5 §2 + PLAN c.5 entry.
  4. **M8.9 §6 game-core inline-test extraction** → CLOSED: bless inline tests, adopt sibling _tests.rs as a forward convention; no big-bang extraction (anti-churn, anti-YAGNI). Recorded in M8.9 §6.
- **Files edited (harness):** M-playtest-c.5-pregate-review-residuals.spec.md (D/E resolved + ptc5g slice), M10.5-fifth-review-residuals.spec.md (D-render-snap resolved), M8.9-server-module-modularization.spec.md (inline-test decision closed), PLAN.md (c.5 entry note + 2 post-gate slices). No .rs/.ts/ADR files touched.
- **⚠ ptc5d PR #234 (feat/ptc5d-okf-tests-filter, ADR-0137) is OPEN and awaiting the supervisor merge tick** — untouched here; next tick should merge per the normal audit/merge protocol. master = 53069b0 (CI GREEN); ptc5a already merged (M-playtest-c.5 slice 1 done).
- **Note:** these harness writes will trip the next cron tick's active-session probe (recent-writes standdown) for ~6 min — expected/benign; the tick after resumes and can merge ptc5d.
- **Runner impact:** M-playtest-c.5 now has slices ptc5a(merged)/b/c/d(PR#234)/e/f/g; ptc5f scope grew (ADR-0085 amend + reachability pin for Decision E, alongside the existing ADR-0090 Decision-A amend). Two new post-gate slices exist in PLAN Phase D.

---
## 2026-07-21T12:14Z — tick mr-sup-cowork-20260721T120204Z-796163-27758 — MERGED ptc5d (PR#234, ADR-0137)
**Action:** single mutating action = merge ptc5d. Fast-path: both ptc5a & ptc5d runs DEAD with clean `.done` (EXIT=0 ATTEMPTS=1); no live chain → proceeded to gates. Gate1 clear (rate_limit_resets_at null). Gate2: active-session probe CLEAR (no IDE claude, no <6min writes); fetched both repos.
**Ground truth:** ptc5a fully landed last session (master 1756f2c; chore PR#233 next-free 0136->0137 merged). ptc5d finished → opened **PR#234** "exclude _tests.rs from OKF bundle + restore two degraded gate teeth (ADR-0137)", CLEAN/MERGEABLE, ci+e2e PASS.
**Audits pre-merge:** model=opus (claude-opus-4-8) ✓; subagents tester+reviewer+red-team+verifier all present → **orchestration CLEAN**; **gating-test integrity CLEAN** — RT-M14.5A-02's `if !status_applied { …return; }` silent vacuous-pass **replaced with hard `assert!`** (tooth RESTORED, ptc5d-3 D3); nightly-smoke-wiring `L-recap` flipped accepted→rejected under tightened 299 ceiling + new `L-overcap`(300 reject) + positive-control(299 accept) ADDED (D4); okf-export.mjs excludes `*_tests.rs` (D1). No skip/only/ignore/deleted teeth. Pre-merge assert: code/test/eval diff ⊆ declared touches, no structural-set files, no in-flight siblings.
**Merge:** PR#234 squash-merged 12:02:38Z → **mergeCommit 102d3b7**, remote branch auto-deleted. `master` **CI 102d3b7 SUCCESS (GREEN)**. Nightly @1756f2c FAILURE = known non-gating mutation-server flake (unchanged).
**Cleanup:** removed ptc5d worktree (.claude/worktrees/ptc5d) + local branch + per-run locks (ptc5d & stale ptc5a). Strays (.claire/, docs/memory-cards/) left untouched.
**ADR index:** ptc5d consumed ADR-0137 → chore **PR#235** bumps `docs/adr/README.md` next-free 0137→0138 (DIGEST 0137 rows already landed in PR#234; README table follows precedent of no per-slice row, cf. 0135/0136). `gh pr merge --auto` REJECTED again (GraphQL "Protected branch rules not configured…enablePullRequestAutoMerge") → **detached merge-on-green job dispatched** (survives shell churn); PR#235 CI pending at record time. `adr_next_free`→138.
**NOTE (shell instability this tick):** DC bash session churned/died repeatedly on foreground network ops (git fetch / gh merge) — worked around by backgrounding all network calls to log+done-flag artifacts (setsid for merges) and polling. The ptc5d merge itself completed (RC=0, confirmed via live PR state = MERGED) despite a session death mid-`gh pr merge`.
**Cost:** ptc5d ≈ $23.95 (final result event). **No composite launch** this tick: shell instability = "any doubt → let next tick launch" (conservative).
**NEXT TICK:** verify PR#235 merged (detached job) — if still open, merge on green manually; reconcile if needed. Then pick next M-playtest-c.5 slice per PLAN §9 / replan §3 OPEN DECISIONS: remaining ptc5b (server SERIAL) · ptc5c (client main.ts SERIAL) · ptc5e (small, Decision C folded) · ptc5f (docs+ADR-0090 Decision A amend). replan §3 still has OPEN DECISIONS for Drew — read before launching. Then M-playtest-d → PLAYTEST GATE. Nightly mutation-server RED non-gating (confirm flake vs real surviving-mutant when convenient).

**[POST-RECORD UPDATE 12:16Z]** Detached job completed within-tick: PR#235 CI green → squash-merged → **master dc796ee**. ADR index fully reconciled (next-free now 0138). No open chore PRs. Mutex released.

## 2026-07-21T15:13:49Z — mr-sup-cowork-20260721T150018Z-823815-14595 — LAUNCH ptc5b (IN-PROGRESS)
Tick reconciled: master GREEN @dc796ee (CI success), 0 open PRs, no inflight/parked runs; stale ptc5a/ptc5d .done flags cleaned (both already merged, PR#232/#234). All M-playtest-c.5 §3 decisions RESOLVED (PLAN §9 L307); D/E map to post-gate milestones, not pre-gate blockers. First unfinished pre-gate slice in §9 order = **ptc5b** (server wild-battle disconnect resolution + battle_wild GC; unblock returning-player re-entry). SERIAL (server disconnect path); launched ALONE (no approved fan-out pair — only disjoint candidate ptc5g is client, and server-module‖client is not an approved pair shape; shell already churned once this tick). ADR-0138 reserved (adr_next_free 138->139). Decision default in brief: auto-flee write-back on disconnect (record in ADR). Launching via mr-launch.sh under setsid, --model opus.

LAUNCH CONFIRMED 2026-07-21T15:16:37Z: ptc5b detached+asserted — session-leader pid 826946 (mr-launch.sh), claude child 826953 (--model opus, stream-json), brief passed intact. Per-run lock .harness-runner.ptc5b.lock records the leader. Chain-owner mutex released; next tick polls the live leader, merges when the PR opens green. Decision default: auto-flee write-back (ADR-0138 to record). — mr-sup-cowork-20260721T150018Z-823815-14595

## 2026-07-21T~16:1xZ — ptc5b TERMINAL: PR #236 OPEN, local full `just ci` GREEN — supervisor to merge
- Branch `feat/ptc5b-wild-disconnect-gc` @ **2eec417** (pushed; commits to squash: plan+ADR `49794b0`, green impl+teeth `1c13a66`, docs `2354db3`, review-fixes `2eec417`). PR **https://github.com/mdrewt/monster-realm/pull/236** (base master). **Local full `just ci` EXIT=0 (×3, incl. verifier's run):** fmt + clippy `-D warnings` + **1364 rust nextest / 0 skipped** + **68 evals** + check-secrets + wasm-pack + client tsc + **1323 vitest / 52 files**. **DID NOT wait on remote CI / did not merge — supervisor owns it.**
- **Delivered (ADR-0138; EARS ptc5b-1..3):** `on_disconnect` (lib.rs) → new `battle::resolve_wild_battle_on_disconnect(ctx, me)` after `pvp::forfeit_on_disconnect`. battle.rs: SSOT predicate `is_ongoing_wild_battle(b,player)` + resolver (collect-then-mutate caller's Ongoing WILD ids → set `Fled` → `write_back_battle_results` log-and-continue → delete `battle_wild` → delete `battle`; **NO `update()`** — deleting, not mark-Fled). **Decision (ADR §D3): auto-flee PERSISTS damaged HP via the flee write-back SSOT; the spec's "restores pre-battle HP" hint was REJECTED (a full restore = "disconnect-to-heal" exploit, disconnect would dominate flee).** DELETE not mark-Fled (no client frame on disconnect + leak/stale-overlay); belt-and-suspenders `battle_wild` delete survives an early write-back `Err`. 4 proof-of-teeth (selection, re-entry-flip soft-lock + mutation, body scan, wiring scan) — all verified biting (red-team + verifier mutation spot-checks; tree clean after reverts).
- **Orchestration (Opus orch):** planner (ACCEPT + 2 adjustments: SSOT predicate not shadow-copy, delete-don't-update) → ADR-0138 + plan checkpoint → **tester** (separate agent) RED teeth (fixed 1 unsatisfiable `battle_wild(` needle + clippy useless_vec) → orchestrator implemented (did NOT edit teeth) → **reviewer + red-team + review-lens(reducer-security)** parallel (all clean; reviewer 3 minor → M-1 needle-anchor/M-2 stale-comment/L-3 order-note ALL addressed; red-team 6/6 DEFENDED; reducer-security 7/7 PASS) → **verifier PASS** (gating diff additive-only, no weakening; 3 mutants killed right teeth; full ci EXIT=0). NOTE: `reducer-security-auditor`/`desync-guard` agent-types absent → used `review-lens` w/ reducer-security prompt; desync-guard moot (no netcode/movement surface).
- **touches-delta (in PR body):** all sanctioned companions — sibling `battle_tests.rs`/`pvp_tests.rs`, `docs/adr/0138-*.md`, `docs/adr/DIGEST.md` (`just adr-digest`), `ARCHITECTURE.md` (M-playtest-c.5 section). **`pvp.rs` NOT touched** (in declared set but only its sibling test needed). NOT touched: guards.rs, evals/**, client/**, game-core/**, schema, module_bindings, Cargo.lock, CHANGELOG (git-cliff), adr/README (supervisor), docs/knowledge/** (`just knowledge` no-op — 42 reducers/34 tables unchanged).
- **Risk (documented, non-blocking):** red-team MEDIUM latent — write-back `Err` path skips HP write-back (free heal in that error case) but prerequisite (mid-battle ownership change) is blocked by escrow guards TR-11/ME-1 → unreachable by normal play; ADR-0138 D4 records it.
- **HARNESS bookkeeping done (supervisor to commit):** spec `M-playtest-c.5-...spec.md` ptc5b **DELIVERED note** added; memory card `monster-realm-ptc5b.md` (auto-memory dir) + MEMORY.md pointer written.
- **ADR next-free = 0139** (ptc5b used 0138). Worktree `.claude/worktrees/ptc5b` + branch removable after merge. Graph reindex AFTER merge (main checkout still master).
- **NEXT (supervisor):** poll remote CI on #236 → squash-merge → ADR-0138 index chore (README next-free 0138→0139). Then remaining M-playtest-c.5: **ptc5c** (overlay mutual-excl, client main.ts SERIAL) · **ptc5e** (SSOT/content/dedup) · **ptc5f** (ledger recon + ADR-0090/0085 amends) · **ptc5g** (render-snap, client, disjoint) → **M-playtest-d** (content) → **PLAYTEST GATE** (do NOT start M18+).

## 2026-07-21T15:41:38Z — RESOURCE-LOCATIONS NOTE (Drew-requested)
Recorded a durable `resource_locations` map in mr-state.json (+ a queue NOTE) after a tick spent discovery time locating the specs. Key facts for future ticks:
- **Specs are HARNESS-level, not project-level:** `$HARNESS/specs/monster-realm-v2/` (= /home/mdrewt/projects/ai-apps/claude-harness/specs/monster-realm-v2/). Holds PLAN.md, playtest-replan-2026-07.md, and per-milestone `M*.spec.md`. The task "Facts" cite `specs/monster-realm-v2/PLAN.md §9` with no absolute anchor — it is NOT under `$PROJ` (`$PROJ/specs/` does not exist).
- **Per-slice `touches:`/EARS/decision-hooks live in the milestone `M*.spec.md`, not PLAN.** PLAN §9 is a one-line-per-slice summary + the resolved-decisions status line. For ptc5* that file is `M-playtest-c.5-pregate-review-residuals.spec.md`.
- **Two different "§3"s:** the M17.5 replan §3 (D-17.5-A..E) vs the M-playtest-c.5 spec's own §3 (Decisions A-E, all RESOLVED 2026-07-20). State/queue mentions of "replan §3 open decisions" meant the c.5 spec's §3.
- **ADR two-location + offset trap** (already in project AGENTS.md, mirrored into resource_locations): impl ADRs 0035+ in `$PROJ/docs/adr/`; design ADRs 0002-0034 in `$HARNESS/specs/monster-realm-v2/adr/`; harness adr/0055-0057 == project 0056/0057/0080.
Everything else (/home/mdrewt/projects/ai-apps/claude-harness/memory/projects template/launch-script/stores, AGENTS.md) was exactly where the task said. — post-launch addendum

## 2026-07-21T15:46:07Z — SHELL-RESILIENCE GOTCHA (Drew-requested, empirical)
The supervisor DC/WSL shell died mid-tick on a ~80s in-process `sleep` poll loop (NOT a network op — the existing WATCH only covered network). Recovered cleanly because run_id was persisted to /tmp/mr_runid.txt and the sync ran as a setsid log+.done job. Durable solutions added to mr-state queue: keep interact calls <~30s + poll by repeated short calls; make every step artifact-recoverable; persist run_id/tick-vars to /tmp; on shell death re-stamp the chain-owner lock heartbeat from the fresh shell (liveness = heartbeat/TTL + rooted-run leader, never the supervisor shell pid).

## 2026-07-21T16:10:57Z — NATIVE SUPERVISOR CUTOVER (Drew-directed)
Supervisor loop migrated off the Desktop-Commander bridge to NATIVE WSL cron: */30 tick via $MEM/mr-native-tick.sh (mechanical gates: kill-switch, flock, live-chain fast-path, rate-limit, human-activity probe, chain-mutex TTL) -> spawns `claude -p` with $MEM/mr-supervisor-prompt-native.md (sonnet decision layer) ONLY when action may be needed. Rooted runs unchanged (mr-launch.sh, opus). Cowork/DC task demoted to daily FALLBACK with heartbeat-standdown directive (mr-state queue[0]). WSL keepalive: .wslconfig timeouts=-1 (needs WSL restart to apply; do NOT restart while runs live) + Startup-folder boot cmd + cron @reboot tick. Rationale: DC/WSL session churn killed supervisor shells twice on 2026-07-21; native execution removes the whole gotcha class (180s cap, Windows-path file tools, churn). See mr-state.native_supervisor for the full map.
...
## 2026-07-25T21:03Z — nh3 + ux3 launched (composite of prior tick's deferred pick)
Reconciled stale nh4/ux1 lock/.done leftovers from already-merged runs (no re-recording needed — the 20:41Z tick's ledger/handoff already captured both merges; master confirmed green at cfbf59e live). The 20:41Z tick had selected nh3+ux3 as next candidates but hit PROBE-TRIPPED because its own git-merge write fell inside the activity window; re-ran the probe this tick (clear — resident pid idle 18h+, only supervisor-owned files touched in the last 6 min) and launched both. nh3 (fable@xhigh, hard tier, ADR-0152): Predictor epoch/generation guard closing the ADR-0085 accepted-risk window (stale-seq rejection post-warp-rebuild evicting a legitimate new op). ux3 (opus@high, routine tier, ADR-0153): justfile preflight-check for $STDB_SERVER reachability before playtest-up/wipe. Disjoint touches confirmed. adr_next_free 152->154.

## 2026-07-25T21:05:42Z — General-improvement round (post-boyscout): 2 bugs fixed, 2 watch-items logged
Brainstorm+verify round beyond boyscout scope. FIXED: (1) tick PATH lacked /home/mdrewt/bin — the asdf BINARY the gh shim execs; retro-4 fix was incomplete (shims without their interpreter), cold-tick reproducible, now verified end-to-end under exact tick PATH. (2) terminal_pr_open grep anchored — unanchored slice-id match cross-hit branches (pt-c1~pt-c1b proven; slice B matched any b-containing branch = always-terminal, masking premature stops). Truth-table tested 7/7. WATCH-ITEMS (logged, deliberately not built): gh-auth failure signatures (401/Bad credentials) currently classify as generic real-failure — fix on first incident; /tmp run-artifact + .bak retention (disk creep, revisit monthly). INVESTIGATED, NO CHANGE: resume prompts should NOT carry ollama summaries (confabulation surface, resume re-derives from git cheaply); lens battery composition (ux1 red-team just caught a real no-op-UI bug at routine tier — battery earning keep); supervisor recurring-work scan (doc-merge reconciliation stays LLM judgment at n=1); brief reorder (no evidence current order bites).

## 2026-07-25T21:18:14Z — Fan-out coordination evaluation (Drew-directed, analyst+judge adjudicated): touches: RETAINED on corrected rationale + 2 scoped additions
Full candidate field scored (static touches / dynamic reservation / optimistic / contract-first / module ownership / task-DAG / semantic partitioning / branch-by-abstraction / hybrid). VERDICT: touches: kept — but its load-bearing role is CHEAP EARLY REJECTION + scope-creep assert, NOT collision prevention (worktrees do that); optimistic-with-loud-gates would catch the same file conflicts but only after a full slice is spent. ADDED: (1) decision-space partitioning as a CONDITIONAL fan-out step (trigger: sibling specs share a domain/registry; scoped to the overlap — judge rejected mandatory-every-launch as n=1 overfit); (2) contract-first micro-slices for ALWAYS-SERIAL structural work. SHELVED with written reinstatement triggers (N>=3 sustained / park-rate >20-30% / budget lifted): reservation, DAG, module partitioning, flags. Wall-clock note: judge corrected the analyst — budget caps SPEND not time-to-batch; parallelism still buys calendar time, hence triggers not rejection. Industry cross-check: 2026 consensus (worktree isolation + file decomposition + serial merges + orchestrator arbitration) matches this loop's architecture.

## 2026-07-25T21:24:21Z — N-raise readiness (Drew intent: N up to 4+)
Verdict from the coordination evaluation STANDS (touches + conditional partitioning + contract-first); what changes is timing. DONE NOW: governor gains inflight_committed_est_usd (per-tier medians on live locks; closes the ~$300 committed-spend blind spot at N=4); SSOT N>=3 LAUNCH PROTOCOL (range pre-assignment mandatory in practice, staggered slot-opening, per-slot memory rule, exposure margin, queue-shape signal). FIRST SHELF REVISITS once N>=3 actually runs: mechanical disjointness helper (max-disjoint-set over candidate touches), module-aware spec authoring guidance, 5h-window anchor (needs one Drew %-report during a heavy window). Host physics note: 4 concurrent rust builds on one WSL box will degrade per-slice wall-clock; measure before assuming 4x.

## 2026-07-25T21:28:31Z — Boyscout x N-raise interaction review: 2 clarifiers added, N-limit contradiction reconciled, safeguards audit
INTERFERENCE FOUND+FIXED: (1) boyscout naming scoped to FILE-LOCAL symbols (exported renames = own slice; at N>=2 a sibling against pre-rename master breaks on second merge); (2) contract-freeze micro-slices are boyscout-EXEMPT (frozen seams must be crisp diffs). RECONCILED per Drew: N_MAX default 2, up to 4 under the N>=3 LAUNCH PROTOCOL (resolves the pre-v3 N<=2-vs-N<=4 SSOT contradiction, flagged by the first plan review and dormant since). SAFEGUARDS AUDITED, NOT RELAXED (with reasons): content-parity/fail-loud gates stay despite range pre-assignment (prevention never retires detection - the gates are the net for when pre-assignment fails and serve single-slice correctness); boyscout reviewer truth-check stays until the 10-slice measurement; governor thresholds stay (in-flight term makes 90% MORE accurate, not over-conservative); same-signature dead-stop + MAX_ATTEMPTS + kill-switch envs + chore batching option all stay (each guards a distinct class; batching is now optional-but-free).

---
## 2026-07-25 — nh3 PARKED at plan checkpoint: touch-set blocker (hidden dependency on main.ts) + a half-fix finding the supervisor must rule on

- **Branch `feat/nh3-predictor-epoch-guard` @ `79956bf`** (pushed; worktree `.claude/worktrees/nh3` from master `cfbf59e`). No PR, no code — the commit is the REVIEWED plan (`docs/specs/nh3-plan.md` v2; planner → reviewer+red-team opus lenses, findings folded in). Full resume memo: `memory/projects/monster-realm-nh3-progress.md`.
- **BLOCKER 1 — hidden dependency, per the fan-out rule:** the launch order's own slice text ("a rejection .catch compares its captured epoch") names code that lives in `main.ts:453-472`, but declared touches were predictor.ts + sibling test + ADR-0085 only. Verified (lenses concurring): no standards-compliant predictor-only fix exists — the discriminator (which instance issued the rejected op) exists only at send time in `sendIntent`'s closure. **Re-serialize with touches += `client/src/main.ts`, `client/src/main.wiring.test.ts`, `client/src/prediction/heldKeys.test.ts`** (per-file justification in plan §0). ux3 disjoint — safe to relaunch immediately.
- **BLOCKER 2 — scope ruling needed (F2, found independently by both lenses):** the spec'd epoch guard closes only HALF the ADR-0085 accepted-risk window (Case M1: stale rejection evicts new legit op — guard fixes; Case M2: the new op's OWN "stale seq" rejection, the spec's quoted solo trigger, dominant post-nh2 — guard can't fix; needs a ~3-line `lastSentSeq` seedSeq-floor in main.ts eliminating the seq collision itself). **Recommended: guard+floor** so nh3-3's "window closes" is true; guard-only requires the ADR to say "half-closed, M2 residual".
- **NEW SLICE CANDIDATE (`nh5`, feel defect, deterministic, unrecorded):** `resetPredictionState()` → `held.clear()` (`main.ts:287`) = walking through a doorway while holding a key stops you dead until release+re-press (red-team F6). Arguably bigger playtest-feel impact than nh3. Also: any future `nh3-e2e` must script `warp → release → re-press` (hold-through-warp is vacuous-green).
- ADR-152 still reserved for nh3 (ux3 holds 153); no ADR file authored yet (deliberate — Decision text depends on the scope ruling; plan §2 has the full outline + alternatives incl. D6).
- Orchestration this session: planner ×1, reviewer(opus) ×1, red-team(opus) ×1 — all read-only; tester NOT invoked (no tests written; slice parked pre-tester per the hidden-dependency stop). Main checkout untouched (no mutations); worktree only.

## 2026-07-25T21:47:02Z — nh3 relaunched same tick: touch-set widened + scope decision defaulted
Parked nh3 run (fable, $17.73, 29 turns) reconciled: PARKED at plan checkpoint on two findings, no code written, no PR. Live-verified: no open PR for nh3, branch feat/nh3-predictor-epoch-guard@79956bf pushed (plan v2 commit only), worktree present, ux3 unaffected (disjoint touches). Active-session probe clear (no resident IDE pid, only supervisor-owned file writes in the last 6 min).

BLOCKER 1 (touch-set) and BLOCKER 2 (scope ruling) were both judged supervisor-defaultable under the 2026-07-25 BLOCKER discipline (reversible, non-architectural/non-security choices; Drew can reverse via follow-up):
- Widened touches: predictor.ts (+sibling test) -> += client/src/main.ts, client/src/main.wiring.test.ts, client/src/prediction/heldKeys.test.ts. Confirmed disjoint from ux3 in-flight (justfile/docs/playtest-ops.md/evals/playtest-verify.eval.mjs).
- Scope decision-defaulted: guard+floor (not guard-only) -- both lenses independently recommended this so ADR-0085's "window closes" claim is actually true (guard alone leaves Case M2, the dominant post-nh2 trigger, unaddressed). guard-only would need the ADR to admit a half-closed window instead.

Relaunched nh3 (fable@xhigh, hard tier, ADR-0152 still reserved) via mr-spawn resuming from the committed plan checkpoint -- tester phase first, no re-planning. ADR-152/153 unchanged (nh3=152, ux3=153). park_counters.nh3=1 (not yet blocked: threshold is ~3).

Logged for later, NOT built: nh5 candidate (held.clear() in resetPredictionState() stops held-key movement dead across a warp until release+re-press -- red-team F6, arguably bigger playtest-feel impact than nh3 itself) -- folded into nh3's relaunch brief as a flag-only item.

No merge this tick (nh3 wasn't finished code, ux3 still live) so no composite launch attempted. Fan-out stays at 2 (nh3 + ux3), within N_MAX default.

## 2026-07-25T22:25:24Z — Upstream fan-out doctrine (Drew-directed; analyst+judge adjudicated): 3-state eligibility + slicing-for-fan-out + 2 safeguard relaxations
ELIGIBILITY (supervisor): new mr-disjoint script = MANDATORY mechanical gate (tested: disjoint/overlap/structural/N=4 cases); 3-STATE model: SAFE->parallel, SAFE+shared-axis->partition-then-parallel, SERIAL; supervisor override is SERIALIZE-ONLY (semantic grounds), never toward parallel. All eligibility operates on DECLARED touches — boyscout stays invisible by construction; A3 code-graph idea shelved WITH that as a written acceptance criterion.
SLICING DOCTRINE (build-loop, applied at spec elaboration): partition-first [conditional on shared axes; evidence: pt-d natural experiment — partitioned id-bands clean, unpartitioned affinity axis = the only collision]; contract-first slice-0 [conditional on new seams]; per-slice test/eval files [mandatory]; after: dependency metadata [column now, mechanism later].
RELAXATIONS: adds-ADR DROPPED from fan-out-ineligible (pre-allocation + 0143/0144 parallel proof; A2 still detects filename collisions = detection net intact); shared-docs DOWNGRADED to warn+reconcile-before-milestone-close (full retirement awaits a doc-parallel proof case). KEPT: schema/new-dep serial (real shared state; contract-first consumers), no auto-derived touches (declared sets anchor the scope-creep assert).
NET EFFECT: more slice-pairs become eligible (ADR-adders, doc-touchers) + specs authored to be parallel by construction + eligibility per-pair cost drops from LLM eyeball to script call — all while boyscout scope, audits, and the serialize-only override preserve the quality gates.

## 2026-07-25 — ux3 TERMINAL: PR #253 OPEN, local full `just ci` GREEN, remote CI running — supervisor to merge
- **Branch `feat/ux3-playtest-preflight` @ `85d4f32`** (pushed, 0 unpushed, worktree clean). PR **https://github.com/mdrewt/monster-realm/pull/253** (base master, from `cfbf59e`), OPEN / **MERGEABLE** / UNSTABLE-pending. **DID NOT merge — `gh pr merge` is supervisor-only.**
- **Local full `just ci` EXIT=0 on the tip:** fmt/clippy + biome + **1456 rust nextest / 0 skipped** + **71 evals PASS / 0 FAIL** + check-secrets clean + wasm-pack + tsc + **1438 vitest / 55 files**. Separate **Semgrep `--config auto --error` on both changed source files = 0 findings / 200 rules**. No `new RegExp` (3 occurrences, all prose prohibitions).
- **Delivered (ADR-0153; EARS ux3-1/2/3):** shared `playtest-preflight` recipe called from `playtest-up` + `playtest-wipe` after the DB-name guard, before the first `spacetime build`/`publish`; `spacetime start` step + preflight step in `docs/playtest-ops.md`; gate extension in `evals/playtest-verify.eval.mjs`.
- **⚠⚠ TWO FINDINGS THAT RESHAPED THE SLICE — read before any `$STDB_SERVER` work.**
  (1) **`curl "$STDB_SERVER/v1/ping"` (the spec's own suggestion) FALSE-REJECTS a valid config.** `spacetime publish -s` accepts server NICKNAMES — `spacetime server list` ships `local` and `maincloud` — so `STDB_SERVER=local` publishes fine while a URL-constructing curl probe fails DNS and confidently misdiagnoses a healthy server. Switched to `spacetime server ping`, which shares `publish -s`'s resolution path. Generalizes: **a guard must resolve its target through the same path as the thing it guards.**
  (2) **`spacetime server ping` exits 0 for ANY completed HTTP round-trip.** Measured on 2.6.0: trailing slash and path suffix → `Server returned 404` at **exit 0**; an unrelated service on the port → `Server could not be reached (500 …)` at **exit 0**; only a real hit prints `Server is online:`. `publish -s` rejects all three. So shared resolution was NOT sufficient and the first implementation shipped this bug — found by the final red-team pass, not by design. The probe now matches the OUTPUT line and echoes the CLI's own last line so the message names the real cause. Also: **`timeout 0` DISABLES the timeout** in GNU coreutils, and `server ping` has no timeout of its own.
- **THE ORCHESTRATION LESSON — behavioral teeth beat source scans.** The first, source-scan-only gate draft let **9 of 19** functionally-broken implementations pass GREEN, two proven to exit 0 against a dead server (body wrapped in `if false`, dead shell function, exit captured-then-ignored, `set +e` at the call site, a `just() { :; }` shadow, call parked in a dead `if` branch). No scan reaches that class. Four behavioral teeth fixed it; **final probe 13/13 killed**, incl. a regression mutant for finding (2). Teeth: negative (dead port) · positive control (live stub — without it "always fails" is a passing impl) · non-SpacetimeDB responder (500 stub) · **call-site** (runs both callers with a fake `spacetime` first on PATH; the only assertion gating that CALLERS honor a guard rather than merely containing the line — the `just() { :; }` shadow also silently disables the pt-a2 dev-reducer honesty gates while their exact-step scan stays green).
- **Anti-vacuity traps hit (all real, all fixed):** a **missing** recipe also exits 1, so `status !== 0` alone passes on a deleted recipe · `just` echoes the source lines of a **non-shebang** recipe (shebang recipes do NOT — verified), so a remediation string can be supplied by a line that never executed → assert the runtime-EXPANDED value · `extractRecipeBody` strips only FULL-LINE `#` comments, so every scan must also run `stripJustfileComments`, **extract-then-strip** · the first docs needle (`spacetime start`) was **already green before any change** (playtest-ops.md:131, nh4) · a count-floor of 2 was ALSO defeated because ux3 itself added a second prose mention → only a fence-aware assertion gates a *runnable* step · **a predicate + 5 fixtures shipped without being wired to the real file** (caught only by the mutation probe — the fixtures were green, the deliverable ungated).
- **Verifier FAILED the first pass** on a real blocker my own CI check had masked: I read the background wrapper's exit code, not `CI_EXIT` in the log — `just lint` was red on a biome format error in the eval. Fixed; **always grep the log for the recipe's own exit code, never trust the wrapper's.**
- **touches-delta (in PR body):** `docs/adr/0153-*` + `DIGEST.md` (`just adr-digest`), `ARCHITECTURE.md` (one paragraph; next-free → **0154**), `docs/specs/ux3-plan.md`. **7 files.** NOT: `CHANGELOG.md` (git-cliff), `docs/adr/README.md` (supervisor), `evals/run.mjs` (auto-discovery), nothing under `client/`/`server-module/`/`game-core/`/`scripts/`. Zero Rust and zero TS source lines changed → no coverage/mutation surface. `just knowledge` no-op. Fully disjoint from concurrent sibling `nh3`.
- **boyscout-delta:** `docs/playtest-ops.md` wipe section hardcoded `monster-realm-playtest` while the rest of the doc uses `$MR_PLAYTEST_DB`; also now names the DB guard that runs first. (~3 lines / 1 hunk, well under cap.)
- **Disclosed residuals:** `playtest-report` + `playtest-verify-release` reach the same server and keep the opaque failure (outside ux3's EARS — named, not silently omitted) · `spacetime server ping` prints `WARNING: This command is UNSTABLE`; drift fails **closed and loudly**, and the positive control makes failing-open impossible · the order assertion is a line scan, so prose naming `spacetime build`/`publish` inside these bodies can false-RED (documented at the helper) · `hasFencedCommandLine` is a line scan, not CommonMark (` ``` ` inside a fence, or `~~~`, would fool it) · behavioral teeth SKIP when the `spacetime` CLI is absent, per the existing `bindings-drift` convention (CI installs it before `just eval`).
- **Ops notes:** the eval's behavioral teeth add **+95 ms**, loopback-only and offline; `mkdtempSync` stub dir + detached stub server are killed/removed in `finally` (verified no leaks). **A `spawnSync`-blocked event loop means an in-process `createServer` never answers — the positive-control stub MUST be a detached child.** Running the `just() { :; }` mutant locally DOES leave an orphaned `vite preview` (it no-ops `playtest-verify-release`, so playtest-up runs on to `npm run build`) — only ever against a deliberately broken justfile; I killed the one my probe created.
- **ADR next-free = 0154** (ux3 used 0153; 0152 held by concurrent `nh3`).
- **HARNESS bookkeeping written by session (supervisor to commit):** this entry; memory card `memory/projects/monster-realm-ux3.md` + MEMORY.md pointer. Spec §ux3 DELIVERED note NOT yet written — supervisor may want it.
- **NEXT SLICES in `M-postgate-ux-hardening`:** **ux4** (cheap repro-and-confirm of the battle monster-switch gap; spec says run it first among the rest) and **ux2** (owner-scoped wallet `#[view]` — SERIAL, schema-touching). `M-postgate-overlay-registry` remains the strongest-evidence candidate (ux1 shipped a badge that is dishonest while any overlay is open).

## 2026-07-25T23:24:28Z — Code-graph research CONCLUDED (probes + 4-pair replay + judge): gate shelved on evidence, advisory annotation in trial
PROBES: graph healthy (13k nodes/52k edges, 0 drift vs master — step-10 reindex discipline holding; one-shot CLI queries 42ms via resident daemon: codebase-memory-mcp cli <tool> <json>). LIMITS PROVEN: content entities NOT first-class (RON files have only structural edges; build.rs glob embedding invisible) -> registry-value collisions undetectable; WASM/bindings boundary has ZERO edges -> cross-language SAFE verdicts are vacuous. REPLAY (4 real pairs): nh4+ux1 graph-coupled yet parallelized clean (FALSE POSITIVE); pt-d1+d2 graph-SAFE yet semantically collided (FALSE NEGATIVE); A0+nh1, nh2+B true-but-vacuous.
JUDGE (corrected my kill-strength bias): gate = SHELVE with named reactivation criteria (content indexer / bindings indexer / recurring same-language misses), not kill; ADVISORY variant earns its own trial (FP costs one glance, FN no worse than nothing).
IMPLEMENTED: mr-disjoint now emits advisory_coupling_edges per pair (trial; context-only doctrine in SSOT; verified on replayed pairs: nh4+ux1 SAFE+5 edges, content pair None). SALVAGE: is_exported boyscout check BACKLOGGED (feasible pre-merge only by indexing the worktree as its own project, ~heavy; textual heuristic cheaper — revisit if reviewer truth-checks prove insufficient). DROPPED: FILE_CHANGES_WITH seeding (needs runtime traces; ledger mining superior). Graph's proven role stays: within-slice recon/planning reads per the briefs.
TRIAL MEASUREMENT: after ~10 fan-out launches, check whether advisory_coupling_edges annotations correlated with any real outcome or were pure noise -> keep or strip the 15 lines.
