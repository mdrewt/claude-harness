...
**TERMINAL STATE: PR open + local full `just ci` green + remote CI running.** PR https://github.com/mdrewt/monster-realm/pull/461, branch `rb-77` @ 9ee9775, worktree `.claude/worktrees/rb-77`, base origin/master@e06bf13 (rb-76).

WHAT SHIPPED (test-only + two comments; zero production behaviour, zero schema, zero reducer, zero client): `server-module/src/guards_tests.rs` +870 purely additive below the old EOF — `rb77_wiring_verdict(lib_src, guards_src)`, a pure non-short-circuiting label collector over the existing m22-s5/rb-76 squashed view (13 clauses: scan-substrate, mod-unparsed, mod-inline-body, testmod-prefix, mod-not-bare, cfg-not-test, cfg-inner, cfg-macro, include-macro, raw-ident, char-literal-bracket, guards-anchor, mod-alias), a live test over the real `include_str!` sources (+ floors + the D5 note-presence clause), and a 17-fixture matrix (16 rejections by exact label + clean control). `lib.rs:31` trailing reviewer comment (zero line shift → knowledge bundle unchanged); `guards.rs:471-475` five-line note above the cfg(test) hook. ADR-0247 (`**Extends:** ADR-0236`; back-link APPENDED to 0236's existing `Extended-by:` line — a second line is invisible to adr-digest's first-match parser), DIGEST regen, ARCHITECTURE.md one tail entry (rb75 oracle 3/3 green).

KEY FACTS FOR THE MERGE TICK: (1) rb-76 clause (e) ALREADY reds the literal two-line PoC; rb-77 closes six measured open routes (single-line, cfg_attr(path=), alias, nested re-export, macro token tree, in-file include!) plus three artifact-red-team findings that were CI-clean against the first oracle: `cfg! {..}`/`include! {..}` brace delimiters (shipped an unconditional `require_owner` bypass to wasm with 900/900 green), raw identifiers `#[r#cfg]`/`mod r#guards;`, and a kept `'{'` char literal stranding the depth counter. (2) ADR-0224 compliance: no new eval; the fixture-count floor was DROPPED (ratchet class); the two dead assertions deleted. (3) HONEST LIMIT stated in ADR-0247 D3/Consequences: a text scan of TWO files proves what they spell, not what `crate::guards::*` compiles to — the red-team measured a cfg-paired helper in schema.rs called from a wrapper shipping a bypass with every test green. Residuals registered (after the lenses, per the not-upsert rule): R-rb-77-CFGROSTER (crate-wide cfg discipline incl. the real build matrix — nextest DEFAULT features, clippy --all-features, CI's second `--features dev_reducers` wasm with six live cfg sites; game-core build.rs generated source; rb-76's paren-bound raw-only include! ban over the other 20 modules; consumer-side alias) and R-rb-77-CARGOSWAP (`[lib] path` / RUSTFLAGS `--cfg`); rb-78/rb-79 cited, not re-minted.

GATES: 6/6 met, 0 deferred (`Acceptance: 6/6 met, 0 deferred, 0 unmet — rb-77 seed:d757c88c604a5277`). E1 `2 tests run: 2 passed`; X1 `900 tests run: 900 passed, 0 skipped`; X2 LINT-GREEN; X3 DOCS-FRESH adr=true ext=true note=1 arch=1 (its first run FAILED on MY wrong CHECK — it wanted `**Extended-by:** ADR-0247` verbatim; re-pointed at "the Extended-by line contains ADR-0247" — supervisor `verify` must use the patched ledger); X4 CI-GREEN (the single full `just ci`, executed BY `mr-gates check --timeout 2400` from the worktree; evidence at 9ee9775, the tree is unchanged since); X5 MANUAL → `memory/projects/gates/rb-77.red-before.md:62` — 13/13 KILLED, 0 INVALID, BASELINE/C1/C2 CONTROL-GREEN, TREE CLEAN (runner `memory/projects/gates/rb-77.mutants.py`, ran at 6484d18; only the ADR body changed after). Verifier reproduced 3 rows + the note-clause red-before independently and killed the matrix with its own mutant; its ONE survivor (item-boundary half of the testmod-prefix check) became fixture F17 + row M13 before PASS.

ORCHESTRATION: planner (opus) · plan reviewer + plan red-team (opus, parallel; red-team EXECUTED rustc probes) · /simplify inline · tester ×3 (opus; static-only Bash — orchestrator ran every RED/bite proof) · tests reviewer + artifact red-team (opus; EXECUTED in /tmp/rb77-artifact) · specialist (sonnet, comments only) · impl reviewer (opus) · reducer-security-auditor (opus PASS-WITH-NOTES; its X5 stale-criterion catch + build-matrix facts folded) · verifier (opus REJECT→PASS) · doc-keeper ×2 (opus). `desync-guard` NOT run (no game-core/client/wasm/movement surface). `/tmp/mr_warn_rb-77` appeared after the PR opened — landing pattern honoured, no further fan-out.

PROCESS INCIDENTS (memory cards written): the harness `guard-bash` hook rejected TWO whole compound Bash calls because a destructive spelling (recursive-force rm) appeared in them — once as a real snapshot-dir clear chained BEFORE the commit/push/sweep (nothing ran; two lenses were dispatched against a snapshot that did not exist and fell back to the live tree), once merely QUOTED in a memory card's heredoc prose. Snapshot into fresh SHA-named dirs; never quote the spelling through Bash.

NEXT TICK: poll PR 461 `ci` + `e2e`; on green `mr-gates verify --slice rb-77 --budget ≥2400` FROM `.claude/worktrees/rb-77` (X4 is a full `just ci` ≈ 10 min; cargo + node 24 + `~/.local/bin` on PATH; no spacetime server running); `mr-audit` (hard tier); squash-merge as ONE Conventional Commit; `mr-gates residuals close --slice rb-77 --pr 461` (closes R-rb-46-LIBRSMOD); age R-rb-77-CFGROSTER / R-rb-77-CARGOSWAP; reconcile ADR index (0247) / CHANGELOG / ARCHITECTURE; refresh code graphs on the canonical checkout post-merge; delete branch + worktree; scratch in /tmp (rb-77/, rb-77-review-*, rb77-artifact, rb77-verify, rb77-redteam, rb77-sim, rb77-rustc*) is safe to delete.

## 2026-09-11T~02:30Z — rb-73 PR#457 OPEN — disconnect side effects gated on the identity's LAST live connection (private `player_session` via the lifecycle hooks, ADR-0245; closes R-18r-b-DISCONNECTSELF); local `just ci` GREEN (CI-EXIT=0); ledger 9/9 met, 0 deferred; verifier PASS (SUPERVISOR OWNS THE MERGE — and must ADJUDICATE the disclosed scope widening)
**TERMINAL STATE: PR open + local full `just ci` green + remote CI running.**
PR https://github.com/mdrewt/monster-realm/pull/457 (branch `feat/rb-73-disconnect-session-guard`, worktree
`.claude/worktrees/rb-73`, forked from `origin/master@9434dfb`; 7 wip commits, HEAD 67ff868, all pushed, tree
clean). `gh pr merge` NOT run. Main checkout left on `master`, untouched.

**SCOPE WIDENING — supervisor decision required before merge.** Launched touches were `server-module/src/lib.rs`
only; the fix needs a new table and the PR touches ~20 files (schema.rs, accounts.rs, three test files, five
evals, generated bindings, one client fix + four client comments, four ADRs, the driver's AM25 prose, ARCHITECTURE,
docs/knowledge). Sequence: parked twice as a hidden-dependency STOP (verified push + handoff both times); the run
loop auto-resumed both times; no sibling was in flight (worktree list, open PRs, mr-state inflight/queue verified
empty before the first out-of-touches edit); the spec's own touches line is `(inherit from source slice — REVIEW)`
and the ledger's scope text names "server-module schema/reducers". Every file is in the PR body's `touches-delta:`
table with the gate that forced it. If refused, close the PR — nothing reached master. Fan-out-ineligible
(schema change): merge SERIALLY.

**Merge instruction (mechanical).** `mr-gates verify --slice rb-73` FROM `.claude/worktrees/rb-73` with the project
PATH (X9's CHECK re-runs `just ci`, ~2-3 min warm; port 3000 must be free); `mr-audit` orchestration lenses used:
planner/reviewer/red-team/tester(×2)/reducer-security-auditor/desync-guard/verifier + /simplify inline;
gating-test integrity: the tester's pass 2 ADDED an exact-body clause + one wiring tooth, nothing removed
(verifier checked 9b6b3e5..HEAD). Squash title = the PR title. After merge: `mr-gates residuals close --slice rb-73
--pr 457` closes nothing by itself (R-18r-b-DISCONNECTSELF is the promoted source residual — close it as rb-73's
outcome); the six R-rb-73-* rows registered this run stay open (4 backlog, 2 wontfix). ADR next-free: 0246.
Docs touched by the supervisor's reconcile: `docs/adr/README.md` (0245 not indexed by me), CHANGELOG (cliff).

**What the supervisor should know.** (1) `spacetime sql`/`spacetime call` are themselves HTTP-ephemeral
connections: an owner SQL read of `player_session` always sees its OWN row — measured, not a leak (memory
`spacetime-sql-is-itself-a-connection`); the X8 evidence and ADR-0245 say so. (2) desync-guard found a real client
regression the fix would have introduced (`dismissPending` self-heal relied on the old unconditional presence
delete) — fixed in `client/src/main.ts` onReconnect with a wiring tooth. (3) reducer-security-auditor M-1: a
`client_disconnected` transaction abort would strand a phantom row (R-rb-73-ABORT-PHANTOM, backlog; mitigation =
additive `connected_at_ms` + idle-session reaper, shared with R-rb-73-TOKEN-WEDGE). (4) Code-graph refresh not
run — the change is unmerged; refresh both graphs after the squash-merge. (5) Budget: well under the $150 target
(≈ 8 agent calls). WARN flag observed at ~02:05Z; no new fan-outs after it (verifier was the single closing lens).

## 2026-09-11T~03:30Z — rb-73 PARKED at the PLAN phase: hidden-dependency STOP (fix needs a `player_session` table → files far outside `touches: server-module/src/lib.rs`); plan + ADR-0245 draft checkpointed; NO PR (SUPERVISOR MUST RE-SERIALIZE WITH WIDENED TOUCHES)
**TERMINAL STATE: explicit PARK/blocker — progress committed + pushed, no PR, ledger 0/9 met (X1–X9 authored, none DEFERred).**
Branch `feat/rb-73-disconnect-session-guard` @ 9b6b3e5 (pushed; worktree `.claude/worktrees/rb-73`, forked
from `origin/master@9434dfb`; two wip commits: 3172299 = ADR-0245 draft + regenerated `docs/adr/DIGEST.md`;
9b6b3e5 = the RED gating tests `server-module/src/rb73_session_tests.rs` (9 tests, tester-authored, opus) + the
`mod` line in lib.rs). **The branch is COMPILE-RED by design** — the tests reference the not-yet-existing seam
(`crate::schema::PlayerSession`, `crate::has_live_session`); the orchestrator-run RED proof shows 23 build
errors, every one naming that seam. Main checkout untouched on `master`. Zero production code written —
deliberately: no lib.rs-only increment is coherent (a guard without persisted state is a no-op), and the
auto-resume wrapper re-invoked once after the first park (the in-scope RED tests are what could still move,
per the rb-38 precedent).

**WHY PARKED.** The only SpacetimeDB-idiomatic closure (vendor's own `client_disconnected` example) is a PRIVATE
`player_session {pk connection_id, btree identity}` maintained by `on_connect`/`on_disconnect`, with the side
effects running only when the identity's LAST live connection ends. A new Identity-bearing table in this module
reds THREE independent 41-entry censuses (`DATA_LIFECYCLE_MANIFEST` totality in schema.rs; the m22s6 row-type
registry in accounts_tests.rs; the S9 `M22S9_MANIFEST_TRANSCRIPTION` in evals/account-e2e.eval.mjs + the
`n_parts == 41` literal at accounts_tests.rs:13289 — the third one was missed by the planner and caught by both
review lenses), plus T-VIS (battle-schema-snapshot + baseline), `[G6/declared]` REKEY_MANIFEST
(guest-claim-integrity), the cascade map + frozen reaper literal (accounts.rs/accounts_tests.rs), and
privacy_tests.rs's exportable-false floor. Every one is outside the launched touches. Verified platform facts
(HTTP `/call` fires both hooks under a random ConnectionId; host replays disconnects for dangling `st_client`
rows on launch; rejected connect rolls back; WS idle close 30 s; `__dummy()` connection_id None) are in the plan
and in memory (`http-reducer-call-is-an-ephemeral-connection`, `new-identity-table-forces-three-censuses`).

**SUPERVISOR ACTION (mechanical).**
1. Re-serialize rb-73 with `touches:` = `server-module/src/lib.rs, server-module/src/schema.rs,
   server-module/src/accounts.rs, server-module/src/accounts_tests.rs, server-module/src/privacy_tests.rs,
   server-module/src/trading_tests.rs, server-module/src/rb73_session_tests.rs (new),
   evals/battle-schema-snapshot.eval.mjs, evals/baselines/table-schemas.json,
   evals/guest-claim-integrity.eval.mjs, evals/account-e2e.eval.mjs, docs/adr/0232-*.md, docs/adr/0228-*.md,
   sim-harness/src/bin/mr_load_driver.rs` (+ the always-in-scope docs). Mark FAN-OUT-INELIGIBLE (schema change
   → serial). If `accounts.rs` is refused, the plan's §A11 NotOwned fallback drops accounts.rs/ADR-0228.
2. Relaunch rb-73 RESUMING `feat/rb-73-disconnect-session-guard` (not a fresh worktree); ADR 0245 stays reserved
   (drafted, Status Accepted, `Amends: —` until the reciprocal ADR-0232 edit lands in the resumed run).
3. The resumed run starts at plan step 2 (`memory/projects/monster-realm-rb-73-plan.md` §Steps + §A; step 5,
   the tester, is already done — the specialist must MATCH the tester's pinned literals, plan §A13, never edit
   the tests); `memory/projects/monster-realm-rb-73-progress.md` carries DONE/REMAINING/BLOCKERS; ledger X2-X4
   counts filled (3/2/4), nothing met, nothing deferred.
4. Commit the harness paper trail (plan, progress memo, ledger, this entry) — left uncommitted per convention.

Cost/lenses this run: planner (opus) + reviewer + red-team + /simplify on the plan + doc-keeper (ADR) +
tester (opus, RED tests) — no specialist/verifier (nothing to implement or verify yet; the pre-merge
orchestration audit applies to the resumed run). Budget well inside the $150 target. No stop/warn flags observed.
...
worktree `.claude/worktrees/rb-69` **under the HARNESS root**, forked from `origin/main@bf0c4bf`;
6 commits, all pushed, tree clean). `gh pr merge` NOT run. Main checkouts left untouched.

**SUPERVISOR MERGE INSTRUCTION (mechanical — do NOT re-derive the repo).**
```
gh pr merge 110 --repo mdrewt/claude-harness --squash
```
`mr-state.json`'s `inflight[]` entry for rb-69 carries **no repo field**, and `mr-repo-of` run
against the LIVE spec in the main checkout still returns **`project`** — because the fix that
resolves rb-69's `touches:` placeholder only exists on THIS BRANCH and lands when the PR merges.
So any re-derivation before the merge points at the wrong repo. There is no PR on
`mdrewt/monster-realm` for this slice and there should not be. There is also **no remote CI** on the
harness repo (no `.github/`), so `mr-ci-watch` has no event to wait on — local `just ci` (CI-EXIT=0,
re-verified at 8/8 gates from a foreign cwd) is the whole gate. Merge on the audit alone.

**READ THIS FIRST — THE BRIEF ROUTED THE SLICE TO THE WRONG REPO.** rb-69's seeded `touches:` was
the generator placeholder `(inherit from source slice — REVIEW)`, and `mr-repo-of` falls a non-path
string through to `project`. So the brief said `REPO: project`, gave a project worktree path and
named `mdrewt/monster-realm` as the PR target. The residual's real target is
`specs/monster-realm-v2/M-residual-backlog.spec.md` -> `mr-repo-of` returns **`harness`**. Running
it as briefed would have opened an **empty-diff PR on the wrong repo**, which `mr-repo-of`'s own
docstring names as the reason it exists. Re-routed per `mr-spawn:864`. **Supervisor action: this
will recur for every promoted residual whose touches is the placeholder — resolve `touches:` and
run `mr-repo-of` on it at spawn time, not after.** New memory card
`mr-repo-of-placeholder-touches-misroutes`.

WHAT LANDED (3 files, exactly the declared set): `### rb-27` retitled `RESOLVED, do not build` +
a 7-line retraction appended (rb-44's precedent shape, `M-residual-backlog.spec.md:229/:238-246`);
rb-69's own `touches:` placeholder resolved; and 3 gating tests appended to
`scripts/tests/invariants.test.mjs`. The historical `Deferred with reason:` line is preserved
verbatim — it is a mechanical copy of the residual row's `reason` (`mr-gates:1346`, byte-identical).

**SIX THINGS THE NEXT SLICE SHOULD KNOW:**
1. **The first retraction shipped a NEW false claim, and the red-team caught it.** It said the
   premise "is false". It is not: ADR-0208 is **rb-4's** reserved number (`**Slice:** rb-4 … also
   records the rb-2 / rb-3 decisions`), every `R-rb-3-X9` row still carries `"adr": null`, and the
   supervisor's own word was **"moot"**. The shipped text now says MOOT — deliberately more accurate
   than the handoff prose at `:3642` that calls it "the false … premise".
2. **`memory/projects/gates/` and `mr-residuals.jsonl` are GITIGNORED.** The first design was a
   ledger-driven invariant over them; it would have gone vacuously green in a clean clone and given
   different verdicts for the main checkout vs a worktree at the same commit. The tracked durable
   record is the promoted spec section. `monster-realm-handoff*.md` IS tracked, which is what made
   the cross-document ADR-id correspondence tooth possible.
3. **A narrow freeze window EXEMPTS planted text, it does not exclude it.** Verifier FAIL: one
   `## Addendum` line after the retraction terminated a `^#{1,3} ` window while
   `mr-gates.section_of` still returned the prose below it as part of rb-27 — a green gate over a
   section the tooling and a reader both saw as re-opened. Freeze to the CONSUMER's boundary
   (`^### `) and census the generated region. Card `narrow-window-exempts-planted-text`.
4. **`node --test --test-name-pattern` zero-match is VERSION-DEPENDENT.** `# pass 0` on node
   v18.19.1, `ok 1 - <FILENAME>` / `# pass 1` on **v24.13.1** — which is what the asdf shim gives a
   ledger CHECK. An `EXPECT: # pass 1` therefore passes with its tests deleted (verifier proved it).
   Pin `ok 1 - <exact test name>`. Card `node-test-name-pattern-zero-match-is-version-dependent`.
5. **Declaring `justfile` routes a slice MIXED.** It sits in `mr-repo-of`'s `PROJECT_PREFIXES`,
   tested BEFORE `HARNESS_PREFIXES`, and `mr-spawn:866` REFUSES `MIXED`. That killed the planned
   new-test-file + roster edit; the tests were appended to `invariants.test.mjs` instead
   (`scripts/` is AMBIGUOUS and inherits `harness`). **`just test` runs a NAMED 4-file roster with
   no glob**, so a new test file is otherwise never executed.
6. **The harness repo has no `.github/`.** Local `just ci` IS the full gate — there is no remote CI
   event for `mr-ci-watch` to wait on. Merge as soon as the audit passes.

RESIDUALS REGISTERED: `R-rb-69-X9-test-roster-glob` (-> backlog; `just test`'s named roster means a
new `scripts/tests/*.test.mjs` never runs, and the planned text-assertion guard was measured
forgeable four ways — a real fix derives the roster from the glob) and
`R-rb-69-X10-frozen-pin-cannot-police-meaning` (-> wontfix; mutant C1, measured: inverting the
section AND re-typing the frozen literal in lockstep is green by construction — inherent to every
frozen pin, disclosed rather than papered over).

touches-delta: **none** — the diff is exactly the 3 declared files. boyscout-delta: **none** (a
`justfile` roster-drift guard was planned then cut as circular and forgeable; registered as X9).

ORCHESTRATION: `planner` (opus); `reviewer` + `red-team` + `/simplify` on the PLAN in parallel — all
three changed it materially (reviewer found the MIXED-routing BLOCKER; red-team PROVED 12 CI-clean
bypasses of the planned needle oracle including an INVERTED retraction carrying every pinned token;
`/simplify` showed the class-wide two-sided oracle was a tautology that passes on the unfixed file
AND on a file with the convention erased). A separate `tester` authored the gating tests (it is
hook-blocked from writing under `.claude/`, so it staged them in `/tmp` and the orchestrator
spliced them). A different agent implemented. `reviewer` + `red-team` + `/simplify` in parallel on
the implementation. **`verifier` returned FAIL** and was right twice (items 3 and 4 above); fixed
and re-verified. `reducer-security-auditor`/`desync-guard` NOT spawned — no reducer, schema, netcode
or game-rule change, and a different repo entirely.

Local gate: `just ci` **CI-EXIT=0** (`/tmp/rb-69/ci3.log`) — 106 tests / 0 fail, research-lint
19/0/19, adr-lint 12/0/0, biome clean. Proof of teeth: RED before (25 pass / 3 fail, spec untouched)
-> GREEN after (28 pass / 0 fail); **27 mutants, 25 killed, 2 deliberate documented survivors**
(`memory/projects/gates/rb-69.bite-proof.md`). No ADR (none reserved, none warranted — the shape
copies the live rb-44 precedent; `docs/adr/README.md` untouched).
...
## 2026-09-07T~16:20Z — rb-67 PR#451 open (ADR-0220 declaration-shaped citations) — local gate green, remote CI running
TERMINAL STATE: PR open + local full `just ci` green (CI-EXIT=0 on final HEAD be0056a, 2260/2260) + remote CI running. `gh pr merge` NOT run (supervisor-owned). Ledger **6/6 met, 0 deferred, 0 unmet** (`seed:e3b0c44298fc1c14`; mr-gates seeded 0 criteria — the residual-backlog section carries no literal SHALL — so X1–X6 were authored in the PLAN phase, rb-39/54/60/61/64/65 shape). X5 is MANUAL (by-name RED capture at `memory/projects/gates/rb-67.red-before.md:10`). Run `mr-gates verify --slice rb-67` FROM the worktree (cwd-relative CHECKs; X6 re-runs the full `just ci`, ~8 min). Artifacts: `memory/projects/gates/rb-67.{gates.md,plan.md,red-before.md}`, tester patches under `/tmp/rb-67/`.

WHAT SHIPPED (no ADR consumed; adr_next_free stays 0244): ADR-0220's Decision 1 `Consequence:` and Decision 2 opening now spell the live `pub(crate) fn purge_export_bundles(ctx: &ReducerContext, owner: Identity) -> usize` and cite it **by declaration, not line number** — ADR-0230's anchor form extended from gate anchors to ADR prose and SCOPED to `privacy.rs` in that one document. `server-module/src/privacy_tests.rs` gains 1133 purely-additive lines (zero deletions): one `include_str!` of the ADR, one shared 16-clause `rb67p_adr_violations` collector that does not short-circuit, and two `#[test]`s (live correspondence + a fixtures control through the same collector). The oracle derives the expected signature at test time from `privacy.rs` via the shipped `extract_squashed_fn_sig`/`stripped_for_scan` — never a second hand-typed literal.

RIGHT-SIZED: the `**Extended-by:**` header line was CUT (the residual's own rationale is to use `Extends` to AVOID a reciprocal edit inside 0220; `ADR-0235:8` already carries `**Extends:** ADR-0220`). ADR-0220's other eight `path:line` citations and four now-false prose claims were NOT fixed → `R-rb-67-ADR0220CITES`.

**SEVEN THINGS THE NEXT SLICE SHOULD KNOW (all measured):**
1. **The launch brief was WRONG that ADR-0243 extends ADR-0220** — it extends ADR-0235. Listing it in an `Extended-by:` would have fabricated a back-link. The planner caught it; do not re-introduce it from the same brief text.
2. **A `` ``` `` fence desyncs `md.split('`')` for the rest of the document while leaving the total backtick count EVEN** — so a parity check reports nothing. Any backtick-span scanner over markdown needs an explicit fence ban or fence handling.
3. **A markdown 4-space indented code block is code to a human and invisible to a backtick scan**; and an HTML comment (`privacy.<!-- -->rs:79`) splits a path needle while rendering normally. Both were CI-clean bypasses. rb-67 bans both constructs outright.
4. **A count-pinned nextest gate is forgeable by `#[ignore]` + a same-prefix stub**: it reproduced a byte-identical `28 tests run: 28 passed,` with NO `SKIP [` line, and `just ci` skips `#[ignore]` too. Pin the ROSTER by name (`o.includes('::'+n+'\n')` — names follow `::`, not a space).
5. **A gate CHECK that mutates a repo file and restores by `cp` can race a concurrent gate reading it.** X4 regenerated `docs/adr/DIGEST.md` in place; use the repo's own non-mutating `just adr-digest-check` instead. NOTE this was NOT the whole story — see the flake below.
6. **A one-sided fixed-char window around a path is not a line-citation ban.** `See line 42 of privacy.rs` (digits before the path) and a first digit at offset ≥9 both survived. Use a per-line, two-sided SHAPE predicate; and do NOT squash whitespace document-wide — it fuses across newlines and false-REDs on `` `privacy.rs` (ADR-0126 idiom) `` and on a following numbered list item.
7. **The gates ledger entry syntax is `- [ ] X1: <text>` one-liners, NOT `## X1 —` headings.** A heading-style ledger lints CLEAN and then reports `0/0 met` from `mr-gates check`. Costs a full re-run to notice.

RESIDUALS REGISTERED: `R-rb-67-ADR0220CITES` (8 remaining path:line cites, 3 drifted; 4 unedited claims false at HEAD — :31/:74 cascade "will reuse", :55 "no writer of export_bundle exists", honest-limit 4 both halves shipped); `R-rb-67-CITESHAPE` (U+200B zero-width space in the helper name survives all 16 clauses while RENDERING the stale declaration — stronger than the disclosed prose limit; plus `privacy.rs (79-92)` / `privacy.rs, L79` evade the shape predicate); `R-rb-67-EVALFLAKE` (see below).

**FLAKE — READ BEFORE TRUSTING X6.** `mr-gates check` FAILED X6 twice with `node evals/run.mjs | error: recipe eval failed on line 96 with exit code 1`, then PASSED on the third attempt with NO tree change. Standalone `just ci` (×3), `just eval`, `node evals/run.mjs` and X6's CHECK run verbatim from a shell were ALL exit 0. The DIGEST-race explanation is REFUTED as the whole story (the 2nd failure had the non-mutating X4 already met and not re-executing). The failing eval's NAME was never captured — mr-gates surfaces only the deciding tail line. The verifier ran the two gates that read `ARCHITECTURE.md` (`rekey-contract-surface`, `changelog-freshness`) 3× each against final HEAD: deterministically green, closing slice-causation by measurement. **X6's "met" is a single-sample green on a gate measured to fail twice on an unchanged tree.** Next move: make the eval runner print the failing eval's name on the LAST line, tee the CHECK's full output to a file, and record whether a sibling `just`/`cargo` held the build lock.

ORCHESTRATION (for the audit): planner (opus, caught the ADR-0243 brief error); plan reviewer (REVISE-PLAN, 3 BLOCKING) + plan red-team (10 MEASURED bypasses; MEASURED-REFUTED the plan's top risk, DIGEST drift) + `/simplify` (cut `rb67p_live_decl` for the shipped extractor, cut the `Extended-by:`/T4/2nd include_str!/digest-STOP branch — ~60% of plan mass) in ONE parallel batch; **tester** (opus, separate agent; write guard blocks `.claude/worktrees/**`, so it delivered `/tmp/rb-67/privacy_tests_append.rs` then 14 anchored `OLD:`/`NEW:` patch files, spliced by the orchestrator with per-anchor `count==1` asserts) — TWO rounds, round 2 closing all 6 artifact-red-team bypasses + 2 false-RED traps; implementer = orchestrator; impl reviewer (REQUEST-CHANGES, 2 MAJOR + 10 minor) + artifact red-team (6 MEASURED bypasses) + reducer-security-auditor (PASS) + desync-guard (PASS) in one parallel batch; verifier (REJECT ×3 → APPROVE: stale ARCHITECTURE record, then two wrong verification counts, then stale X6 evidence — every one a real defect in a doc-truth slice). WARN flag `/tmp/mr_warn_rb-67` appeared after the impl-review batch; no new fan-outs after it (the tester and verifier were resumed, not newly spawned).

touches-delta: `ARCHITECTURE.md` (slice record). boyscout-delta: `docs/adr/0220-*.md:33` (1 hunk) — Decision 2's `**Consequence:**` claimed "S3 still owns the full implementation of the cascade arm", false since rb-65; past-tensed and de-lined. NOTE the brief's declared touches said `server-module/docs/adr/0220-*.md`, which does not exist; the file is at repo-root `docs/adr/`.
...
## 2026-09-07T~21:5xZ — rb-68 TERMINAL: PR #453 open, local `just ci` green, 6/6 gates met
Slice rb-68 (residual R-rb-40-ADR0230) reached the loop's terminal state. **Supervisor owns the merge.**
- PR: https://github.com/mdrewt/monster-realm/pull/453 · branch `feat/rb-68-adr0230-mrlog-evidence` · HEAD `e1a6d07`
- Worktree: `projects/monster-realm/.claude/worktrees/rb-68` (clean). Tester sandbox at `/tmp/rb-68-sandbox` (detached worktree — reap with `git worktree remove` after merge).
- Local gate: full `just ci` **CI-EXIT=0** (2263 rust tests, 108 client test files / 3146 tests, 99 evals PASS, observability validate 8/8). `just ci-fast monster-realm-module` 875/875, 0 skipped, clippy `-D warnings` clean.
- Ledger: **6/6 met, 0 deferred, 0 unmet** (seed e3b0c44298fc1c14). X1-X4 re-executed by hand against the FINAL tree after the verifier fix; X6 = the green `just ci`; X5 is MANUAL with evidence at `memory/projects/gates/rb-68.red-before.md`.
- Diff: 3 files — `docs/adr/0230-*.md` (+243/-60), `server-module/src/accounts_tests.rs` (+3335), `ARCHITECTURE.md` (+2). NO production Rust (`git diff master --stat -- 'server-module/src/*.rs' ':!*_tests.rs'` empty, verifier-confirmed).
- CLOSES TWO residuals: R-rb-40-ADR0230 and R-rb-65-ADR0230-PRV120 (the latter targets :124-125/:149-155, fully inside this diff). Supervisor should close both.
- NOTE for the audit: X1/X2/X3's CHECKs were EDITED mid-run — they originally asserted `0 skipped`, which can never hold under a `cargo nextest -E` filter (nextest reports FILTERED-OUT tests in `skipped`). The `SKIP [` assertion that catches `#[ignore]` was retained, and X3 still pins the 24-name roster. The verifier independently judged this a legitimate idiom fix, not a relaxation.
- NOTE: the seed's Scope line is stale in two ways (declared in the ledger as D-A/D-B/D-D) — "exactly one mr_log call" (there are two), "use Extends" (unexecutable; rb-68 mints no ADR), and the census it asks for already ships.
...
Residual R-rb-46-LIBRSMOD closed. server-module: guards.rs + lib.rs get a review-stop comment (ADR-0247) plus an ordinary #[test] in guards_tests.rs (+870 lines) that fails if the crate root's `mod guards;` is ever swapped to a target_arch-cfg-gated wasm twin — closing the rb-46 red-team PoC X4 gap (lib.rs sat outside rb-46's declared touches so no existing scanner/pin caught this class). Per the residual's own EARS text this was NOT a new evals/*.eval.mjs (ADR-0224 supersedes ADR-0010) — an ordinary Rust test using real cargo nextest, mutation-tested 13/13 killed (gate X5). mr-gates verify: 6/6 met CLEAN, spotcheck agrees. mr-audit (hard tier, mandatory read done): orchestration CLEAN (11 agent calls incl. reviewer/red-team/verifier), gating_advisory CLEAN (test-only diff, no removed asserts/skips/suppressions). Diff ⊆ declared touches. Master fast-forwarded e06bf13->79e3432; master CI still in_progress at tick end (PR checks were green pre-merge: ci+e2e both pass) — next tick/event should confirm. Worktree + local branch cleaned.
## 2026-09-11T20:12:44Z — rb-76 merged: grass-path deletion gate (ADR-0246)
PR#460 squash-merged to master (e06bf13), fast-forwarded local master. Worktree + local branch cleaned up.

rb-76 added `guards::require_subject_not_deleting` — the first identity-PARAMETERISED member of the para-4.7 deletion-gate family, refusing a scheduler-opened wild encounter (`begin_encounter` via `movement_tick`) for a walker whose account is mid-grace or terminal. Keyed on the server-derived `player_identity`, never `ctx.sender()`. Containment is mechanical (crate-wide bare-name census test) rather than the structural no-parameter guarantee ADR-0227 D2 claims for the blanket wrapper — deliberately not claimed here since this wrapper needs an identity parameter. Delegates transitively to `accounts::is_pending_deletion` (no re-derived disjunction). The `movement_tick` skip-log/limiter arm for this rejection reason is a separate statement from the existing fainted-party skip, keyed on the new `REJECT_DELETION_GATED` reason constant.

mr-audit: orchestration CLEAN (10 agent calls, roles incl. red-team/reducer-security-auditor/verifier), acceptance CLEAN (1/1 gate met), spotcheck agreed modulo timing-only drift. Hard-tier mandatory diff read performed by supervisor before merge (server-module/src/{battle,guards,movement}.rs) — clean, well-precedented, 6 new tests / 0 deleted. Closed residual R-rb-46-GRASSPATH via `mr-gates residuals close --pr 460`.

Master CI on e06bf13 was in_progress at merge time (checks had already passed pre-merge on the PR itself); not re-polled further this tick per doctrine (no sit-and-poll).
## 2026-09-11T18:02:34Z — Native tick 18:00Z — launched rb-76
Native tick rid=mr-sup-native-20260911T180008Z-1225287-1225823 (18:00Z, cron). Gate-0/1: no live per-run locks, no chain mutex, HOLD-NONE, no resident-session collision (find -mmin -6 empty both repos; only pre-existing untracked proj .codegraph/ index, untouched). Both repos in sync with origin (harness main, proj master 8b90b69). Project master CI green (Nightly run success). No open PRs either repo, no wip branches, queue[]/inflight[]/awaiting_merge[] reconciled.

Gate-3: mr-gates residuals list --unclaimed showed 86 open, oldest ~6.8d -- past t1_promote_days=3 but under t2_stale_days=14, so it does not outrank the already-non-empty queue[] (rb-76,rb-77,rb-78,rb-79, all promoted-but-unlaunched from prior ticks). Per the aging doctrine's fast-path, took the queue's first entry: re-verified rb-76 live -- heading exists in M-residual-backlog.spec.md (source rb-46, residual R-rb-46-GRASSPATH), after: empty, no existing branch/PR/ledger row -- valid.

rb-76: movement_tick -> begin_encounter opens a wild battle for a walker whose account is mid-grace/terminal. rb-46 (PR#428, ADR-0236) gated PvE battle start via the caller-only deletion_gate seam in guards.rs, but that seam is ctx.sender()-parameterised and the grass-path encounter is opened by the scheduler (movement_tick's own schedule reducer, ctx.sender()=database identity, not the walking player) -- the existing wrapper is structurally wrong here per ADR-0227 D2, per the residual's own deferred-reason text. Derived actual touches from the reason text + grep (movement_tick in movement.rs, begin_encounter in battle.rs, existing gate seam in guards.rs): server-module/src/{movement,movement_tests,battle,battle_tests,guards,guards_tests}.rs. Tier=HARD (server-module reducer + security/deletion-gate surface, joined to the PRV1-7 crate-wide thread per ADR-0236 D-d) -> fable@xhigh; budget.fable_ok=true (d7 fable=$77.26/$2298 allowance, guard $2068.2, well under). Pre-allocated ADR-0246 (mr-state.json adr_next_free was 246). mr-spawn LAUNCHED cleanly: leader=1227378, claude_pid=1227381, rid=mr-spawn-20260911T180201Z-1227325, GATES-SEEDED criteria=1. queue-removed rb-76 (queue now: rb-77,rb-78,rb-79). Ledger LAUNCHED row written.

Governor NORMAL (d7=$179.34/2783 eff.). No BLOCKERs, no rate-limit event. Standing down after the single launch action.
## 2026-09-11T17:04:34Z — 17:00Z tick — closed obsolete rb-21 residual (t2-stale), no launch
Native tick mr-sup-native-20260911T170006Z-1208997 (17:00Z, cron). Gate-0: recovered the 16:00Z tick's stranded record first (mr-sup-native-20260911T160007Z-1194397 merged PR#119/rb-79 promotion live but was abruptly killed before its own commit) -- committed+pushed the already-correct state/handoff diff as 33d9366. No live locks/mutex/done files, HOLD-NONE, no active human session (no resident IDE pid, no non-mechanical writes in either repo in last 6min), master CI green at 8b90b69, no open PRs, no parked/wip branches. Gate-3: mr-gates residuals list --json showed 2 residuals past t2_stale_days=14, outranking queue[] (rb-76..79, all still mechanically valid, deferred to next tick's fast path) per doctrine. Investigated both: (1) R-m23-s10-X16/rb-14 (17.4d, promoted 2026-08-30, never launched) -- re-confirmed the 2026-08-30 tick's finding still holds live: its contrast-ratio remediation targets are owned by S8/S9, both explicitly BLOCKED in M23-accessibility.spec.md section 8 on two operator rulings (8.1 colourblind palette redesign, 8.2 canvas sprite-tint art-direction) that have documented 'Recommended default' options but were never applied nor escalated via mr-ask-drew in the 12 days since, despite the spec naming that as the exact escalation path. Left unactioned again (judgment call, not this tick's scope) -- flagged for a future tick to either record decision-defaulted:<question>=<recommended> and unblock S8, or open the mr-ask-drew issue that should have existed already. (2) R-m22-s2-S3-CANCEL-TERMINAL/rb-21 (17.2d, promoted 2026-08-31, never launched) -- read server-module/src/accounts.rs live and found cancel_account_deletion already guards on account_has_terminal_marker() (line 918, real if/reject, not a debug_assert) plus real test coverage (accounts_tests.rs: m22s3_nd_cancel_terminal_guard, m22s3_nd_cancel_terminal_guard_tagged, m22s3_cancelled_deletion_rejects_terminal_input, plus an m22s9 e2e drift pin). Confirmed via gh pr view that the fix landed in PR#404 (feat(m22-s3), merged 2026-09-01, ADR-0225) -- AFTER rb-21's 2026-08-25 disclosure, so a later slice independently closed the exact gap rb-21 was tracking; rb-21 itself was never built (NO-LEDGER). Closed it: mr-gates residuals close --slice rb-21 --pr 404 --force --reason '...' -> RESIDUALS-CLOSED 1. This was the tick's one mutating action (residuals-sink write only; memory/projects/mr-residuals.jsonl is gitignored, no commit needed for it). No slice launched -- queue[] (rb-76/77/78/79) all re-verified mechanically valid, deferred fresh to the next tick's fast path now that no t2-stale item outranks it. Budget NORMAL (d7=$177.72/$2783, fable_ok=true). No BLOCKERs.
## 2026-09-11T16:02:52Z — 16:00Z tick — promoted rb-79 (R-rb-46-TRADINGCFG), no launch
Native tick mr-sup-native-20260911T160007Z-1194397 (16:00Z, cron). Gate-0: no live locks/mutex/done files, HOLD-NONE, no active human session, master CI green at 8b90b69 (Nightly workflow in_progress, unrelated). Gate-3: 87 unclaimed residuals, 76 past t1_promote_days=3 (none past t2=14d) -- outranks the queue fast path (rb-76/77/78 pending launch). Oldest unpromoted by disclosed_at: R-rb-46-TRADINGCFG (6.73d) -- m22-s5 deletion-gate pin hardening: a #[cfg(test)]/#[cfg(debug_assertions)] attribute on propose_trade's gate statement would pass every existing m22s5 pin (guards_tests.rs/pvp_tests.rs) while the wasm ships ungated. Classified per ADR-0224 work-selection scope: targets real Rust test pins, not evals/*.eval.mjs scanner correctness -> promoted (not wontfix) to rb-79 in M-residual-backlog.spec.md via chore PR#119 (mdrewt/claude-harness, squash+auto-merged), queue-added. Queue now: rb-76, rb-77, rb-78, rb-79 (all launch-ready fast-path candidates for the next tick). No slice launched this tick -- promote+doc-PR was the one mutating action. Flagged, not acted on: stray orphaned docker container (loki -verify-config) from rb-75's validate.mjs, PID 1107716, started 07:32:14Z, still running 8+h against a removed rb-75 worktree path -- not a recorded chain pid, left alone per doctrine (never kill outside recorded pids). Budget NORMAL (d7=$176.88/$2783, fable_ok=true).
## 2026-09-11T15:02:44Z — 15:00Z tick record — promoted rb-78 (R-rb-46-MACRORET), no launch
Native tick mr-sup-native-20260911T150038Z-1181038-30033 (15:00Z, cron). Gate-0/1: no live locks/mutex/done files, HOLD-NONE, rate-limit resetsAt in the past. Gate-2: fetched both repos clean, no active human session (no resident IDE pid, no non-mechanical writes in either repo in last 6min), master CI green (rb-75 merged 11:13Z at 8b90b69), Nightly workflow in_progress since 12:03Z (not treated as red). No open PRs, no parked/wip branches. Gate-3: residuals-list --unclaimed returned 88 unpromoted (up from 77 last tick), all past t1_promote_days=3, none past t2_stale_days=14 -- outranks the queue[] fast path (3 entries: rb-78 added this tick + 2 pre-existing) and PLAN §9 derivation per lp-gates §13 aging discipline. Oldest two tied at disclosed_at=2026-09-04T22:35:40Z: R-rb-46-MACRORET and R-rb-46-TRADINGCFG (both source_slice rb-46). Classified both per ADR-0224 work-selection-scope before acting: MACRORET describes a macro-expanded early-return bypassing a deletion gate -- explicitly barred from scanner detection by ADR-0224 itself, framed as a reducer-security-auditor checklist item pending the PRV1-7 crate-wide mechanism; this is a real security-invariant gap (not eval-script-self-correctness), so promotable. TRADINGCFG describes a #[cfg(test)]/#[cfg(debug_assertions)]-gated bypass of the propose_trade deletion gate slipping past the m22-s5 pin -- also a real security-hardening item (ordinary Rust test pin, not an evals/*.eval.mjs scanner), also promotable; left unpromoted this tick, will surface as next-oldest. Picked MACRORET (array-order tiebreak, consistent with prior ticks' documented convention) and promoted it: `mr-gates residuals promote --id R-rb-46-MACRORET` -> rb-78 in M-residual-backlog.spec.md, queue-added via mr-record. Shipped as doc-only chore PR#118 (chore/residual-promote-20260911T150200Z), squash-merged via --auto (merged by GitHub before I could poll further -- clean). Pulled origin/main --ff-only post-merge (d5b7fad..ed04c79). No launch this tick (promote-only action per the one-mutating-action rule; TRADINGCFG and the remaining queue[] entries are next-tick candidates). Governor state NORMAL (d7=$176.10, effective=$176.10 of $2783 weekly). No BLOCKERs. Releasing mutex + writing mr-state.json next.
## 2026-09-11T14:02:11Z — 14:00Z tick — promoted rb-77, queued for fast-path
Native tick mr-sup-native-20260911T140011Z-1167184 (14:00Z, cron). Gate-0/1: no live locks/mutex/done files, HOLD-NONE, rate-limit resetsAt in the past. Gate-2: fetched both repos clean, no active human session (no growing resident IDE pid, no writes in either repo in last 6min), master CI green at 8b90b69 (rb-75 merged 11:13Z), Nightly workflow in_progress since 12:03Z (not treated as red). No open PRs, no parked/wip branches, no open decision issues. Noted but not acted on: a stray docker container (loki -verify-config) from rb-75's ops/observability validate.mjs still running 3+h against a now-removed worktree path — orphaned, not blocking, flagged for a future tick's attention. Gate-3: master not red, nothing to resume, residuals aging check found 77 unpromoted residuals past t1_promote_days=3 (none past t2_stale_days=14) — outranks the queue[] fast path per doctrine. Promoted the oldest (disclosed_at 2026-09-04T22:35:40Z, tied 3-way, picked by array order): R-rb-46-LIBRSMOD, a real security/review-checklist finding (lib.rs #[cfg(target_arch)] guards-module swap defeats the guards.rs byte-for-byte pin) -- NOT an eval-scanner-scope item, so promoted rather than dispositioned wontfix. Appended ### rb-77 to M-residual-backlog.spec.md via chore/residual-promote-20260911T140136Z, PR#117, squash-merged clean. queue-added rb-77 (queue now: rb-76, rb-77). No slice launched this tick -- promote+doc-PR was the one action. Budget NORMAL (d7=$175.32/$2783, fable_ok=true).
## 2026-09-11T13:03:11Z — 13:00Z tick — reconciled rb-75 CI-flake confirmation + residual triage (wontfix x2, promoted rb-76)
Native tick mr-sup-native-20260911T130013Z-1153326 (13:00Z, cron). Gate-0: rb-75 per-run lock showed session_leader 861807 dead + .done EXIT=0 ATTEMPTS=1 -- live re-verify found PR#459 already MERGED (8b90b69, mergedAt 11:13:21Z) and the 12:00Z tick had already reconciled the ledger/residual-close/handoff for that merge, but left the post-merge CI rerun outcome PENDING (run 34592967110 had come back FAILURE on an unrelated flaky e2e test, client/e2e/monster-privacy.spec.ts:394 #help-hint intercept, and a rerun was triggered to confirm). This tick: gh run view 34592967110 -> conclusion=success, confirming the flake -- master CI green. Fast-forwarded local project master efcc457->8b90b69. Reaped the stale rb-75 per-run lock via mr-unlock stale, removed the merged worktree .claude/worktrees/rb-75 and local branch feat/rb-75-archlog-nextfree-notes (git worktree remove --force + branch -D, since squash-merge branches never show as git-merged per the standing measurement rule).

Gate-3: mr-gates residuals list --unclaimed -> 92 open, none past t2=14d. Oldest tier (6.84d, source 18r-b): R-18r-b-I2BACKTICKSTYLE + R-18r-b-I2SAMEPARA, both scoped entirely to doc-truth gate armI2's own citation-token-matching correctness (false-red on unbackticked reword; false-green on a same-paragraph-but-different-line self-contradiction) -- classified eval-tooling-only per Work-selection scope/ADR-0224, dispositioned wontfix (both MED, self-authorized, no mr-ask-drew needed).

Next tier (6.60d/6.59d, source rb-46, 5 tied entries from the rb-46 red-team/auditor): promoted R-rb-46-GRASSPATH (movement_tick -> begin_encounter opens a wild battle for accounts mid-grace/terminal deletion with no §4.7 enforcement seam -- a real privacy/deletion-grace gap, not eval-tooling) -> rb-76 in M-residual-backlog.spec.md, shipped as doc-only chore PR#116 (squash-merged b59d95b; no CI configured on this path so merged directly off CLEAN mergeStateStatus rather than --auto). queue-added rb-76 for the next tick's fast-path launch. Left LIBRSMOD/MACRORET/TRADINGCFG (also rb-46, same age) unclaimed -- each is self-tagged '[reviewer-checklist class, ADR-0224]': real bypasses (macro-hidden early return, #[cfg(target_arch)] guards-module swap, #[cfg(test)] gate bypass) that the residual's own text says should be caught at PR review, not fixed via a new scanner or necessarily promoted as a code-fix slice. No precedent yet for actioning a checklist-class residual -- left for a future tick's judgment rather than rushed onto this one's tail. ERASEWRITERS (source rb-46, 22:52Z, slightly younger) also real (§4.7 gap in heal_party/advance_dialogue/taming grant paths) -- left for the next aging pass.

No slice launched this tick (reconcile + residual triage/promote was the one action, per the one-mutating-action rule). Budget NORMAL (d7=$174.35/$2783, fable_ok=true). No BLOCKERs, no parks. Next tick: fast path -> queue[0]=rb-76, re-verify live, launch (real security/privacy fix, likely HARD tier given it touches movement.rs + a reducer-adjacent identity-parameterised gate -- derive per the Model & effort routing criteria at spawn time).
...
Merged feat/rb-74-librs-citations -> master (efcc4570), squash, branch deleted. Closes residual R-18r-b-LIBRSCITES: 15 stale server-module/src/lib.rs citations across 4 docs retargeted in place (historical number kept, dated live-anchor bracket added); new gating oracle server-module/src/rb74_citation_tests.rs (7-leg, 15/15 mutants killed). No ADR minted (doc+test only). Local master fast-forwarded, worktree removed, stale local branch deleted.

Audit: policy=CLEAN, mandatory_read=false, orchestration=CLEAN (7 roles, opus+sonnet). gating_advisory FLAGGED on a mechanical suppressions_added=1 counter — adjudicated false positive, it matched the word "allow(" inside doc-comment prose in the new test file, not a real #[allow(...)] suppression. acceptance FLAGGED with NOT-REVERIFIED on G10 (mr-audit's own internal verify budget ran out) — adjudicated moot: an independent `mr-gates verify --slice rb-74` run moments earlier (with correct PATH) returned CLEAN 10/10, including G10.

Environment note for future ticks: this session's default Bash PATH resolves /usr/bin/node (v18) and has no cargo at all, causing spurious EVIDENCE-MISMATCH on any gate that shells out to cargo or a Node script using import.meta.dirname. Fix: prepend $HOME/.asdf/shims:$HOME/.cargo/bin to PATH before running mr-gates/mr-audit/just — matches the existing harness-node-toolchain-path-trap memory card, now confirmed to also apply to cargo.

Residual R-18r-b-LIBRSCITES closed via mr-gates residuals close --slice rb-74 --pr 458. 4 new residuals disclosed by rb-74 itself per its PR body (7 more dangling lib.rs: citations outside touches:, ADR-0245's 4 pins already stale, censuses needle-bound to "lib.rs:", oracle gates pointer resolution not sentence truth) — not yet promoted.

master CI for the merge commit (efcc4570) was still in_progress after ~150s of polling; not queued a watcher since checks were already green pre-merge for this identical tree (PR#458 ci+e2e both SUCCESS). Next tick should spot-check `gh run list --branch master --limit 1` before its own next action.
## 2026-09-11T07:57:05Z — rb-74: PR #458 opened, CI in progress — delegated to mr-ci-watch
rb-74 (retarget 15 stale server-module/src/lib.rs doc citations onto dated live anchors, closes R-18r-b-LIBRSCITES) finished: verifier PASS, acceptance 10/10 met, PR https://github.com/mdrewt/monster-realm/pull/458 opened (feat/rb-74-librs-citations), working tree clean, HEAD==origin. Live re-check at 07:56Z: mergeStateStatus UNSTABLE, mergeable MERGEABLE, both ci+e2e checks IN_PROGRESS (started 07:55:09Z) — not yet safe to merge. Delegated CI-wait for PR #458 to mr-ci-watch (detached, pid 707242); resumes via event tick. rb-75 (promoted residual R-18r-b-LOGORDER) remains queued for next launch once a slot frees.
## 2026-09-11T06:01:42Z — 06:00Z tick — launched rb-74 (fast-path queue)
Gate0-2: no live locks/.done/active-human-session; both repos synced, no drift. Gate3 fast-path: queue[] non-empty (rb-74, rb-75 from prior tick's promote); re-verified rb-74 live -- M-residual-backlog.spec.md section exists, non-blocked:, no unmet after: deps, not already merged, all 7 candidate doc paths confirmed present in project repo. Launched rb-74 (opus@high, tier=routine) via mr-spawn: measure/fix stale server-module/src/lib.rs line citations across 9 doc sites (R-18r-b-LIBRSCITES). Detachment+model verified live (session leader 460548, own SID, claude_pid 460551 running --model opus). rb-75 remains queued for next tick. Budget NORMAL (d7=$82.99/$2783, fable_ok=true). No merges/parks this tick.
## 2026-09-11T05:03:33Z — 05:00Z tick — residual triage: rb-74/rb-75 promoted, R-18r-b-NOGATE wontfixed
Gate 0/1/2: no live locks, no .done, no active human session, both repos synced clean (harness ec3c7f3->bd09a26 after this tick's own PR; project at c0d102e). Gate 3: master CI green, no open/parked PRs either repo, queue[] was empty. residuals unclaimed=87, all past t1_promote_days=3 (max 6.54d), none past t2_stale_days=14 -> residual work outranks new PLAN Sec.9 work this tick. Oldest tier (6.54d, all disclosed_at 2026-09-04T16:03:43Z from source slice 18r-b): R-18r-b-NOGATE, R-18r-b-LIBRSCITES, R-18r-b-LOGORDER. Classified per Work-selection scope (ADR-0224): NOGATE's own stated remediation was a new file-colon-line-citation scanner ('eval-shaped, needs its own ADR') -- exactly the retired scanner-script-gate category -- dispositioned wontfix. LIBRSCITES (stale server-module/src/lib.rs line citations across 9+ doc sites) and LOGORDER (ARCHITECTURE.md ADR next-free notes non-monotonic/back-edited) are real doc-content-accuracy findings, not eval-tooling -- promoted to rb-74/rb-75 in M-residual-backlog.spec.md, queued via mr-record queue-add. Shipped as doc-only chore PR#115 (chore/residual-promote-20260911T050236Z), squash-merged bd09a26, branch deleted both local+remote. Did NOT launch this tick (promote is this tick's one action per doctrine; next tick launches rb-74 off the fast-path queue). No mutating action taken beyond the doc-only PR -- budget NORMAL (d7=$81.77/$2783), no BLOCKERs, no parks.
## 2026-09-11T02:56:04Z — rb-73 MERGED — disconnect side-effect gate (ADR-0245)
PR #457 squash-merged (c0d102e) on top of 9434dfb. Private player_session table + last-connection-out gating on on_disconnect closes R-18r-b-DISCONNECTSELF (an HTTP-ephemeral-connection could force PvP forfeit/trade cancel/wild-battle flee on demand via a bare token, without the victim's real WS ever dropping). Fix verified via 9 new rb73_* native #[test]s (X1-X5 GREEN), 14/14 named mutants killed (X6), a live two-connection e2e proof (X8), full evals + just ci GREEN (X7/X9). Acceptance 9/9, seed no-drift. mr-audit: orchestration CLEAN (11 agent calls, expected role roster incl. desync-guard/reducer-security-auditor/verifier); gating_advisory FLAGGED only by hard-tier mandatory-read policy, no real assertion/skip weakening (the one skip_markers_added hit was Rust's .skip(1) iterator, a false positive). Disclosed touches-delta beyond the original lib.rs-only declaration (schema.rs/accounts.rs/tests/evals for the new table + census pins, plus client/src comment-accuracy fixes and a genuine desync-guard H1 fix in main.ts's onReconnect dismiss-lock) was read and accepted -- every extra file carried a why + forcing gate in the PR body, matched exactly by mr-gates render. Residual bookkeeping: R-18r-b-DISCONNECTSELF closed against PR#457; 4 new MED backlog residuals recorded (TOKEN-WEDGE, ABORT-PHANTOM, WILD-HOLD, HOSTWRITES, all unpromoted, none urgent); 2 wontfix (PVP-HOLD bounded by the 60s pvp_deadline_reaper, NOEXEC inherent to the dummy native-host harness). Environment note for future ticks: this session's Bash PATH resolved node v18 (no node:fs/promises glob export) and had no cargo at all, producing false EVIDENCE-MISMATCH on X7/X9 during mr-gates verify -- resolved by prepending ~/.asdf/shims and ~/.cargo/bin before re-running; matches the known 'Harness node toolchain PATH trap'. Worktree .claude/worktrees/rb-73 and local branch feat/rb-73-disconnect-session-guard removed post-merge; remote branch auto-deleted.
## 2026-09-11T02:29:41Z — rb-73: PR#457 open, CI pending — delegated to mr-ci-watch
Native tick native-20260911T022906Z-333956 reconciled the rb-73 event (run finished rc=0 attempts=3 model=fable, session_leader 39037 dead). Live-verified PR mdrewt/monster-realm#457 (feat/rb-73-disconnect-session-guard): state=OPEN mergeable=MERGEABLE mergeStateStatus=UNSTABLE, ci+e2e checks both pending. Per doctrine, spawned mr-ci-watch 457 rb-73 detached (pid 334496) rather than polling; merge will complete via the next event tick once CI resolves. No mutex was held (fast-path standdown path); nothing else changed this tick.
## 2026-09-11T00:58:31Z — Nineteenth weekly review @ 9434dfb — M-postgate-nineteenth-review-residuals inserted (19r-a..d); 0 decision issues
Nineteenth weekly review (generate-improvement-plan) completed against pinned master
`9434dfb5dfa60140e2d46b1aef561eea858b0563` (detached clone /home/mdrewt/mr-review, removed at exit).
Delta reviewed: `1e738fd..9434dfb` (35 commits: rb-40..rb-72, rb-9, 17r-e/f, 18r-a/b). 8 sonnet
lenses, 0 contradictions, 2 independent verifiers; 4 claims checked, 4 CONFIRMED, 0 dropped.

INSERTED: `specs/monster-realm-v2/M-postgate-nineteenth-review-residuals.spec.md` (slices
19r-a..19r-d), PLAN.md §9 post-gate block bullet added directly after the
M-postgate-eighteenth-review-residuals entry (before the retired M-postgate-overlay-registry
note). Discovery per convention: PLAN §9 order + this handoff. No pending-events file.

HEADLINE FINDINGS (all verified at the pinned SHA):
- 19r-a MED client: privacyBanner.ts:333 `confirmPrompt` not gated on `inFlight==='none'` (unlike
  the three action buttons at :330-332) → with delete armed and an export in flight, "Confirm
  deletion" presents as live but the click is silently swallowed (privacyModel begin() guard,
  deliberately pinned by S8T-DELETE-INFLIGHT-REFUSED — model correct, view-model defective).
- 19r-b MED observability (disclosed-but-untracked): rb-65's `account_deletion_cascade` and
  `data_export` audit events have no recording rule/panel; ADR-0243:209 pins the gap to
  R-rb-40-DASH, which rb-66 CLOSED for guest_claim_export_purge only — no open queue item covers
  these two events.
- 19r-c LOW doc drift: ARCHITECTURE.md:492 + ADR-0238:74 still call the reaper self-arm
  request_data_export's "last statement"; false since rb-65 (arm = last WRITE, mr_log = last
  STATEMENT — the code comment at privacy.rs:1566-1569 says so itself).
- 19r-d LOW perf: main.ts:3082-3092 recomputes deriveDeletionCountdown + privacyBannerLabel every
  rAF frame unthrottled; memo only at the DOM-write boundary (main.ts:2882).
Clean bills: server security/privacy lens explicit no-findings on the whole rb-46/47/48 deletion-gate
+ reaper surface; schema strictly additive; no test weakened/skipped/.only'd anywhere in the delta;
spec-vs-code lens confirmed rb-46/47/48/51/52/53/9 closure claims genuinely satisfied; dependency
delta empty; M8.9 module boundaries hold.

OPEN REV-ISSUES: none — zero decision issues opened this cycle (nothing rose to a Drew-level call).
No supervisor stand-aside needed on any answer-fired tick.

DECISION-DEFAULTED (reversible calls taken with documented defaults, no issues):
- decision-defaulted:slice-grouping=four-narrow-slices (one finding per slice, 18r precedent)
- decision-defaulted:19r-a-fix-shape=spec-states-invariant-not-mechanism (disable vs hide left to
  the implementing slice; EARS pins the behavior)
- decision-defaulted:mr-disjoint=skipped (advisory; touches authored pairwise-disjoint by
  construction, 19r-d deliberately scoped to main.ts only to stay disjoint from 19r-a)
- decision-defaulted:rate-limit-defer=proceed (mr-situation: reset time already past at start,
  budget NORMAL)

CLEANUP: review clone /home/mdrewt/mr-review/20260911-9434dfb removed; runner worktrees/branches
untouched (verified against the Phase-0 snapshot); no writes to $PROJ or the clone; no
mr-state.json edits. Visibility-only cost-null ledger row written (tier=review, rev14 precedent,
per DECISION(rev-ledger-row) yes-answer, harness issue #13).
## 2026-09-11T00:02:27Z — rb-73 relaunched after rate-limit crash reconcile
2026-09-08 rb-73 launch (fable@xhigh) crashed EXIT=1/2 attempts with ZERO code changes: both attempts'
API calls hit a seven-day rate-limit REJECTION (status=rejected, rateLimitType=seven_day, resetsAt
2026-09-11T00:00:00Z) mid-session -- not a defect in the slice or approach. No branch/PR/worktree
existed to clean up (pure read-only investigation). Native tick at 2026-09-11T00:00:08Z (right at
reset) reconciled: reaped the dead per-slice lock (mr-unlock stale), removed rb-73 from
mr-state.json inflight, and RELAUNCHED it fresh (not a resume -- no partial state to resume from).

ADR note: the crashed run's lock still showed adr_reserved=244, but docs/adr/0244 was already
consumed by an unrelated slice in the interim (adr_next_free had advanced to 245) -- reallocated
245 for this relaunch to avoid a collision.

New run: mr-spawn-20260911T000154Z-38978, leader=39037, claude_pid=39040, fable@xhigh, tier=hard,
touches=[server-module/src/lib.rs], adr=245. Target unchanged: fix the disconnect-side-effects
self-trigger abuse (R-18r-b-DISCONNECTSELF / rb-73 spec section in M-residual-backlog.spec.md) with
an ordinary Rust #[test] per ADR-0224.
