...
- **Arming criterion for slice 2, pre-registered:** after 10 merged slices carrying ledgers, promote
  ...
  of them (fires on all 10 or 0 of 10 ⇒ no information — lp-04's lesson) and every adjudicated false
...
11. **The completeness number in the PR body is generated and byte-compared** (a pure integer).
...
- **`defer_count >= 2` on the same origin criterion ⇒ `mr-ask-drew`.** Twice-deferred is a sizing or
  feasibility problem, not a scheduling one — mirroring `mr-launch.sh`'s existing "cost-parked a
  SECOND time ⇒ ask Drew" precedent.
