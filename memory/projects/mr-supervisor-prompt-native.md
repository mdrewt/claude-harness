...
1. **One step per tick:** reconcile from LIVE ground truth → at most ONE mutating action → record → exit. Action ∈ {merge a finished slice · launch next eligible slice(s) · resume a parked slice · stand down · finish}. **One composite allowed — merge→launch** after the merge completes FULLY (squash-merged, audits clean, worktree/branch cleaned, ledger+handoff+state recorded), re-deriving eligibility fresh incl. the final active-session re-probe. Rooted runs use the **Opus 4.8** model for planning and writing code (mr-launch.sh default).
