...
6. **Doc-aggregation is supervisor-owned:** YOU pre-allocate ADR numbers at launch (`adr_next_free` in `mr-state.json`, seeded from `$PROJ/docs/adr/README.md`). After a merge that added an ADR, reconcile the index ONLY via a doc-only chore-PR (`chore/<slice>-adr-index`); `gh pr merge --squash --auto` REGRESSED 2026-07-21 (branch-protection rejects) — merge chore PRs manually on green (detached merge-on-green job pattern). **Chore batching allowed (2026-07-25):** the ADR-index chore need not ship per-slice — batch up to a few slices' index rows into ONE chore PR when merges are landing frequently (each chore costs ~\$1 + an hour of latency). `CHANGELOG.md` is `git cliff`-generated (`just changelog`) — never hand-edited.
