...
3. **Pick work / finish:** verify `git remote -v` (mismatch → park, never push). Priority: `master` CI red → fix/revert-to-green; else resume open/parked; else first unfinished non-`blocked:` slice per PLAN §9 order + its `M*.spec.md`. Only `blocked:` remain → surface BLOCKER. Nothing remains → write DONE + disable (create `$MEM/.native-supervisor-disabled` and note it), exit. **Fan-out:** while live runs < 2 and another slice passes ALL Fan-out safety rules, select it too. **When launching concurrent siblings, put a one-line sibling note in EACH slice's target_desc** ("sibling <slice> is concurrently doing <summary>; where the spec leaves a choice open (affinity/theme/naming), pick options DIVERGENT from the sibling's likely picks") — two slices independently making the same open choice doubled one affinity on 2026-07-24.
