...
- **`mr-audit --slice S --log L --repo R --base B --head H [--tier hard]`** — runs both mechanical audits. `CLEAN` → proceed. `FLAGGED` or `AUDIT-ERROR` → YOU read the diff/log and adjudicate before merging (hard-tier is always FLAGGED: mandatory read). The mechanical pass cannot judge semantic test-weakening (changed thresholds, suppression legitimacy) — that judgment is yours on every FLAGGED verdict.
...
6. **Doc-aggregation is supervisor-owned:** YOU pre-allocate ADR numbers at launch (`adr_next_free` in `mr-state.json`, seeded from `$PROJ/docs/adr/README.md`). After a merge that added an ADR, reconcile the index ONLY via a doc-only chore-PR (`chore/<slice>-adr-index`); **Doc-only chore PRs: `gh pr merge --squash --auto` is the standard path again** (Drew enabled repo auto-merge 2026-07-25; verified live on PR#245; the 2026-07-21 regression is RESOLVED — no more detached merge-on-green babysitting). Batching still allowed: combine a few slices' index rows into one chore when merges land frequently. **Feature-slice PRs are NEVER auto-merged** — they wait for YOUR audited merge (mr-audit gate), always. `CHANGELOG.md` is `git cliff`-generated (`just changelog`) — never hand-edited.
...
3. **Pick work / finish:** verify `git remote -v` (mismatch → park, never push). Priority: `master` CI red → fix/revert-to-green; else resume open/parked; else first unfinished non-`blocked:` slice per PLAN §9 order + its `M*.spec.md`. Only `blocked:` remain → surface BLOCKER. **BLOCKER discipline (Drew 2026-07-25):** park-and-wait ONLY for irreversible, architectural, security, spec-contradicting, or explicitly-gated (playtest) decisions. For REVERSIBLE scope/content choices (roster counts, naming, tuning ranges, doc placement): take the documented default or most conservative option, record `decision-defaulted:<question>=<choice>` in the handoff + ledger notes, and PROCEED — Drew reviews asynchronously and reverses via a follow-up slice if needed. (An 88-min stall on a reversible roster question, 2026-07-25.) Nothing remains → write DONE + disable (create `$MEM/.native-supervisor-disabled` and note it), exit. **When you raise a gate-class BLOCKER (playtest gate, irreversible decision):** also write `$MEM/.blocked-on-human` — first line = one-line reason, plus ` wake_file=<absolute path>` naming the artifact whose appearance lifts the gate (playtest gate: `wake_file=/home/mdrewt/projects/ai-apps/claude-harness/specs/monster-realm-v2/playtest-feedback-2026-07.md`). Cron ticks then stand down FREE until the wake file exists; events and manual runs still get through. **Fan-out:** while live runs < 2 and another slice passes ALL Fan-out safety rules, select it too. **When launching concurrent siblings, PRE-ASSIGN any open choice space in each target_desc** — you have both spec entries in hand, so partition deterministically ("sibling A: take affinities X/Y; sibling B: take Z/W") rather than asking runs to guess divergence; if the space cannot be enumerated, name the sibling and its summary so runs can at least coordinate away from it. (Two slices independently making the same open choice doubled one affinity on 2026-07-24.)
...
- N ≤ 2 runs; merges serial + verifier-gated; orchestration depth = 1; one supervisor chain at a time (the mutex).
