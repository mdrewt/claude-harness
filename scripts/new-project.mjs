...
if (!name || !stack) { console.error('Usage: new-project.mjs <name> <stack> [description]'); process.exit(1); }
if (!/^[a-z0-9][a-z0-9-]*$/.test(name)) { console.error('Name must be kebab-case.'); process.exit(1); }
...
if (!existsSync(stackDir)) {
  ...
  console.error(`Unknown stack "${stack}". Available: ${stacks.join(', ')}`); process.exit(1);
}
const dest = path.join(ROOT, 'projects', name);
if (existsSync(dest)) { console.error(`projects/${name} already exists.`); process.exit(1); }
...
async function copyMerge(src, dst) {
  ...
  for (const e of await readdir(src, { withFileTypes: true })) {
    ...
    if (e.isDirectory()) { await copyMerge(s, d); continue; }
...
const TOKMAP = { NAME: name, STACK: stack, YEAR: String(new Date().getFullYear()), DESCRIPTION: description };
const TOKEN_RE = /\{\{(NAME|STACK|YEAR|DESCRIPTION)\}\}/g;
const BINARY_EXT = /\.(png|jpe?g|gif|ico|webp|woff2?|ttf|otf|eot|pdf|zip|gz|tar|mp3|wav|mp4|wasm|so|dylib|dll|exe|bin)$/i;

async function fill(dir) {
  for (const e of await readdir(dir, { withFileTypes: true })) {
    const p = path.join(dir, e.name);
    if (e.isDirectory()) { if (e.name === '.git') continue; await fill(p); continue; }
...
try {
  ...
  try { execFileSync('git', ['config', 'user.email'], { cwd: dest, stdio: 'ignore' }); }
  catch { hasIdentity = false; }
  if (!hasIdentity) {            // only set a local fallback when no global identity exists
  ...
  execFileSync('git', ['commit', '-qm', 'chore: scaffold from template'], { cwd: dest, stdio: 'ignore' });
} catch { /* git optional */ }
