...
function fill(text, name) { return text.split('{{NAME}}').join(name); }
...
let names = [];
if (only) names = [only];
else { try { names = (await readdir(path.join(ROOT, 'projects'), { withFileTypes: true })).filter(d => d.isDirectory()).map(d => d.name); } catch {} }

if (names.length === 0) { console.log('No projects found under projects/.'); process.exit(0); }
...
for (const name of names) {
  ...
  else console.log(`${name}: ${apply ? 'updated' : 'drift in'} ${changes.length} managed file(s):\n` + changes.map(c => '   - ' + c).join('\n'));
}
