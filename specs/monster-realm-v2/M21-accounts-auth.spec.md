...
- [ ] Deployment-timed follow-up (separate commit — OQ1/OQ4/OQ5/OQ6 are all resolved; it stays a
  separate commit because it needs a real deployed Better Auth instance). **HARD SEQUENCING GATE (per
  the 13r-c handoff entry, 2026-08-10T04:02Z, memory/projects/monster-realm-handoff.md): this task MUST
  NOT land before `13r-c-2` lands** (undrafted as of this writing — migrates
  `evals/trade-escrow-guards.eval.mjs` off its old whole-crate comment-stripping approach onto
  `rust-scan.mjs`). Do not relax this from a supervisor tick without re-reading that handoff entry in
  full first. Flip `ALLOWED_ISSUERS`/`ALLOWED_AUDIENCE` in `accounts.rs` to the
