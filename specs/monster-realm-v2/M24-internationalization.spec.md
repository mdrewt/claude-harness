...
| **S3** | Migration batch A — the two densest views (**43** sinks: 26 + 17, E7). *[As built, m24-s3 / ADR-0259: the real scanner sees 42 sinks (25 + 17) of which 24 FAIL §2.2 — the ratchet counts FAILING sinks, so `HARDCODED_CEILING` 81 → 57; plus 5 scanner-invisible hoisted literals (`'You'`, `'Opponent'`, the three outcome texts) migrated with no ratchet effect; 32 keys added. Merged PR #TBD.]* | `client/src/ui/battleView.ts`, `client/src/ui/pvpView.ts`, `client/src/ui/i18n/messageIds.ts`, `client/src/ui/i18n/catalog.en.ts`, `client/src/ui/i18n/__fixtures__/i18n-hardcoded.json` | S2 |
| **S4** | Migration batch B — the mid-density views (**77** sinks: 13+17+15+16+16, E7). *[As built, m24-s4 / ADR-0260: the real scanner sees 77 sinks split 13+16+15+16+17 (raisingView 16, shopView 17 — the E7 per-file split was off by one twice; the sum holds), of which **46 FAIL** §2.2 — `HARDCODED_CEILING` 57 → 11; plus 7 scanner-invisible hoisted literals (tradeView's two `#renderSide` heading args + four `#actionLabel` returns, boxView's `prompt()` label) migrated with no ratchet effect; 54 keys added (96 total). PR #490.]* | `client/src/ui/evolutionView.ts`, `raisingView.ts`, `boxView.ts`, `tradeView.ts`, `shopView.ts`, `client/src/ui/i18n/messageIds.ts`, `client/src/ui/i18n/catalog.en.ts`, `client/src/ui/i18n/__fixtures__/i18n-hardcoded.json` | S2, S3 |
| **S5** | Migration batch C — the tail (**45** sinks, E7) incl. the `<bdi>` wrap at `leaderboardView.ts:57` + `client/index.html` static strings. *[As built, m24-s5 / ADR-0261: the real scanner sees **55** sinks across the 13 files (7+5+6+4+4+4+4+4+4+2+2+7+2), of which **11 FAIL** §2.2 — the entire whole-tree remainder, so `HARDCODED_CEILING` 11 → **0** and I18N-19 is met here (S6 moves no ratchet: `main.ts`'s four are `reportError` arguments, never counted); plus 5 scanner-invisible hoisted literals migrated with no ratchet effect; 16 keys added (112 total); `#help-title`/`#rename-submit`/`#tradepropose-submit` literals removed from `index.html` and resolved in `show()`; `#help-hint` stays static (ADR-0151 D2) so `chrome.helpHint` has zero call sites — S7's DEAD-KEY gate must exempt it or S6 must own it; `dir="ltr"` added on `index.html:2`. PR #TBD.]* | `client/src/ui/tradeProposeView.ts`, `dialogueView.ts`, `claimView.ts`, `sessionView.ts`, `renameView.ts`, `menuView.ts`, `leaderboardView.ts`, `helpView.ts`, `errorOverlayView.ts`, `questLogView.ts`, `healView.ts`, `privacyView.ts`, `evolutionNotice.ts` (post-spec files inside S2's whole-tree scan, ADR-0257 D7), `client/index.html`, `client/src/ui/i18n/messageIds.ts`, `client/src/ui/i18n/catalog.en.ts`, `client/src/ui/i18n/__fixtures__/i18n-hardcoded.json` | S2, S4 |
| **S6** | Boot wiring: `negotiateLocale` call, `lang`/`dir` flip in `main.ts`; `evals/i18n-catalog-shape.eval.mjs` **(new)** incl. the 47-char width budget. *[As built, m24-s6 / ADR-0262: the gate ships as the co-located `client/src/ui/i18n/catalogShape.test.ts` (ADR-0224 retires new evals; same vehicle substitution as S0/S2) plus the sibling `client/src/main.i18nBoot.test.ts`; the hunk is MODULE-scope (`?locale=` getAll → `navigator.languages` → `Object.keys(CATALOGS)`, `setLocale`, one `lang` + one `dir` write); the resolver is imported as `t as i18nT` (a11yCopy's `t` sits in a frozen ADR-0206 region); the six `chrome.status.*` `reportError` literals migrate with **no ratchet effect** (never sinks — the JSON baseline is unchanged at 0); `chrome.helpHint` stays static and S7's DEAD-KEY roster exempts exactly it; SHAPE-03 encodes the ADR-0256 D5 grammar (the regex here would red `chrome.helpHint`); width = NFC code points; `fr` = {one, many, other} so S7's catalog may not use `oneOther`. PR #493.]* | `client/src/main.ts`, `client/src/ui/i18n/catalogShape.test.ts` **(new)**, `client/src/main.i18nBoot.test.ts` **(new, sibling)**, `client/src/ui/i18n/__fixtures__/i18n-hardcoded.json` (declared, unchanged) | S1 |
...
each of S3/S4/S5 ratchets `HARDCODED_CEILING` down by its own migrated count (43 → 77 → 45, with `main.ts`'s remaining 4 migrated in S6; 43+77+45+4 = 169 exactly, so the arithmetic closes) *[As built: the ceiling counts FAILING sinks (81 at S2), so the per-batch decrements are the failing counts — S3 measured 24 (81 → 57), ADR-0259; S4 measured 46 (57 → 11), ADR-0260; the 169 total-sink arithmetic is the E7 census, not the ratchet.]*; S6's landing takes it to 0 and
