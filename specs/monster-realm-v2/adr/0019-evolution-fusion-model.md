...
**Supersessions:** this amendment supersedes ADR-0147 (evolution-fusion-taxed-carry — the A0 work to repair fusion and carry fields through fuse() is explicitly not inherited) and ADR-0149 (item-triggered-evolution-content — the discrete item-trigger design is subsumed into the essence-source model, a generalization rather than a deployment of that design). Full detail on what survived the redesign (species 30/31 BST/flavor/migration-fidelity, the shop-stocking convention, Steamveil sidegrade flavor) vs. what was discarded (the reduce-to-average fusion model, A0's field-tax math, A1's preview-reducer, C's lineage-tracking) is in §3 of the implementation-ready spec ("Retirement of prior-milestone slices" — corrected pointer 2026-08-02; an earlier draft's section numbering didn't survive into the final 7-section spec).

(Editorial note, 2026-08-02: this Supersessions paragraph belongs to THIS amendment specifically — it was found stranded after two later addenda during a routine consistency check and moved back here, immediately following the amendment it summarizes. Nothing about its content changed.)
...
### Addendum — 2026-08-02 (five of six §4 decisions confirmed; Trust expanded to 5 named tiers; cutscene delivery designed)

**Confirmed as specced, no design change:** Bond retirement (folds into Trust); full essence reset on evolution, including against the already-shipped species-7 collision case.

**Confirmed STILL PROVISIONAL, not final (kept separate from the "confirmed" bucket above to avoid misreading — spelled out again in the spec §4, "STILL OPEN, deliberately"):** the tier cap. Drew reviewed and confirmed the provisional framing itself is accurate, NOT that `<= 5` is now a locked-in final count — this remains the one genuinely open item of the six.

**Confirmed with a scope addition:** strict tier-per-edge advancement (R5, no skipping) stays exactly as specced — AND Drew clarified dead ends (a species with no outgoing evolution path at all) are explicitly allowed, expected to be rare. No rule change was needed: R10 only ever required incoming reachability, never an outgoing edge, so this was already permitted — it's now recorded as intentional rather than an open question.

**Confirmed with a roadmap commitment:** evolution-graph visibility stays fully public during development, but Drew committed to a version of spoiler-gating before production release — flagged in the spec (§4) that `evolution_path`'s current fully-public table shape will need to become an owner-scoped/filtered surface for that later work, a schema change, not a pure client one.

**Resolved, not just confirmed:** Trust-tier granularity. `TrustTier` expands from the original 4 variants to 5, Drew's exact naming and order: `Hostile` / `Wary` / `Neutral` / `Friendly` / `Devoted`. Applied throughout the spec (EG1-2, EG1-5, EG1-6, EG3-3/EG3-4). One latent inaccuracy caught and fixed while doing this: EG1-2 previously described the schema default as "the lowest variant," which was never actually consistent with the Bayesian-smoothed formula (`K=10`, zero history → `smoothed = 0.5` exactly, the true midpoint) even under the original 4-variant scheme — corrected to default to `Neutral`.

**Post-evolve cutscene, delivery mechanism designed then simplified:** Drew specified: each step of a chain evolution gets its own cutscene, shown in order; cutscenes are shown at the first available opportunity (e.g. on reconnection after a disconnect); the server expects an explicit client ACK. A first draft of this (one queue row per event, plus a reducer that rejects out-of-order ACKs) was reviewed and simplified after Drew asked whether ACK was the right mechanism at all: same guarantees, less machinery, using one PRIVATE row per player holding an ordered `Vec` of pending reveals (reusing the `Vec<SpacetimeType>` pattern `evolution_path.essence`, EG1-4, already establishes in this same spec) instead of a multi-row table with bespoke ordering enforcement — Vec append order IS display order, nothing separate to get out of sequence. ACK is deliberately at-least-once, not exactly-once: a crash between showing a cutscene and the ACK call landing re-shows it next session, a considered tradeoff (rarely repeating a cosmetic notification beats a chance of silently losing one). Still deferred to the `uxd` milestones, not built in EG1–EG5 — recorded here in full because the schema shape matters to whoever picks it up.

Implementation-ready spec (updated): `M-evolution-essence-graph.spec.md` §4 (five items resolved), §2 (EG1-2, EG1-5, EG1-6, EG3-3, EG3-4 — Trust-tier rename), §6 (cutscene delivery design).

### Addendum — 2026-08-02 (movement call site resolved; whole-party Quality-Time confirmed)

**Trigger:** the review pass surfaced a real, unbuildable gap in EG2-8/EG2-12's movement call site (no monster reference on `Character`; the assumed reducer looked like it fired per-frame). Rather than leave it as an open engineering question, Drew asked for a concrete design and investigated the actual live movement/prediction code with him before proposing one.

**Resolution:** `check_and_evolve`/`accrue_quality_time` hook into `enqueue_move` — the one genuinely player-triggered movement reducer live in the shipped client — NOT `movement_tick` (the scheduled reducer that actually computes tile-by-tile movement). This isn't a stylistic choice: `movement_tick` is scheduler-only, and calling growth-crediting logic from a scheduled reducer would violate this project's own no-idle-accrual invariant (EG2-9) — a player could queue moves and walk away while the tick kept crediting Quality Time. `enqueue_move`'s existing backpressure gating (re-issues only once the previous step drains) turns out to already throttle it to roughly once per tile during sustained movement — the earlier "fires every frame" review finding was based on the code's shape (checked every frame) rather than its actual gated call cadence. `set_move`/`clear_queue` are real, tested, but currently unused in the shipped client (a prior attempt at exactly this "replace queue on rapid direction change" idea was reverted after measured bugs — up to 1.75 tiles of backward teleport, permanent desync); not wired in now since nothing calls them, flagged for the same treatment if ever revived.

**Which monster(s):** Drew confirmed Quality Time credits the WHOLE active party, not just the lead — "playtime with the monster actively in the party," his own directive's literal wording. `enqueue_move` resolves the party via the existing `lead_party()` helper (already used by the grass-encounter path) and loops the credit/check over every returned monster id.

Implementation-ready spec (updated): `M-evolution-essence-graph.spec.md` EG2-8, EG2-12 (movement call site fully specified; the earlier "pre-build blocker" note removed as resolved).
