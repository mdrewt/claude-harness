...
| 3 | **The harness generator scaffolds a green build** (`just new monster-realm spacetimedb-game`) on the real toolchain (Rust 1.96.0, spacetime 2.6.0, wasm-pack, `just`) | M0 | The whole build order assumes the template works | Scaffold, `just ci` → empty-but-green |
