...
|---|---|---|---|---|
| 1 | ~~**RLS (`client_visibility_filter`) actually filters**~~ — **ANSWERED: NO. Closed, do not re-verify.** | ADR-0015, M6, M16, M21 — all owner-private data | Hidden genes / ranked PvP picks / PII leak | **RESOLVED (ADR-0040/0087/0180 D18a; re-verified at 2.8.1 by ADR-0197):** `client_visibility_filter` is behind `feature = "unstable"` **and** carries the crate's own `// TODO: RLS filters are currently unimplemented, and are not enforced.` — byte-identical at crate 1.12.0 and 2.8.1. It publishes clean and filters nothing. The ADR-0015 fallback is now the *permanent* design: must-never-leak data lives in a **private table** read through an owner-scoped `#[view]` (`my_wallet`/`my_conversation`/`my_account`/`my_monster_pub`). Re-open **only** if that TODO is deleted upstream |
...
| 3 | **The harness generator scaffolds a green build** (`just new monster-realm spacetimedb-game`) on the real toolchain (Rust 1.96.0, spacetime 2.8.1, wasm-pack 0.15.0, `just`) | M0 | The whole build order assumes the template works | Scaffold, `just ci` → empty-but-green |
